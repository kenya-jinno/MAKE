...
%  MDPI MAKE submission — main_paper_MAKE42.tex
%  English version of main_paper_NN42.tex (Japanese draft, version 42)
%  Version 42: revisions in response to Peer_Review_Report41.md (minor to
%  moderate revisions)
...
Under the manifold hypothesis, this paper proposes a \textbf{conditional,
diagnostic ID-guided protocol that closes the loop of estimation, setting,
and verification}: a search window for $m$ is derived from an estimate
$\hatdID$ of the intrinsic dimension of the input data
...
on MNIST and Fashion-MNIST (3+ seeds each), the three steps operate under a
reproducible decision rule (MNIST passes; on Fashion-MNIST the rule
diagnostically detects a boundary case on the over-pruning side), and on
the MNIST grid used in this paper the
ID-guided search window reduces the training runs of a full grid search by
roughly 60--80\% while attaining the grid-best downstream linear-probe
accuracy inside the window.
...
inflates on natural images, and for Conv-VAEs the AU truncation fails at
standard $\beta$, with $\beta$-strengthening restoring truncation (3 seeds)
but not order consistency.
...
\begin{enumerate}
  ...
  \item \textbf{Multi-seed validation and quantified applicability
        ...
        The Step-3 evidence for Fashion-MNIST (3 seeds) provides a
        diagnostic detection example of the decision rule---V1 pass with a
        violation of the lower V2 bound (Section~\ref{sec:exp_fashion});
        the $\beta$-strengthened Conv-VAE recovery (3 seeds) achieves V1
        only, not V2; the natural-image AU plateau is a single-seed
        exploratory observation (Appendix~\ref{app:exploratory}); and
...
3 seeds), and---as quantification of
the applicability boundary---Conv-VAEs and natural images (CIFAR-10, SVHN).
There is a clear asymmetry in the state of validation:
\textbf{for fully connected architectures we provide complete multi-seed
validation} (MNIST passes the rule; on Fashion-MNIST the rule
diagnostically detects a boundary case on the over-pruning side), whereas
\textbf{for convolutional architectures and natural images the protocol is
at the stage of probing its applicability boundary} (a 3-seed
$\beta$-strengthening boundary experiment and single-seed exploratory
observations)---the reader should keep this asymmetry in mind throughout.
...
$(2.3, 6.8)$; recomputed per seed, the verdicts are unanimous at the
...
AU saturates below $\hatdID$ (lower V2 bound violated) & Over-regularization ($\beta$ too large for the data/decoder), or the Step-1 reference includes directions the VAE prunes at this $\beta$ & Cross-read the MSE trend; consider reducing $\beta$; treat $[\mathrm{AU}, \hatdID]$ as the candidate range (Fashion-MNIST; Section~\ref{sec:exp_fashion}) \\
...
structure---from this sign, the surrogate errs on the side of
\textbf{underestimating truncation} (the actual truncation is stronger than
the surrogate predicts; details in Appendix~\ref{app:proofs}).
\end{Remark}
...
$0.27$; unanimous across the per-seed verdicts of the 3 seeds) but V2
...
First, \textbf{manifold entanglement}: MNIST consists of centered,
...
like a union of submanifolds whose local dimension varies strongly across
classes and regions (Pope et al.~\cite{pope2021intrinsic} likewise report
large inter-class variation of the ID).
...
$\delta_T$ of Proposition~\ref{thm:twonn_stability}) as $m_{\mathrm{ref}}$
grows.
...
\begin{itemize}
  \item \textbf{Validation scope}: multi-seed validation of the full
        ...
        single-seed with separately quantified uncertainty; the
        Fashion-MNIST Step 3 is a diagnostic detection example with V1
        passing and the lower V2 bound violated;
        Section~\ref{sec:exp_fashion}).
...
3 seeds; in the 3-seed Fashion-MNIST verification the decision rule
diagnostically detected a boundary case on the over-pruning side, with V1
passing and the lower V2 bound violated), with the ID-guided window
reducing the training runs of the full
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\supplementary{The following supporting information is submitted together
with the manuscript for peer review:
(i) the experiment scripts, in one-to-one correspondence with the
experiment names in Table~\ref{tab:exp_list};
(ii) the analysis scripts that regenerate every table and figure of the
main text from the training logs;
(iii) the training logs (JSON, all seeds); and
(iv) environment information (random seeds and library versions).
Upon acceptance, these materials will be deposited in a public repository
(GitHub/Zenodo) and the URL/DOI will be added to the final manuscript.}
...
and the prediction of the threshold $\lambda_j > \beta\tau^2$ errs on the
side of \textbf{underestimating truncation (overestimating the number of
active dimensions)}.
