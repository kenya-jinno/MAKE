...
%  MDPI MAKE submission — main_paper_MAKE40.tex
%  English version of main_paper_NN40.tex (Japanese draft, version 40)
%  Version 40: revisions in response to Review_codex39.txt (accept with
%  minor corrections)
...
%    - MDPI production placeholders (dates, article no., DOI, ORCID) left
%      for the editorial stage (presentation 1)
%  Compiled with official mdpi.cls
%  Compile: pdflatex main_paper_MAKE40.tex  (x3 for cross-references)
...
the AU truncation fails at $\beta = 4$, with $\beta$-strengthening
restoring truncation but not order consistency (single-seed exploratory
runs).
...
\begin{enumerate}
  ...
  \item \textbf{Multi-seed validation and quantified applicability
        ...
        over-pruning detected by the MSE cross-check; 3--5 seeds); on
        fully connected VAEs on MNIST, the AU verification is multi-seed
        ...
        (DANCo, ESS), and the 3-seed reference-model comparison
...
3 seeds; Fashion-MNIST Step 3 is single-seed), and---as quantification of
the applicability boundary---Conv-VAEs and natural images (CIFAR-10, SVHN).
...
mean-verdict lower edge is $2.24$).
...
observations (the strengthened-$\beta$ Conv-VAE runs and Fashion-MNIST
Step 3) are labeled as such where summarized, with details in
...
(the AE window endpoints use the TwoNN--MLE envelope, whereas the VAE
verification point follows the primary-TwoNN rule of
...
(Appendix~\ref{app:exploratory}).
Step 1 is thus multi-seed for Fashion-MNIST; the VAE-side AU verification
(Step 3), by contrast, remains a single-seed preliminary observation with
the same tendency as MNIST (Appendix~\ref{app:exploratory}), and multi-seed
replication of the full protocol is future work.
...
\begin{itemize}
  \item \textbf{Validation scope}: multi-seed validation of the full
        protocol is limited to synthetic manifolds and the MNIST AU
        verification (the MNIST Step-1 sweep is single-seed with separately
        quantified uncertainty; Fashion-MNIST is multi-seed for Step 1
        only).
