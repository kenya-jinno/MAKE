%% main_paper_NN37.tex
%% MDPI MAKE 投稿用日本語草稿
%% 入力データの内在次元に基づく AE/VAE ボトルネック次元の選定（版37）
%% 版37の変更点：Review_codex36.txt（Minor revision; Major 1--7, Minor 1--10）への対応
%%   - Torus m=3--5 の AU=2（過剰刈り込み）を新実験 Exp-Syn-Verify（5シード）で確定し，
%%     「m > demb で常に AUsat=demb」の誤記述を修正．MSE 検証との併用が境界域を
%%     検出することを明示し，V1/V2 の限界（AU=demb の検証ではない）を議論（Major 1）
%%   - 検証点 m_ver の事前指定規則を定義，シード毎判定（全会一致）と
%%     閾値不変範囲（V1: 0.67--0.94, V2: 2.3--6.6）を追記，
%%     「保守的な報告規約であり統計検定ではない」と明記（Major 2, Minor 3）
%%   - [dhat, 2dhat] を「既定探索窓」と改称し一般性を弱める（Major 3）
%%   - サロゲートのレート活性化と AU 分散閾値の対応を橋渡しする段落を追加（Major 4）
%%   - Step 1 の妥当性記述を緩和（参照 AE 経由は実用的安定化；PCA は対照例示），
%%     DANCo 784 退化の説明を追加（Major 5, Minor 6, 7）
%%   - 探索コスト削減を「本論文の MNIST グリッド構成での実測」に限定し，
%%     Step 1 を 3 シードで数える対称な会計も併記（Major 6, Minor 5）
%%   - 命題1にコンパクト性・位相的埋め込みの仮定を明示（Major 7）
%%   - 要旨の限定句を整理，kappa の注意，その他 Minor 対応（Minor 1, 4, 8）
%% コンパイル: platex main_paper_NN37.tex && platex main_paper_NN37.tex && dvipdfmx main_paper_NN37.dvi
...
本論文は，多様体仮説のもとで
\textbf{$m$の探索窓を入力データの内在次元の推定値$\hatdID$から導き
...
真の次元既知の合成多様体と MNIST の全結合 VAE（いずれも3シード以上）では，
3ステップが再現可能な合否判定規則のもとで機能し
（Swiss Roll：$\AUsat = \dtrue = 2$；Torus：$\AUsat = \demb = 3$，
ただし境界域$m \leq 4$の過剰刈り込みは MSE 検証が検出；
...
Conv-VAE では$\beta = 4$で AU 遮断が働かない
（$\beta$増強での回復は単一シードの探索的観察にとどまる）．
...
\begin{enumerate}
  ...
  \item \textbf{多シード実証と適用境界の定量化}（§\ref{sec:exp}）：
        ...
        過剰刈り込みは MSE 検証が検出する；3--5シード）と MNIST の全結合 VAE
...
検証点$m_{\mathrm{ver}}$は\textbf{テストグリッド中$2\hatdID$以上の最小の点として
AU 曲線を観察する前に指定する}（事後選択を避けるため；
複数の$m_{\mathrm{ver}}$での確認を推奨する）．
...
閾値$0.9$・係数$3$は作業既定値であり，本論文の全設定で判定は
V1 閾値$\in (0.67, 0.94)$・V2 係数$\in (2.3, 6.6)$の範囲で不変，
かつ全多シード設定でシード毎判定も全会一致であった（§\ref{sec:exp}；
補助資料の解析スクリプトで再計算できる）．
...
（表\ref{tab:failure}第4行；Torus での実例は§\ref{sec:exp_syn}）．
...
表\ref{tab:failure}第4行（位相的障害）が発火し，余裕の拡大が指示される．
...
\textbf{MSE 検証が位相的障害を検出}（$m_{\mathrm{ver}}$超で MSE が3倍改善）——
表\ref{tab:failure}第4行に従い余裕を拡大し，
$m = 6$で全検証が成立して\textbf{合格}（$\AUsat = 3 = \demb$；3シード，$\sigma = 0$）．
...
\textbf{判定}（Step 3 規則；$m_{\mathrm{ver}} = 20$はグリッド中$2\hatdID$以上の
最小点として事前指定）：AU $= 13.3 \pm 0.5$，活性率$0.67 \leq 0.9$（V1 成立），
...
実験面では真の次元既知の合成多様体
（十分な余裕のもとで$\AUsat = \dtrue$または$\demb$；
Torus 境界域$m \leq 4$の過剰刈り込みは MSE 検証が検出；3--5シード，$\sigma = 0$）と
