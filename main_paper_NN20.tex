...
\begin{enumerate}
  ...
  \item \textbf{自然画像での AU 飽和条件の徹底探索と stable identification 基準}：
        ...
        $\beta_{\max}=80$・薄型デコーダ条件で$m=512$付近に AU 飽和プラトーの初観測に成功した（strict な stable identification は未達）．
        「stable identification」の5項目判定基準を明文化し（§\ref{sec:natimg_stable_id}），
        $\mknee$検出法の比較実験（実験 Exp-NatImg-Knee，§\ref{sec:exp_natimg_knee}）で自然画像での最安定手法を特定した．
\end{enumerate}
...
\textbf{Exp-Real-Geom} & \textbf{実データ幾何比較：複数モデル TwoNN 安定性} & \textbf{MNIST+Fashion-MNIST, AE/IsometricAE/CAE/DAE} & \textbf{TwoNN 安定性条件の実証} & \textbf{RQ2, RQ3} \\
\textbf{Exp-NatImg-Sweep} & \textbf{自然画像拡張スイープ} & \textbf{CIFAR-10, $m\in[32,1024]$, $\beta_{\max}\in\{10,20,40,80\}$} & \textbf{AU 飽和条件の徹底探索} & \textbf{RQ1} \\
\textbf{Exp-NatImg-Knee} & \textbf{$\mknee$検出法比較（自然画像）} & \textbf{CIFAR-10, 4手法比較} & \textbf{検出法の再現性比較} & \textbf{RQ1} \\
...
AU 飽和プラトーの初観測（$\beta_{\max}=80$，薄型デコーダ，CIFAR-10）を達成した（strict な stable identification は未達）．
