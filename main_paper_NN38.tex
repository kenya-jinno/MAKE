%% main_paper_NN38.tex
%% MDPI MAKE 投稿用日本語草稿
%% 入力データの内在次元に基づく AE/VAE ボトルネック次元の選定（版38）
%% 版38の変更点：Review_codex37.txt（Minor revision; Minor 1--8, 体裁 1--5）への対応
%%   - Step 1 の主推定量を「収束した TwoNN 値（整数丸め）」として事前登録し，
%%     MLE/DANCo/ESS はクロスチェックと規定．MLE 側の 2*dhat が次グリッド点を
%%     越える場合はそこでも検証（MNIST: m=32 も合格）（Minor 1, 6）
%%   - Torus マージンの記述を「m >= 5（細グリッド）；粗グリッドでは
%%     m = 6 = 2 d_emb が最初の検証済み点」に要旨・Step 2・結論で統一（Minor 2）
%%   - 「本文はすべて多シード」を「VAE 検証結果はすべて多シード」に限定し，
%%     単一シードの再利用スイープ（Step 1 参照 AE スイープ・下流 AE 側）を明記（Minor 3）
%%   - 閾値不変域を「多シード平均判定（V1: 0.67--0.94）」と
%%     「シード毎判定（V1: 0.70--0.93）」に分離（拡張 Exp_AU_Rule.py で算出）（Minor 4）
%%   - Fashion-MNIST の Step-1 推定値を dhat ~= 10 に統一
%%     （グリッド設計値 d=9 は予備推定と明記）（Minor 5）
%%   - 報告要件の充足主張を軟化（Minor 7）
%%   - 命題3への参照を「条件付き安定性のスケッチ」として一貫させる（Minor 8）
%%   - 「V1 のみ回復（V2 は未達）」の表現を統一（体裁 2）；表8キャプションで
%%     探索窓と検証点を区別（体裁 3）；Torus の 3 倍 MSE 改善に m=4→6 を明記（体裁 4）；
%%     要旨を軽く圧縮（体裁 5）
%% コンパイル: platex main_paper_NN38.tex && platex main_paper_NN38.tex && dvipdfmx main_paper_NN38.dvi
...
（Swiss Roll：$\AUsat = \dtrue = 2$；Torus：$m \geq 5$で$\AUsat = \demb = 3$，
境界域$m \leq 4$の過剰刈り込みは MSE 検証が検出；
...
（$\beta$増強は遮断を回復するが$\hatdID$とのオーダー整合は未達；
単一シードの探索的観察）．
...
\begin{enumerate}
  ...
  \item \textbf{多シード実証と適用境界の定量化}（§\ref{sec:exp}）：
        ...
        なお，Fashion-MNIST の Step 3・Conv-VAE の$\beta$増強による回復
        （V1 のみで V2 は未達）・自然画像の
...
V1 閾値$\in (0.67, 0.94)$・V2 係数$\in (2.3, 6.6)$の範囲で不変であり，
シード毎に再計算した判定も既定閾値で全会一致，かつ
V1 閾値$\in (0.70, 0.93)$・V2 係数$\in (2.3, 6.6)$の範囲で不変であった
——シード毎の下限がやや狭いのは，$m_{\mathrm{ver}} = 20$で AU $= 14$となる
MNIST の1シードによる（§\ref{sec:exp}；
補助資料の解析スクリプトで両範囲を再計算できる）．
...
\textbf{本文の VAE 検証結果はすべて3シード（42, 123, 777）以上の多シード実験}である．
...
単一シードの探索的観察は\ref{app:exploratory}に分離した．
