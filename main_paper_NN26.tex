...
%% AE/VAEにおける有効潜在次元の診断的解釈（版26）
%% コンパイル: platex main_paper_NN26.tex && dvipdfmx main_paper_NN26.dvi
...
Conv-VAE・自然画像設定では，stable identification の5項目判定基準を定義し，
AU 診断が困難になる条件を定量化することで，
フレームワークの適用可能範囲（validity range）と applicability boundary を明確化する．
\end{abstract}
...
\begin{figure}[H]
  ...
  \caption{Exp-Mnist-PCA: (Left) MNIST PCA eigenvalue spectrum and linear-theory water-filling level ($\beta\tau^2 \approx 1.18$, $d^*=10$). (Right) Comparison of linear theory prediction (red solid line; sharp saturation assumed at $d^*=10$) and observed AU$(m)$ curve of nonlinear FC-VAE (per seed and mean). The gradual AU increase in the nonlinear VAE provides the theoretical basis for $\mknee$ instability.}
  \label{fig:em}
\end{figure}}{}
...
\textbf{$m=256$超への拡張（実験 Exp-Conv-M512，詳細は付録 C）}：
$m \in \{384, 512\}$まで拡張しても AU$\approx m$（全次元活性）が全3シードで維持され，
$\mknee$はグリッド端 512 に張り付いたままであった（付録 C，表参照）．
...
300 エポック，3シードを組み合わせた網羅的スイープを実施した（詳細は付録 C）．

\textbf{主要知見（3点）}：

\textbf{（1）高$\beta$・薄型デコーダ条件での AU plateau-like behavior の初観測}：
$\beta_{\max}=80$・薄型デコーダ条件で$m \in [512, 1024]$にわたって AU$\approx 24$付近の
plateau-like behavior が観測された（条件付き・予備的結果）．
後述の stable identification 5条件（§\ref{sec:natimg_stable_id}）は全て未達であるが，
「自然画像でも十分大きな$\beta$とデコーダ容量制限の下で AU 飽和が起こりうる」
という実験的証拠が初めて得られた（mechanistic validation の前進）．

\textbf{（2）Stable identification の5条件：全て未達}：
シード安定性（$\sigma(\mknee)=64$），グリッド安定性（密化で$\pm35\%$変動），
$\beta$安定性（$\beta_{\max}=64$では飽和消失），AU プラトー安定性（$\sigma=13\%$，基準5\%），
TwoNN・AU 整合性（$\mknee \approx 512 \gg 3\hat{d}_\mathrm{ID} \approx 75$）の5条件全てが未達であり，
strict な stable identification は今後の目標である（付録 C に5条件の詳細判定を示す）．

\textbf{（3）TwoNN ID の頑健性}：
全条件で潜在 TwoNN ID は$24.7$〜$25.4$（$\sigma \leq 0.6$）と安定し，
Pope ら\cite{pope2021intrinsic}の$\approx 26$と整合する．
TwoNN は$\beta$やデコーダ容量の変動に対して頑健である．

\textbf{$\mknee$検出法比較（Exp-NatImg-Knee，詳細は付録 C）}：
自然画像では AU 曲線が MNIST より滑らかで固定$\tau$法が最適とは限らない．
4手法（固定$\tau$，二階差分，平滑化曲率，区分線形）の比較では，
\textbf{平滑化後曲率法}が最も再現性が高かった（$\sigma=96$，詳細は付録 C）．
ただし全手法で stable identification 条件1（$\sigma \leq 0.1\cdot\mknee = 51.2$）は未達であり，
検出法改善よりも AU 飽和自体の安定化が先決であることが確認された．
...
実験面では，MNIST・Fashion-MNIST において AU 値（$\sigma \leq 0.5$）・TwoNN ID（$10.03 \pm 0.12$）の
高い再現性を3シードで確認し，密グリッド実験で$\mknee = 10.3 \pm 0.9$と安定化した
（Exp-Mnist-Multi，Exp-Mnist-Dense；§\ref{sec:exp_multiseed2}，§\ref{sec:exp_el}）．
実データ幾何比較実験（Exp-Real-Geom，§\ref{sec:exp_real_geom}）により，
「等長性改善より$m \gg \hat{d}_\mathrm{ID}$が TwoNN 安定性の支配的条件」を
4モデル・2データセットにわたって実証した（命題\ref{thm:twonn_stability}に理論的根拠）．
...
CIFAR-10 では Pope ら\cite{pope2021intrinsic}の推定値（$\approx 26$）を独立に再現した（Exp-Nat-Cifar）；
$\mknee$は AU 曲線形状から読む探索的ヒューリスティックであり，密グリッド・アンサンブルによる安定化が必要である
（フレームワークの適用可能性の3段階整理は表\ref{tab:applicability_tier}に示す）．

\begin{table}[H]
\centering
\caption{Three-tier summary of framework applicability. A: stable operation confirmed; B: partial confirmation with limiting conditions; C: stable identification not achieved.}
\label{tab:applicability_tier}
\small
\begin{tabular}{p{3.5cm}p{2.5cm}p{2.3cm}p{2.3cm}p{3.0cm}}
\toprule
Setting & TwoNN ID & AU monitoring & $\mknee$ & Evidence \\
\midrule
\textbf{FC-VAE + MNIST/F-MNIST}
  & A ($\sigma\leq0.12$)
  & A ($\sigma\leq0.5$)
  & B (dense: $10.3\pm0.9$)
  & Exp-Mnist-Base/Dense, Exp-Fashion \\
\addlinespace
\textbf{Conv-VAE + MNIST}
  & A (stable)
  & B (onset at $m\geq96$)
  & B (identifiable at $\beta\geq10$)
  & Exp-Conv-M256, Exp-ConvV-HiBeta \\
\addlinespace
\textbf{Natural images (CIFAR-10/SVHN)}
  & A (Pope et al.\ consistent)
  & B (plateau-like at $\beta_{\max}=80$, thin decoder)
  & C (5 criteria unmet)
...
$\beta_{\max}=4$での AU 飽和未観測は$\beta$設定不足に起因することを高$\beta$実験（Exp-ConvV-HiBeta）で逆証明し，
CIFAR-10 拡張スイープ（Exp-NatImg-Sweep）では$\beta_{\max}=80$・薄型デコーダ条件で
AU plateau-like behavior を初観測した（stable identification 5条件は全て未達；付録 C）．
