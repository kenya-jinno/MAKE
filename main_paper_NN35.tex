%% main_paper_NN35.tex
%% MDPI MAKE 投稿用日本語草稿
%% 入力データの内在次元に基づく AE/VAE ボトルネック次元の選定（版35）
%% 版35の変更点：Review_codex34.txt（Major 1--8）を受けた全面再構成（一から書き直し）
%%   - 中心主張を「入力データの内在次元（多様体次元）によるボトルネック次元の選定」に一本化
%%   - ID 誘導型選定プロトコル（推定→設定→AU 検証）を主貢献として提示（Major 1）
%%   - m_knee を名前付き診断指標から降格し，付録の探索的解析へ移動（Major 2）
%%   - 定理を「サロゲート活性化条件（命題）」に改称，tau^2 の定義と KL 近似の注意を
%%     主張本体に明記（Major 3）
%%   - TwoNN の安定性 vs 妥当性の区別を方法論の一部として本文前半に配置（Major 4）
%%   - 本文は多シード実験のみで構成，単一シード実験は付録で探索的と明記（Major 5）
%%   - 従来のモデル選択基準（validation loss / ELBO）との関係を考察に追加（Major 6）
%%   - 分量を大幅圧縮（NN34 比で本文約半分）（Major 7）
%% コンパイル: platex main_paper_NN35.tex && platex main_paper_NN35.tex && dvipdfmx main_paper_NN35.dvi
...
表現の質を左右する最も基本的な設計パラメータであるにもかかわらず，
実務では試行錯誤や慣習により選ばれることが多い．
本論文は，多様体仮説のもとで
\textbf{$m$を入力データの内在次元（多様体次元）の推定値$\hatdID$から選定し，
その選定を VAE の Active Units（AU）で検証する}という
ID 誘導型選定プロトコル（推定→設定→検証の3ステップ）を提案する．
理論面では，（i）自己交差のない再構成に$m \geq \demb \geq \dID$が必要という位相的下界と，
（ii）線形ガウス設定における水充填型のサロゲート活性化条件
$\lambda_j > \beta\tau^2$（$\lambda_j$：入力共分散の固有値，$\tau^2$：観測ノイズ分散）を根拠として，
「$m$は$\hatdID$より大きめに設定すれば，VAE の KL 正則化が余剰次元を自動遮断し，
AU がその遮断結果を報告する」という選定原理を与える．
実験面では，真の次元が既知の合成多様体（3シード）でプロトコルが正しい次元を返すこと
（Swiss Roll：$\AUsat = \dtrue = 2$，Torus：$\AUsat = \demb = 3$），
MNIST（3シード）で$\hatdID = 10.03 \pm 0.12$（TwoNN）と AU 増加の鈍化域が整合することを示す．
さらに，プロトコルの適用限界を定量化する：内在次元推定は$m \gg \hatdID$の参照表現でのみ安定し
（4モデル比較，MNIST・Fashion-MNIST，3シード），
自然画像では$m \approx 2\hatdID$を超えると推定値が膨張する；
Conv-VAE では$\beta$が小さいと AU 遮断が機能しない（$\beta=4$で$\dstar > 512$）．
本検証は中規模データ・全結合型 VAE を中心とし，
大規模自然画像への完全適用と変化点検出の独立検証は今後の課題である．
\end{abstract}
...
本論文の中心的主張は一文で述べられる：
\begin{quote}
\textbf{ボトルネック次元$m$は入力データの内在次元の推定値$\hatdID$から
「$m \gtrsim \hatdID$（位相的余裕を含む）」として選定でき，
VAE の Active Units（AU）——KL 正則化が実際に使用している座標数——が
その選定の学習後検証信号として機能する．}
\end{quote}
この主張の根拠は理論と実験の両面から与えられる．
...
余剰次元を自動的に遮断（posterior collapse）し，AU が実効的な使用次元数を報告する」
ことを理想化設定で保証する——すなわち，
\textbf{AE では$m$の選定そのものが表現次元を決めるのに対し，
VAE では「大きめの$m$＋AU による読み取り」という安全側の運用が可能}になる．
...
\begin{enumerate}
  \item \textbf{ID 誘導型ボトルネック次元選定プロトコル}（§\ref{sec:protocol}）：
        ...
        VAE：$m \gtrsim 2\hatdID$の大きめ設定），（S3）AU・再構成誤差による学習後検証，
        の3ステップからなる実行可能な手順を，判定規則・失敗モード・報告要件とともに与える．
  \item \textbf{選定原理の理論的根拠}（§\ref{sec:theory}）：
        ...
        「VAE は$m$の過剰を自動修復するが，その遮断閾値は$\beta$に依存する」
        という運用上の含意を導く．
  \item \textbf{多シード実証と適用境界の定量化}（§\ref{sec:exp}）：
        真の次元既知の合成多様体（3シード）でプロトコルが正しい次元を返すことを確認し，
        MNIST（3シード）で$\hatdID \approx 10$--12 と AU 鈍化域の整合を示す．
        さらに適用限界を定量化する：
...
本プロトコルの名前付き出力とはせず，探索的解析として付録\ref{app:knee}に位置づける．
...
（DANCo\cite{ceruti2014danco}はより精密だが計算コストが高く，今後の課題とする）．
...
本研究は，この機構を「$m$の過剰設定を自動修復する安全装置」として
選定プロトコルに組み込み，その定量的根拠として
...
AU 値がほぼ不変であることを確認済み；付録\ref{app:sens}）．
...
\begin{tabular}{c}
  ...
  \fbox{\parbox{0.78\linewidth}{\centering
    ...
    AE：$m \in [\hatdID,\, 2\hatdID]$を候補範囲とする（位相的余裕を含む） \\
    VAE：$m \gtrsim 2\hatdID$と大きめに設定（KL が余剰次元を自動遮断）
  }} \\[3pt]
  $\Big\downarrow$ \\[1pt]
  \fbox{\parbox{0.78\linewidth}{\centering
    \textbf{Step 3：学習後検証} \\[1pt]
    VAE：AU$(m)$の飽和値・鈍化域が$\hatdID$近傍にあるか（多シード） \\
    AE：MSE$(m)$の急改善が$m \approx \hatdID$〜$2\hatdID$で止まるか \\
    不整合の場合：失敗モード表\ref{tab:failure}で原因を切り分け
  }}
\end{tabular}
...
命題\ref{thm:twonn_stability}と実験（§\ref{sec:exp_real_geom}）で裏づける．
...
よって$m \in [\hatdID,\, 2\hatdID]$を候補範囲とし，
位相的障害（$\demb > \dID$；Torus 型）の可能性を余裕として吸収する．
用途が再構成精度重視なら範囲の上側，圧縮重視なら下側を選ぶ．

\textbf{VAE の場合}：命題\ref{thm:linear_vae}の含意により，
$m$が必要次元を超えても KL 正則化が余剰次元を遮断するため，
\textbf{$m \gtrsim 2\hatdID$と大きめに設定するのが安全側の運用}である．
...
\begin{itemize}
  \item \textbf{VAE：AU 検証}．AU$(m)$が$m$より小さく飽和（または増加が鈍化）し，
        その水準が$\hatdID$と同一オーダーにあれば，
        「データの多様体次元を反映した圧縮が起きている」ことの証拠となる．
        単純な多様体では急峻な飽和（$\AUsat = \dtrue$または$\demb$）が，
        実データでは緩やかな鈍化が現れる（§\ref{sec:exp_mnist}）．
  \item \textbf{AE：MSE 検証}．MSE$(m)$の急速な改善が$m \approx \hatdID$〜$2\hatdID$で
        鈍化するかを目視確認する（アルゴリズミックな肘点検出器は提案しない；
        変化点の自動検出はグリッド・シード依存性が強く，付録\ref{app:knee}の
        探索的解析にとどめる）．
\end{itemize}
...
滑らかな$d$次元多様体$\mathcal{M} \subset \R^N$を連続な AE により
自己交差なく再構成し，理想的にゼロ再構成誤差を達成するためには，
$m \geq \demb \geq d$が必要である
（$\demb$は$\mathcal{M}$の位相的最小埋め込み次元）．
\end{proposition}

\begin{proof}
ゼロ再構成誤差には$f|_\mathcal{M}: \mathcal{M} \to \R^m$が連続な単射である必要があり，
$d$次元多様体の$\R^m$（$m < \demb$）への連続単射埋め込みは
位相的埋め込み次元の定義より存在しない\cite{hatcher2002algebraic}．
\end{proof}
...
線形エンコーダ・デコーダとガウス事前分布を持つ$\beta$-VAE を考える（詳細は付録\ref{app:proofs}）．
...
非線形実験を解釈するための\textbf{解釈的サロゲート}である（導出は付録\ref{app:proofs}）．
\end{proposition}

\begin{remark}[プロトコルへの含意]\label{rem:implication}
（i）$m > \dstar$の余剰次元は最適解で遮断される——
...
\begin{remark}[経験的 AU との乖離]
実際の非線形 VAE では各$m$で独立に確率的最適化を行うため，
経験的 AU$(m)$は非単調な揺らぎを持ち，
...
（ii）\textbf{実データ検証}——MNIST・Fashion-MNIST でプロトコル全体が機能するか
（§\ref{sec:exp_mnist}，§\ref{sec:exp_real_geom}）；
（iii）\textbf{適用境界}——Conv-VAE・自然画像でどの条件が破れるか（§\ref{sec:exp_boundary}）．
\textbf{本文の結果はすべて3シード（42, 123, 777）以上の多シード実験}であり，
単一シードの探索的観察は付録\ref{app:exploratory}に分離した（例外は本文中に明記する）．
...
Fashion-MNIST：300 エポック（Exp-Real-Geom；$n_{\mathrm{train}} = 20{,}000$）．

\textbf{ID 推定}：TwoNN（$k = 2$）\cite{facco2017twonn}，
MLE（$k = 10$；$k \in \{5, 10, 20\}$で安定性確認済み）\cite{levina2005mle}．
AU 閾値$\delta = 10^{-2}$（付録\ref{app:sens}）．

\textbf{再現性}：
実験コード・設定・乱数シード・学習ログ（JSON）・主要な表と図の再生成スクリプトは
査読時に補助資料として添付し，採録時に公開リポジトリ（GitHub/Zenodo）の URL と DOI を確定する．
...
$m$を$\demb$より大きく設定しても，VAE（$\beta = 4$）の AU は
Swiss Roll で$\AUsat = 2 = \dtrue$，Torus で$\AUsat = 3 = \demb$に
全シードで飽和した（標準偏差 0.0）．
一方，vanilla AE の潜在座標使用数は$m$に張り付く（自動遮断なし）——
命題\ref{thm:linear_vae}の含意（i）（iii）と整合し，
「VAE は大きめの$m$を自動修復し，AU がその結果を報告する」という
プロトコルの中核機構が ground truth 上で確認された．
...
Torus・M\"obius Band での$\AUsat = \demb$の対応は前処理（標準化）と
十分な収束に依存する条件付きの経験的観測であり，
理論的必然性は主張しない（単一シードの位相多様体実験は付録\ref{app:exploratory}）．
...
（3シードで$10.03 \pm 0.12$；$m_{\mathrm{ref}} = 64$），
MLE（$11.3$--$11.9$）も同一オーダーを与える．
複数$m_{\mathrm{ref}}$での収束と2推定量の一致という Step 1 の要件を満たし，
\textbf{MNIST の参照値$\hatdID \approx 10$--$12$}を得る．
対照として線形 PCA は累積寄与率 80\%に 43 次元，90\%に 86 次元を要し，
非線形多様体構造を線形法が大幅に過大評価することがわかる（図\ref{fig:e4_dualaxis}左）．

\begin{table}[H]
\centering
\caption{MNIST Step 1: TwoNN/MLE estimates through reference AEs of
increasing $m_{\mathrm{ref}}$ (300 epochs, single seed 42 for the sweep;
the 3-seed value at $m_{\mathrm{ref}} = 64$ is $10.03 \pm 0.12$).}
\label{tab:mnist_s1}
\small
\begin{tabular}{rcccc}
\toprule
$m_{\mathrm{ref}}$ & MSE & TwoNN ID & MLE ID & CKA \\
\midrule
...
AE 側でも MSE の急改善は$m \approx 16$--20 で鈍化し（表\ref{tab:mnist_s1}），
候補範囲$[10, 24]$と整合する．
...
(3 seeds, 300 epochs). Left: per-seed AU$(m)$; right: mean$\pm$std band.
AU values are highly reproducible ($\sigma \leq 0.5$).}
...
一階差分比率に基づく変化点検出（閾値$\tau_{\mathrm{knee}} = 0.25$；付録\ref{app:knee}）を
...
変化点検出はグリッド設計の補助にとどめる（詳細な感度分析は付録\ref{app:knee}）．

\subsubsection{Fashion-MNIST}

Fashion-MNIST では Exp-Real-Geom（§\ref{sec:exp_real_geom}；3シード，300 エポック）により
$\hatdID \approx 9$（TwoNN，$m_{\mathrm{ref}} = 5\hatdID$--$10\hatdID$で$9.9$--$10.1$に収束）を得た．
VAE の AU 検証を含むフルプロトコルの多シード適用は，
単一シードの予備観察（付録\ref{app:exploratory}）で MNIST と同傾向を確認したが，
多シード再現は今後の課題である．
...
（$d = \hatdID$；MNIST は 10，Fashion-MNIST は 9）でスイープし，
...
$\beta_{\max} = 10$では$m = 256$で AU $= 93$（36\%），
$\beta_{\max} = 20$では AU $= 66$（26\%）と明確な飽和傾向が誘発された
（Exp-ConvV-HiBeta；\textbf{単一シードの探索的観察}，詳細は付録\ref{app:exploratory}）．
$\beta$の増大で遮断が低次元側へ移動するという定性的予測が的中しており，
...
CIFAR-10・SVHN の Conv-VAE（3層 Conv，$\beta_{\max} = 4$，150 エポック，3シード，
...
\textbf{自然画像では$m_{\mathrm{ref}} \approx 2\hatdID$付近でのみ推定が信頼でき，
「大きいほど良い」は成り立たない}（表\ref{tab:failure}第3行の失敗モード）．
...
Step 1 がスイープに先立って候補範囲を$\hatdID$近傍に絞り込み（探索コストの削減），
Step 3 の AU が「性能とは独立に，何次元が実際に使われているか」を報告することで
冗長次元の有無を直接可視化する．
実際，MNIST では MSE 検証・AU 検証・$\hatdID$の3者が同一の候補範囲$[10, 24]$を支持した．
一方，本論文は下流タスク精度や disentanglement 指標との系統比較は行っておらず，
「ID 誘導型の選定が下流性能の観点でも最適か」の検証は今後の課題である．
...
（iv）CIFAR-10 での既報値との整合，という複数の間接証拠に支えられているが，
不偏性の直接証明ではない．
このため本プロトコルは$\hatdID$を「候補範囲の中心を与える参照値」として運用し，
Step 3 の検証で閉ループを構成する．
残された課題として，原理の異なる推定量（DANCo\cite{ceruti2014danco}・ESS）との系統比較と
ブートストラップ信頼区間の報告を挙げる．
...
\begin{itemize}
  \item \textbf{検証範囲}：フルプロトコルの多シード検証は合成多様体と MNIST に限られる
        （Fashion-MNIST は Step 1 のみ多シード）．
        Conv-VAE の$\beta$増強による回復（Exp-ConvV-HiBeta）は単一シードの探索的観察であり，
        多シード再現が必要である．
  \item \textbf{自然画像での Step 3}：AU 飽和の安定同定は未達であり，
        $\beta_{\max} \in [40, 120]$の密スイープ・デコーダ容量制御・シード安定性の改善が
        今後の課題である（付録\ref{app:exploratory}）．
  \item \textbf{$\beta$依存性}：AU 検証の読み取りは$\beta$に依存する
        （$\beta \in [2, 8]$で AU パターンの安定性を確認済み；付録\ref{app:sens}）．
...
（S2）$\hatdID$に位相的余裕を加えた$m$の設定（VAE は大きめ＋KL 自動遮断），
（S3）AU・MSE による学習後検証——として肯定的かつ運用可能な回答を与えた．
理論面では位相的下界と水充填型サロゲート活性化条件$\lambda_j > \beta\tau^2$が
プロトコルの各ステップを根拠づけ，
実験面では真の次元既知の合成多様体（$\AUsat = \dtrue$または$\demb$；3シード，$\sigma = 0$）と
MNIST（$\hatdID = 10.03 \pm 0.12$と AU 鈍化域の整合；3シード）で機能を確認した．
同時に，適用境界を定量化した：
内在次元推定はボトルネック余裕$m_{\mathrm{ref}} \gg \hatdID$でのみ安定であり
（等長性改善は副次的；4モデル比較），
自然画像では$m_{\mathrm{ref}} \approx 2\hatdID$を超えると膨張する；
Conv-VAE では$\beta$不足が AU $= m$という信号として検出され，
$\beta$増強で遮断が回復する．
今後の主要課題は，自然画像での AU 検証の安定同定，
変化点検出の nested validation，および原理の異なる ID 推定量による Step 1 の相互検証である．
...
両者の差は aggregated posterior $q_\phi(z)$と事前分布$p(z)$の不一致
...
\section{単一シード・探索的実験}\label{app:exploratory}

本付録の実験はすべて\textbf{単一シード（42）の探索的観察}であり，
本文の主張の直接の根拠には用いていない．

\subsection{Conv-VAE の高$\beta$誘発実験（Exp-ConvV-HiBeta）}

MNIST Conv-VAE（KL サイクリックアニーリング，150 エポック，
$m \in \{16, 32, 64, 96, 128, 192, 256\}$）で$\beta_{\max}$を増強した結果を
表\ref{tab:hibeta}に示す．
$\beta_{\max} = 10$で活性率が$m = 256$において 36\%まで低下し，
変化点検出（$\tau$法）は$\approx 128$を返す；
$\beta_{\max} = 20$ではさらに低次元側（$\approx 96$）へ移動する．
$\beta$増大で遮断が低次元側へ移動するというサロゲート条件の定性的予測と整合する．

\begin{table}[H]
\centering
\caption{Exp-ConvV-HiBeta (single seed 42, exploratory): AU of the MNIST
Conv-VAE under strengthened $\beta_{\max}$ (activation rate in parentheses).}
\label{tab:hibeta}
\small
\begin{tabular}{rccc}
\toprule
...
多シード再現は今後の課題である（Step 1 の多シード値は Exp-Real-Geom；
