...
%% AE/VAEにおける有効潜在次元の診断的解釈：多指標分析フレームワーク（版23）
%% コンパイル: platex main_paper_NN23.tex && dvipdfmx main_paper_NN23.dvi
...
%% ===================================================================
\begin{abstract}
オートエンコーダ（AE）・変分オートエンコーダ（VAE）のボトルネック次元$m$は，
学習された潜在表現が利用する有効次元数を規定する重要な設計パラメータであるが，
AE/VAE が実際に何次元の有効表現を学習しているかを定量的に診断する体系的な方法論は
十分に確立されていない．

本論文は，この問いに対して，
Active Unit（AU）Dynamics・内在次元（ID）推定（TwoNN/MLE）・MSE エルボー・PCA を組み合わせた
\textbf{多指標診断フレームワーク}を提案する．
本フレームワークは有効潜在次元を一意に決定することを目指すのではなく，
各指標を\textbf{頑健指標}（AU 値），\textbf{数値安定な補助的参照推定量}（TwoNN ID；等長性バイアスあり），
\textbf{探索的指標}（AU 鈍化点$\mknee$；グリッド・シード依存）に明示的に分類し，
単一指標への過信を避けながら診断的に活用する解釈的基盤を提供する．

理論面では，線形$\beta$-VAE において AU 活性化しきい値$\lambda_j > \beta\tau^2$（第$j$固有値$\lambda_j$，
デコーダノイズ分散$\tau^2$，正則化係数$\beta$）を導出し（定理1），
$\mknee$のグリッド感度の形式的上界を示す（命題2）．

実験面では，MNIST・Fashion-MNIST における AU 値（3シード間$\sigma \leq 0.5$）と
TwoNN ID（$m=64$: $10.03 \pm 0.12$）の高い再現性を確認し，
密グリッド実験により$\mknee = 10.3 \pm 0.9$と安定化可能であることを示す．
Conv-VAE・自然画像設定では，AU/$\mknee$の stable identification には至らないが，
stable identification の5項目判定基準を定義し，
フレームワークの適用可能範囲（validity range）と境界条件を定量化する．
\end{abstract}
...
\begin{enumerate}
  \item \textbf{有効次元の多指標診断フレームワーク}（主要貢献）：
        ...
        自然画像・Conv-VAE 設定（Exp-NatImg-AU，Exp-NatImg-Sweep）では，
        AU/$\mknee$ の stable identification には至らないが，
        stable identification の5項目基準を定義し，
        フレームワークの適用可能範囲（validity range）と境界条件を定量化する
        （§\ref{sec:disc_convvae}，§\ref{sec:disc_limitation}）．
  \item \textbf{線形$\beta$-VAE の AU 活性化しきい値の導出}（理論的貢献）：
        データ固有値$\lambda_j$，デコーダノイズ$\tau^2$，正則化$\beta$に対して，
        線形ガウス設定において第$j$潜在次元が活性となる条件
        $\lambda_j > \beta\tau^2$を水充填定理と KKT 条件から導出し（定理\ref{thm:linear_vae}，§\ref{sec:theory}；詳細証明は付録），
        $d^*(\beta) = |\{j:\lambda_j > \beta\tau^2\}|$という AU 数の閉形式上界を与える．
        Lucas et al.~\cite{lucas2019elbo}の定性的議論と比較して，
        $\beta \neq 1$の正則化への拡張としきい値の定量的導出が新規性である
        （Tipping \& Bishop~\cite{tippingbishop1999ppca}の確率的 PCA における
        $\beta=1$の特殊ケースを包含する）．
        $\mknee$のグリッド感度の形式的上界（命題\ref{thm:mknee_instab}；理想化単調飽和モデル下）と
        アンサンブル安定化戦略（§\ref{sec:disc_mknee_improve}）も提示する．
  \item \textbf{実データ幾何比較実験と TwoNN 適用条件の実証}：
        ...
        実データで実証し，命題\ref{thm:twonn_stability}（§\ref{sec:theory_stability}；
        不偏性の保証ではなく条件付き安定性の十分条件のスケッチ）として理論化した．
  \item \textbf{AU 鈍化点$\mknee$の特性解析と安定化戦略}（補助的貢献）：
        MNIST 疎グリッド多シード実験で$\mknee \in \{32,64\}$（$\sigma=15$）となり，
        密グリッド実験（Exp-Mnist-Dense）で$\mknee = 10.3 \pm 0.9$（3シード）と大幅に改善．
        グリッド密度が不安定性の主要因の一つであることを3要因分離実験で定量化した（§\ref{sec:exp_el}）．
\end{enumerate}
...
\begin{description}
  ...
  \item[RQ3] 幾何的正則化（IsometricAE）やボトルネック余裕（$m / \hat{d}_\mathrm{ID}$ の大小）は，
             TwoNN ID 推定の安定性にどのような影響を与えるか．
             （本研究の結論：$m \gg \hat{d}_\mathrm{ID}$ という十分なボトルネック余裕が，等長性改善より TwoNN 安定性への支配的因子であることを，
             合成多様体・実データで実証した；§\ref{sec:exp_ep}，§\ref{sec:exp_iso_id}，§\ref{sec:exp_real_geom}．）
\end{description}
...
自然画像実験（Exp-Nat-Cifar・Exp-Nat-SVHN）では，TwoNN 補助推定の外部妥当性を
Pope ら\cite{pope2021intrinsic}との照合で確認し，
...
$\dzopt$（$= m^{\mathrm{cand}}$） & 診断候補次元（一意な「最適値」ではない） & §\ref{sec:framework} \\
$\dzsat$（$= m^{\mathrm{sat}}$） & AU 急激な飽和点（合成多様体型の診断候補） & §\ref{sec:framework} \\
$\mknee$（$= m^{\mathrm{knee}}$） & AU 増加率鈍化点（実データ型の粗い目安） & 式\eqref{eq:mknee} \\
...
$\mknee$（AU 鈍化点） & \textbf{探索的指標} & $\sigma=15$（疎グリッド）; $\sigma=0.9$（密グリッド） & 有効次元の粗い目安 \\
\bottomrule
\end{tabular}
\medskip
\footnotesize
...
\end{tabular}
\caption{有効潜在次元の診断パイプライン．
診断候補次元$\dzopt$はボトルネック次元の一意な最適解ではなく，
多指標（AU 値・TwoNN ID・$\mknee$）を総合した診断上の候補である．}
\label{fig:framework}
\end{figure}
...
命題\ref{thm:lower_bound}はブラウワーの領域不変性定理の直接適用として厳密に成立する位相的下界であり，
厳密な主張は「ゼロ再構成誤差を達成するために必要な次元」に限定される．
\textbf{定理\ref{thm:linear_vae}}は線形ガウス設定での厳密な結果であり，
AU 活性化しきい値を閉形式で与える（証明詳細は付録\ref{app:proofs}）．
...
ブラウワーの領域不変性定理\cite{hatcher2002algebraic}より，
...
\begin{theorem}[線形$\beta$-VAE における Active Unit 活性化しきい値]\label{thm:linear_vae}
線形エンコーダ・デコーダ・ガウス事前分布を持つ$\beta$-VAE（詳細設定は付録\ref{app:proofs}）の
$\beta$-ELBO 大域的最適解において，水充填定理と KKT 条件から次が成立する：
...
\noindent
詳細な証明は付録\ref{app:proofs}に示す．本文ではしきい値$\lambda_j > \beta\tau^2$の
直感的解釈と実験との対応のみを述べる：
$\beta\tau^2$が水充填の「水面」として機能し，固有値がこれを超える次元のみが活性化される．
\end{theorem}

\begin{proof}[証明スケッチ（詳細は付録\ref{app:proofs}）]
...
\begin{proposition}[$\mknee$のグリッド感度上界（理想化単調飽和モデル）]\label{thm:mknee_instab}
...
位置づけの凡例：\textbf{主実験}（フレームワーク中核の実証），補助（前提条件の検証），\textit{限界検証}（適用範囲の定量化）．}
\label{tab:exp_list}
\small
\begin{tabular}{p{3.5cm}p{4.5cm}p{3.5cm}p{2.0cm}p{1.5cm}}
\toprule
実験ラベル & 名称・目的 & データ/設定 & 位置づけ & RQ \\
\midrule
Exp-Syn-1 & MSE 肘点・ID 確認 & 合成多様体 & \textbf{主実験} & RQ1, RQ2 \\
Exp-Syn-AU & AU 飽和（AE vs VAE） & 合成多様体 & \textbf{主実験} & RQ1,2 \\
Exp-Syn-Top & 位相と AU（付録 B） & Torus/Möbius 等 & 補助 & --- \\
Exp-Syn-Iso & IsometricAE 等長性 & 合成多様体 & 補助 & RQ3 \\
Exp-Stab & TwoNN 数値安定性 & Swiss Roll + MNIST & 補助 & RQ2 \\
Exp-Geom & AE $m$スイープ $\kappa$・TwoNN & MNIST AE & 補助 & RQ2,3 \\
Exp-Iso-ID & 等長性改善の TwoNN 影響 & Swiss Roll & 補助 & RQ2,3 \\
Exp-Mnist-Base & MNIST VAE $m$スイープ & MNIST FC-VAE, $m=1$〜256 & \textbf{主実験} & RQ1,2 \\
Exp-Mnist-Multi & MNIST 多シード検証 & 3 シード, 150/300ep & \textbf{主実験} & RQ1,2 \\
Exp-Mnist-PCA & 理論-実験橋渡し & MNIST PCA & 補助 & RQ1 \\
Exp-Mnist-Dense & 密グリッド3因子分離 & MNIST FC-VAE & \textbf{主実験} & RQ1 \\
Exp-Fashion & Fashion-MNIST 検証 & Fashion-MNIST FC-VAE & \textbf{主実験} & RQ1,2 \\
Exp-Conv-AE & Conv-AE ID 整合性 & MNIST Conv-AE & 補助 & RQ2 \\
Exp-Conv-Fix & 固定$\beta$ Conv-VAE & MNIST Conv-VAE & \textit{限界検証} & RQ1 \\
Exp-Conv-Ann & KL アニーリング & MNIST Conv-VAE & \textit{限界検証} & RQ1 \\
Exp-Conv-M256 & Conv-VAE 拡張$m$ & MNIST, $m\leq256$ & \textit{限界検証} & RQ1 \\
Exp-Conv-M512 & Conv-VAE $m\leq512$ & MNIST, 3シード & \textit{限界検証} & RQ1 \\
Exp-ConvV-HiBeta & 高$\beta$ AU 飽和誘発 & MNIST Conv-VAE & \textit{限界検証} & RQ1 \\
Exp-Nat-Cifar & CIFAR-10 TwoNN 外部検証 & CIFAR-10 Conv-VAE & \textbf{主実験} & RQ2 \\
Exp-Nat-SVHN & SVHN 部分崩壊 & SVHN Conv-VAE & \textit{限界検証} & RQ1 \\
Exp-NatImg-AU & CIFAR-10 AU 飽和条件探索 & CIFAR-10, $\beta_{\max}\leq40$ & \textit{限界検証} & RQ1 \\
Exp-Real-Geom & 実データ幾何比較 & MNIST+FMnist, 4モデル & \textbf{主実験} & RQ2,3 \\
Exp-NatImg-Sweep & 自然画像拡張スイープ & CIFAR-10, $m\leq1024$ & \textit{限界検証} & RQ1 \\
Exp-NatImg-Knee & $\mknee$検出法比較 & CIFAR-10 & \textit{限界検証} & RQ1 \\
Exp-Mnist-Dyn & 学習ダイナミクス（付録 B） & MNIST, 単一シード & 補助 & --- \\
Exp-Mnist-Ent & Manifold Entanglement（付録 B） & MNIST & 補助 & --- \\
Exp-Tau-Cross & $\tau_\mathrm{knee}$感度分析 & 複数DS & 補助 & RQ1 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{$\AUsat = 36$の解釈（実験 Exp-Mnist-Ent，予備検証；付録 B）}
MNIST VAE の$\AUsat = 36$（$m=256$）は$\hat{d}_\mathrm{ID} \approx 10$〜12 を大幅に超えており，
$\mknee = 12$とは区別する（§\ref{sec:disc_incidental}）．
Manifold Entanglement 仮説の検証（Exp-Mnist-Ent）は詳細を付録 B に委ねる：
クラス数$K \in \{1,2,5,10\}$制限実験（75ep）で TwoNN ID が$K=10$の 8.89 まで増加するが，
$\AUsat$は未収束であり結論は今後の課題である．

\paragraph{学習ダイナミクス（実験 Exp-Mnist-Dyn，探索的；付録 B）}
MNIST（$m=16$，200ep）での大域・局所構造の時間的分離（2フェーズパターン）は
単一シードの探索的観察であり，詳細を付録 B に委ねる
（Trustworthiness の急速形成後に TwoNN ID が漸進的増加；多シード検証は今後の課題）．
...
MNIST と同一のアーキテクチャ・設定を適用し，フレームワークの異種実データへの適用性を確認する．
...
$\beta_{\max}=80$・薄型デコーダ条件で$m=512$付近に AU plateau-like behavior の初観測（AU$=24.1\pm2.8$，以降ほぼ一定傾向）．
...
\textbf{（1）高$\beta$・薄型デコーダ条件での AU plateau-like behavior の観測}：
$\beta_{\max}=80$・薄型デコーダ条件で，$m \in [512, 1024]$にわたって
AU$\approx 24$付近での AU plateau-like behavior が観測された
（表\ref{tab:natimg_sweep}，太字行）．
後述の stable identification 5条件（§\ref{sec:natimg_stable_id}）は全て未達であり，
この観測は conditional/preliminary な結果として扱う．
...
という条件付きの実験的証拠を得た（mechanistic validation の前進）．
...
フレームワークが読み取る診断候補次元$\dzopt$（$= m^{\mathrm{cand}}$）は「最適値」ではなく，
AU 値・TwoNN ID・$\mknee$を総合した多指標診断上の候補であることを強調する
（$\mknee$は探索的指標であり，AU 値・TwoNN ID が主要な読み取り指標である）．
フレームワークの安定動作は FC-VAE と MNIST/Fashion-MNIST の範囲で最もよく実証されており，
Conv-VAE や自然画像に対しては，限界検証実験によりフレームワークの
validity range（適用可能範囲）と applicability boundary（適用限界条件）を定量化した
（表\ref{tab:applicability_tier}）．
本論文の主要貢献として，
（i）実データ幾何比較実験（Exp-Real-Geom）と TwoNN の条件付き安定性スケッチ（命題\ref{thm:twonn_stability}）により，
「$m \gg \hat{d}_\mathrm{ID}$が TwoNN 安定性の支配的条件」を理論・実験の両面から示した；
（ii）自然画像拡張スイープ（Exp-NatImg-Sweep）と stable identification 判定基準（§\ref{sec:natimg_stable_id}）により，
AU plateau-like behavior の条件付き観測（$\beta_{\max}=80$，薄型デコーダ，CIFAR-10）を達成し，
自然画像での stable identification への境界条件を定量化した（strict な stable identification は今後の目標）．
理論面では，線形$\beta$-VAE における AU 活性化しきい値$\lambda_j > \beta\tau^2$を
水充填定理・KKT 条件から導出し（定理\ref{thm:linear_vae}）；
$\mknee$のグリッド感度の上界（命題\ref{thm:mknee_instab}；理想化モデル下）と
安定化戦略（式\eqref{eq:mknee_ens}，§\ref{sec:disc_mknee_improve}）を提示した．
...
\begin{table}[H]
\centering
\caption{フレームワーク適用可能性（validity range）の3段階整理．
「A」=安定動作確認，「B」=部分的確認・限界条件あり，「C」=stable identification 未達．}
\label{tab:applicability_tier}
\small
\begin{tabular}{p{3.5cm}p{2.6cm}p{2.4cm}p{2.4cm}p{3.5cm}}
\toprule
設定 & TwoNN ID & AU 監視 & $\mknee$同定 & 実証根拠 \\
\midrule
\textbf{FC-VAE +} \textbf{MNIST/F-MNIST}
  & A（$\sigma\leq0.12$）
  & A（$\sigma\leq0.5$）
  & B（密グリッドで$10.3\pm0.9$）
  & Exp-Mnist-Base, Exp-Mnist-Dense, Exp-Fashion \\
\addlinespace
\textbf{Conv-VAE + MNIST}
  & A（安定）
  & B（$m\geq96$で AU$<m$）
  & B（$\beta\geq10$で同定可，$\beta=4$では未到達）
  & Exp-Conv-Fix, Exp-Conv-Ann, Exp-Conv-M256/M512, Exp-ConvV-HiBeta \\
\addlinespace
\textbf{自然画像}
\textbf{（CIFAR-10/SVHN）}
  & A（Pope値と整合）
  & B（$\beta_{\max}=80$・薄型でplateau-like挙動）
  & C（5条件全未達；stable ID未達）
  & Exp-Nat-Cifar, Exp-NatImg-Sweep, Exp-NatImg-Knee \\
\midrule
\multicolumn{5}{p{14.5cm}}{\small
Stage 1 の$\hat{d}_\mathrm{ID}$は等長性バイアスを含む補助的参照推定量（$\kap\approx10^9$；§\ref{sec:exp_ep}で数値安定性を実証）．
$\mknee$はいずれの設定でも探索的指標であり，一意な最適ボトルネック次元ではない．}\\
\bottomrule
\end{tabular}
\end{table}

この3段階整理は，本フレームワークの validity range（適用可能範囲）と
applicability boundary（適用限界）を定量的に記述する枠組みである．
本論文は「有効潜在次元を一意に決定する方法の提案」ではなく，
各指標の頑健性・限界・適用条件を定量化した診断フレームワークの確立を目指す．
...
自然画像での AU/$\mknee$ stable identification は未達であり，
Exp-NatImg-AU・Exp-NatImg-Sweep により必要$\beta$条件，デコーダ容量条件，
stable identification 5基準の現達成状況を定量化した——これは診断フレームワークの
validity range の境界条件を示す知見として位置づける
（§\ref{sec:disc_convvae}，§\ref{sec:disc_limitation}）．
