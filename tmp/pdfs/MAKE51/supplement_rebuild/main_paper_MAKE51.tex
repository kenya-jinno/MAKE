% MDPI MAKE51: revised against MAKE50_revision_review.md
...
The contribution is a retrospective comparison of search policies and
their accuracy--cost trade-off, together with a record-level audit.
We contrast a historical fixed-window policy with an ID-initialised
local search, an otherwise identical search without ID, ascending search
and a full-grid reference. Four image datasets, five candidate seeds and
five regularisation weights are covered. We report the full factorial
sensitivity to the ID coefficient and reliability thresholds, and separate
the cost of acquiring an ID bank from its reuse.

The inherited pruning implementations are treated as implementation audits,
outside the main policy comparison. A normalized relevance readout and a
new, bounded native-model diagnostic test specific implementation issues;
neither is presented as a reproduction of an original pruning method.
A separate MNIST retraining experiment evaluates Monte Carlo ELBO capacity
...
\begin{Algorithm}[ID-local replay and its matched controls]
...
\item For ID-local, charge the reference bank and compute Equation~\ref{eq:checks}.
If either threshold fails, request ascending widths and return the first pass.
\item Otherwise start at the smallest grid width at least $c\hat d$,
clipped to the largest width. For the midpoint control, omit all reference
costs and instead start at zero-based index $\lfloor(|\mathcal M|-1)/2\rfloor$.
...
Full-grid-$Q$, ascending-$Q$, the legacy path, ID-local, midpoint-local
and certified ID-local are replayed on all 100 dataset--weight--seed
conditions. Paths are completed within the stored grid (at most 12
...
requests and canonical cost. Midpoint-local controls the local stopping
rule while removing ID acquisition; it is a simple control fixed for
the replay, not a tuned or universally optimal starting point.
...
validated cross-dataset pruning baseline. All new checkpoints are saved.
...
a dataset-level advantage of any criterion.
...
Search design changes what a cost comparison can identify. Exhausting
a lower ID window includes work already sufficient for ascending search;
its higher expense is largely determined by construction. The new replay
instead exposes the trade-off between fewer candidate requests and
certifying the smallest qualifying width. A no-ID local control can also
save requests. The results therefore do not attribute every reduction
from full-grid search to ID information.

ID-local matches the grid minimum at the inherited default throughout the
recorded ladder, but relies on fallback in three datasets. The factorial
analysis shows both decision changes at reliability thresholds and
nonminimal outputs at other settings. Shared reference banks, one split,
retrospective policy design and a small dataset collection limit
generalization. Acquisition and reuse costs answer different deployment
questions and cannot be collapsed into one universal ranking.
...
The pruning evidence remains deliberately outside the main comparison.
Normalizing the stored ARD perturbations corrects one identifiable
readout error, but does not validate the prior-update procedure or
recover an infinitesimal Jacobian. The native HC trajectory supplies
one new implementation diagnostic, not broad source-method validation.
The new MC ELBO comparison likewise covers one MNIST seed and
proxy-selected checkpoints; it does not complete a four-dataset,
five-seed ELBO study. Exact ARM/source-ARD reproduction, an independent
reference-bank replication, missing natural-image training curves and
complete end-to-end timing remain open. The contribution is thus a
reproducible comparison of specified search policies and diagnostic
assumptions, not a definitive ranking of established pruning methods.
...
We separated latent-width search decisions from diagnostic readouts and
implementation history. The historical fixed-window expense contrast
does not by itself test the value of ID information. Retrospective
local-search and matched-control replays quantify candidate savings,
reference acquisition costs and gaps from the full-grid minimum.
Reliability thresholds alter which datasets use ID, and some local
policies miss qualifying smaller widths. The paired AU analysis does
not establish an AU-specific benefit on its near-ceiling prediction task.
A bounded new ELBO experiment and corrected relevance audit improve
measurement validity while leaving source-method validation incomplete.
Claims about efficient capacity selection therefore depend on explicit
search rules, quality targets, cost units and implementation evidence.
...
\texttt{MAKE51\_reproducibility.zip} contains the configuration, original
result records, replay tables, analysis code, dependencies, split indices,
new MNIST checkpoints and a SHA-256 manifest. Extract the archive and
follow its \texttt{README.md}; Appendix~\ref{app:inventory} maps its contents.
Historical MAKE49 checkpoints are unavailable. The archive is supplied
with this revision; it is not described as a publicly deposited dataset
and has no archive DOI.}
...
audit, reaggregation, additional model validation, figure preparation
...
\path{src/make51/summarize_validation.py}. The manuscript assembly entry point is
\path{src/make51/build_manuscript.py}. The generated main TeX embeds its
...
The accompanying \texttt{MAKE51\_reproducibility.zip} supplies the files,
...
New model records & \path{results/make51/model_validation/} \\
Snapshot identities & \path{results/make51/source_manifest.json} \\
Figures & \path{results/figures/MAKE51/} \\
\bottomrule\end{tabularx}
\end{table}
