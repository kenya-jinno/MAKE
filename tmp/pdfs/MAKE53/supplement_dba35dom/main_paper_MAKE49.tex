...
%=================================================================
\abstract{%
Several criteria promise a cheaper choice of the latent dimension of a
variational autoencoder than grid search: placing a search window from an
intrinsic-dimension estimate, counting the latent variables that stay
active after training, or pruning capacity during training under a
reconstruction constraint or a relevance prior. Each has been evaluated on
the datasets its authors used, but not against the others under a shared
protocol. We provide that comparison, using a common quality target, a
common accounting of cost, and a protocol fixed before the experiments,
across four datasets, five seeds and five regularisation settings.
We introduce threshold sensitivity, meaning how far the selected dimension
moves when a criterion's free parameter is varied, as a measurable
property. It orders the criteria reproducibly across datasets and spans
two orders of magnitude, from one method that is insensitive to it to
another that moves by over a hundred dimensions on the same data.
Within this comparison, intrinsic-dimension guidance did not improve on an
ascending scan under the same target and declined to apply on two of three
image datasets, and the activity-based readout was neither more stable
than the readout it refines nor distinguishable from matched random
features in a pre-registered prediction task. We also document three
implementation failure modes that leave training intact and results
plausible.}
...
threshold sensitivity; benchmarking; reproducibility}
...
The latent dimension of a variational autoencoder~\cite{kingma2014vae} has
to be set before training, and it is usually set by grid search: train a
model at each candidate capacity and keep the one that performs best. This
is reliable and expensive, and several cheaper criteria have been proposed
in its place. Some estimate the intrinsic dimension of the
data~\cite{facco2017twonn,levina2005mle} and use it to place a search
window~\cite{bonheme2022fondue}. Others train a single large model briefly
and read off how many latent coordinates remain in use, either from the
variance of the posterior means~\cite{burda2016iwae} or from a
classification of the latent variables into
types~\cite{bonheme2023active}. Others again prune during training, under
an explicit constraint on reconstruction quality~\cite{deboom2020geco} or
under a prior that suppresses irrelevant axes~\cite{saha2025ardvae}.
...
the cases where a method declines to answer. Without that, a reported
saving cannot be separated from the conditions in which it was measured.

This paper supplies that comparison, and the contribution is of two kinds.

The first is the comparison itself. We fixed the protocol in advance --- the
quality target, the budget, the termination states, the decision rules and
the number of seeds --- and recorded every subsequent change with its
reason. We then evaluated seven published or standard criteria, together
with grid, coarse and fixed-dimension baselines, across four datasets,
five seeds and five settings of the regularisation weight. To our
knowledge no previous study places these particular criteria on a common
quality scale with a common cost model.

The second is a property of such criteria that the comparison makes
visible. Almost every rule we examined carries a free parameter, and the
dimension it selects moves when that parameter is varied. We call the size
of that movement the \emph{threshold sensitivity} of the criterion, and we
find that it orders the criteria reproducibly across datasets, spanning
two orders of magnitude. One method sits at zero because it introduces no
such parameter; plain grid search under a reconstruction criterion, the
least elaborate rule in the comparison, does not. The useful question
about a selection rule is therefore not whether it has a free parameter
but how strongly its answer depends on one, and that question has a
reproducible answer.

Within this comparison, two criteria that have attracted interest did not
support the decision. Guidance by an intrinsic-dimension estimate did not
improve on an ascending scan under the same quality target, and its
reliability checks declined to apply on two of the three image datasets.
The activity-based readout was not more stable than the variable-type
readout it was meant to refine, and in a prediction task fixed in advance
it was not distinguishable from a matched set of random features. The
quality target that did transfer across methods is not ours: it coincides
with the reconstruction constraint of an existing pruning method, and we
report the correspondence rather than claim the construction.

We also document three implementation failure modes encountered in this
work, all of the same kind and all invisible in aggregate results.
Section~\ref{sec:results} presents the measurements and
Section~\ref{sec:discussion} discusses what would change the conclusions.
...
use as a comparison method~\cite{bonheme2022fondue} already proposes choosing the latent dimension
...
Second, the pruning method we added during revision~\cite{deboom2020geco}
selects the smallest capacity that satisfies an explicit upper bound on reconstruction error.
...
problem. Principal component analysis~\cite{tippingbishop1999ppca} gives a linear
answer; Isomap~\cite{tenenbaum2000isomap} and locally linear
embedding~\cite{roweis2000lle} give nonlinear ones by preserving geodesic or
...
task. We use the two-nearest-neighbour estimator~\cite{facco2017twonn} as the
primary estimator and a maximum-likelihood estimator~\cite{levina2005mle}
as a cross-check; other families
exist~\cite{ceruti2014danco,johnsson2015ess,camastra2016intrinsic,allegra2020data},
including graph-based estimators built from geodesic entropic
graphs~\cite{costa2004geodesic}. We did not
...
would not resolve it. Deep representations have been reported to have lower intrinsic dimension
than their inputs~\cite{ansuini2019intrinsic,pope2021intrinsic}, which is part of the
...
training. One approach~\cite{deboom2020geco} places a binary gate on each latent
coordinate, penalises the number of open gates~\cite{louizos2018l0}, and holds reconstruction error below a
stated bound by a Lagrange multiplier updated from the constraint
violation~\cite{rezende2018geco}; the number of gates left open is the answer. Because the bound
...
our common quality target. Another approach~\cite{saha2025ardvae} replaces the fixed prior with
...
standard diagnostic for posterior collapse~\cite{burda2016iwae,lucas2019elbo}, and the classification of latent variables into active, mixed and passive
types~\cite{bonheme2023active} is a refinement of it. Both are read from a trained model rather than used to choose its
...
A conference paper by the same authors~\cite{obata2025manifold} reported that the intrinsic
...
manifold's embedding dimension, which is bounded above by $2d$ for smooth manifolds~\cite{hatcher2002algebraic}. This gives a geometric reason to look near twice an estimate of
...
threshold set by the regularisation weight~\cite{cover2006elements};
related behaviour has been reported for trained
models~\cite{rolinek2019pca}. This gives a reason to expect
...
averaged over the batch~\cite{higgins2017betavae}. This matters: an earlier version of this work
...
The likelihood follows the data type: Bernoulli for the binary image dataset~\cite{matthey2017dsprites},
Gaussian elsewhere. This is stated in every table and figure,
...
The datasets are MNIST~\cite{lecun1998},
Fashion-MNIST~\cite{xiao2017fashion}, dSprites~\cite{matthey2017dsprites}
and CIFAR-10~\cite{krizhevsky2009learning}.
