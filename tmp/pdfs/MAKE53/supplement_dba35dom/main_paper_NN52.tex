...
各データセットで3シードの追加実験を行い，ARMによる剪定実装と厳密ヤコビアンによる関連度読み取りを評価する。
nativeモデルの品質，次元転用，学習制約の充足を別々に報告する。
構成要素の検証は実装監査を改善するが，原著の剪定法全体の再現を確立するものではない。
別途，MNIST実験でMonte Carlo ELBOによる容量選択を評価する。
...
\subsection{データセット横断の追加構成要素検証}\label{sec:gpuvalidation}
追加実験は同じ4データセットと固定分割，シード42，123，777，
最大候補次元（64，CIFAR-10のみ256），300 epochsを用いる。
事前分布の中心化2設定を含め，ARMは12実行，ARDは24実行である。
これらは過去の5シード比較およびMNISTの1シード容量選択実験とは別である。
3シードの集計には標本SDを用いる。
シードの反復と分割の共有は，独立したデータセット反復を構成しない。
...
最終モデルにおいて，検証データの先頭100例の事後平均で，
自動微分によりデコーダのヤコビアンを評価する。
事前分散を$v_j$，デコーダの出力座標を$\ell$とすると，スコアは次式である。
\begin{equation}
 ...
 \label{eq:exactard}
\end{equation}
読み取り値は，スコアを降順に加算し，総和の99\%を説明するのに必要な個数とする。
ここで厳密とは有限摂動の代わりに自動微分を使うことを指し，
標本や学習に伴う不確実性を除くものではない。
記録されたnative ARDのMSEは未剪定の全次元モデルを評価したもので，
関連度から選んだ個数における品質ではない。
両手法とも，次元転用では個数を最近傍の候補次元へ写し，
同じシードの保存済み通常VAE（$\beta=1$）を評価する。

新しい全実行で，5 epochsごとに評価モードのMSE曲線を保存する。
ARMは学習データの2000例の部分集合を用いる。
ARDの学習用プールの部分集合には事前分布更新用の例も含まれるため，その旨を明記する。
記録された学習時間には，これらの定期評価が含まれる。
ARMは最終native評価時間も記録する。
ARDはヤコビアン時間を別に記録するが，最終native評価時間は個別に記録しない。
したがって，これらは部分的な時間計測である。
新しいスクリプトはモデルのcheckpointを保存しない。

評価コードの監査により，追加ELBOフィールドの利用範囲を限定する。
32標本のオプションで変わるのはMSEだけであり，
再構成尤度は依然として事後平均で評価される。
一方，KLはARDを含め，マスクを適用しない標準正規事前分布を用いる。
これらのフィールドは学習したnativeモデルのMonte Carlo ELBOではない。
したがって追加記録はMSE，ゲート，関連度の個数，曲線に用いる。
ELBOによる容量選択の証拠は，\ref{sec:newvalidation}節の別のMNIST実験に留まる。
...
\subsection{追加のnative剪定・関連度結果}\label{sec:newpruning}
表\ref{tab:newcounts}は，過去と追加実験の未変換の読み取り値を比較する。
一部のデータセットでは近い値となるが，全4データセットで同一ではない。
ARMのdSprites平均は54.0であり，過去のHCの個数64とは異なる。
個数だけでは小容量化の成功を測れない。
\begin{table}[H]
...
厳密ヤコビアンを用いたARDのゼロ中心の読み取り値は，
MNIST，Fashion-MNIST，dSprites，CIFAR-10でそれぞれ6.0，4.0，9.0，26.7となる
（表\ref{tab:ardnew}）。
標本平均で中心化する変種では6.0，4.0，9.7，26.3となる。
この3シードにおける小さな差が示すのは限定的な中心化感度であり，
事前分布の指定に対する不変性ではない。
両設定とも，3種類の自然画像データセットでは全次元モデルと転用先の双方で$Q$を満たし，
dSpritesでは双方で満たさない。
次元を削減したnative ARDモデルは評価していない。
\begin{table}[H]
...
過去の未正規化CIFAR-10 ARDの個数は14.2だったが，
前回の正規化監査ですでに26.0へ変更されている（表\ref{tab:ard_normalized}）。
新しい個数はその修正後の読み取り値に近い。
事前分布の更新と学習実行も変更されているため，
14.2からの差を厳密ヤコビアンだけに帰属することはできない。
付録\ref{app:gpu}に，これらの追加実行について全データセットの曲線，
ゲート分布，分離したtest MSE，時間の内訳を示す。
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/id.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/beta.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/selected_curves.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/learning.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/readouts.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/delta.pdf}
...
追加剪定実験では全4データセット・3シードへ対象を広げ，
native MSE，ゲート，関連度スコア，曲線，部分的な時間計測を得た。
しかし，次元転用がnativeモデル自体の小容量化と同等になるわけではない。
dSpritesではこの区別が特に明瞭であり，
失敗したnative ARMモデルの個数を通常VAEへ転用すると，転用先は成功する。
同様に，ARDの全次元モデルの品質からnative剪定後の品質を保証することはできない。
厳密ヤコビアンによる読み取りは有限刻み幅の誤差を除くが，
学習と事前分布更新の選択は依然として実装の一部である。
KLのガウス近似はARD原著と共通する。
標本平均を中心とする分散更新は感度分析の変種であり，
原著のゼロ中心設定に対して必要な修正ではない。
...
品質と費用を明示的に定義し，
内在次元誘導によるVAEのボトルネック選択と活性ユニット診断を評価した。
信頼性判定，fallback，参照費用を固定すると，
ID由来の開始点と中点は全100条件で同じ次元数を選び，
IDが使われる条件ではID開始点の候補要求数が多かった。
判定を持たない中点との見かけの精度差はfallbackで説明される。
局所探索の最小性は，信頼性と係数の設定にも依存する。
追加のARMおよび厳密ヤコビアンARD実験では，
native品質，転用次元の品質，学習制約を切り分け，
実装と測定に残る限界を明らかにした。
AUは，今回の天井に近い課題で追加の予測価値を確立しない。
これらの結果が支持するのはID誘導と潜在診断の限定された評価であり，
一般的な効率や剪定の保証ではない。
...
同梱の査読用補足資料\texttt{MAKE52\_reproducibility.zip}には，
...
\end{table}
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/native_hc.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/new_mnist_curves.pdf}
...
新しい記録は，各データセットにつきARMおよびARDの各分散更新設定で3シードを含む。
表\ref{tab:armgates}のゲート区分と図\ref{fig:armconstraints}の個々の制約推移は，
閾値処理した次元数だけでは剪定の成功を診断できない理由を示す。
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/arm_constraints.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/arm_curves.pdf}
...
\begin{figure}[!htbp]
\includegraphics[width=\textwidth]{results/figures/NN52/ard_curves.pdf}
...
記録された\texttt{elbo}および\texttt{val\_elbo}フィールドは，
出所を保持するため未変更の生ファイルに残すが，
native ELBOの測定値としては用いない（\ref{sec:gpuvalidation}節）。
追加剪定実験のcheckpointは保存されていないため，
native ARDの剪定後再構成や，修正したnative尤度を，これらの集計ファイルだけから復元することはできない。
解析勾配によるARM検査は構成要素を検証するものであり，
そのようなモデル全体の評価に代わるものではない。

\section{再現性資料の一覧と記録上の残課題}\label{app:inventory}
再集計の入口は\path{src/make51/rebuild_review.py}であり，
保存JSONと設定ファイルを読み，過去の記録から数値表，6図，対応付き予測解析を出力する。
新しい探索解析は\path{src/make51/replay_search.py}，
範囲を限定したモデル検証は\path{src/make51/validate_models.py}である。
その出力は\path{src/make51/summarize_validation.py}で集計する。
追加実行の再集計は\path{src/make52/summarize_revision.py}，
原稿の組立ては\path{src/make52/build_manuscript.py}による。
生成した主TeXは表の内容を埋め込み，図PDFを参照する。
英語版はプロジェクト直下で\texttt{pdflatex}を参照が確定するまで繰り返してコンパイルする。
同梱の\texttt{MAKE52\_reproducibility.zip}は，一覧だけでなく実ファイルを収録する。
READMEは低費用の再集計と任意の再学習を区別し，
依存関係の版と利用できない過去の成果物を示す。
\texttt{MANIFEST.sha256}で収録ファイルを識別する。
アーカイブには固定したソースのスナップショットを含め，
非公開リポジトリへのアクセスなしで利用できる。
...
独立したMNISTモデル記録 & \path{results/make51/model_validation/} \\
追加開始次元対照 & \path{results/make52/start_width_ablation.json} \\
追加ARM・ARD記録 & \path{results/make52/geco_arm_all.json}; \path{results/make52/ard_native_all.json} \\
追加実行の集計 & \path{results/make52/manuscript_summary.json} \\
追加ソースの識別 & \path{results/make52/manuscript_source_manifest.json} \\
スナップショットの識別 & \path{results/make51/source_manifest.json} \\
図 & \path{results/figures/MAKE51/}; \path{results/figures/MAKE52/} \\
\bottomrule\end{tabularx}
\end{table}
...
追加の開始次元数と剪定の記録は，それぞれ独立した事後解析である。
再集計では，統合ファイルとデータセット別ファイル，3シードの網羅，
300 epochsまでの曲線，未変換個数の読み取り，アンカー目標，
転用先キャッシュの指標を照合する。
これは提供記録の整合性を検証するものであり，
利用できないcheckpointの再実行との同等性を検証するものではない。
