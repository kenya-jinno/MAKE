"""Build MAKE53 from its text template and verified saved-record tables."""
...
abstract = (ROOT / "src/make53/abstract.tex").read_text()
body = (ROOT / "src/make53/manuscript_body.tex").read_text()
body = re.sub(r"@@section:(\w+)@@",
              lambda m: (ROOT / "src/make53" / (m[1] + ".tex")).read_text(), body)
for prefix, directory in [("table", "make51"), ("table52", "make52"), ("table53", "make53")]:
...
assert "@@" not in body
# Use mathematical minus signs consistently in numerical table columns.
body = re.sub(r"(?<=& )(-\d+(?:\.\d+)?)(?= &)", lambda m: "$" + m[1] + "$", body)
...
bib += "\n\\end{thebibliography}\n\\end{document}\n"
text = pre + abstract + "\n\\begin{document}\n" + body + bib
(ROOT / "main_paper_MAKE53.tex").write_text(text)
print(f"MAKE53: {len(text.splitlines())} lines, {len(order)} references")
