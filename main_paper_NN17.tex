...
\begin{enumerate}
  \item \textbf{頑健指標 AU 値の実証的確認，および TwoNN ID の数値安定性確認}：
        ...
        TwoNN 推定の\textbf{外部妥当性}を確認した（§\ref{sec:exp_natural}）——自然画像での AU/mknee 側の完全検証については，
        十分な AU 飽和観測に必要な$\beta$条件・アーキテクチャ条件が判明した（Exp-NatImg-AU，§\ref{sec:exp_natimg_au}）．
...
\begin{enumerate}
  \item \textbf{有効次元の多指標分析フレームワーク}（主要貢献）：
        ...
        ただし，自然画像・Conv-VAE での AU/mknee 側の完全検証は今後の課題である．
...
\textbf{Exp-Iso-ID} & \textbf{等長性改善が TwoNN ID に与える影響} & \textbf{Swiss Roll, AE vs IsometricAE} & \textbf{幾何的正確性検証} & \textbf{RQ2, RQ3} \\
...
$m=4$（$> d_\mathrm{true}$）では TwoNN が$2.03$へ回復し正確な推定が得られる．}
\label{tab:ep_swiss}
\small
\begin{tabular}{lcccc}
\toprule
...
$m=4$（$> d_\mathrm{true}$）では TwoNN が$\approx2.03$へ回復する．}
\label{tab:ep_swiss}
\small
\begin{tabular}{lcccc}
\toprule
...
TwoNN ID の系統的バイアスは$\kap \in [1.1, 2.1]$の範囲では等長性改善によって有意には低減されないことが明らかになり，
$m \gg d_\mathrm{ID}$という運用条件の確保が TwoNN 利用における支配的な設計指針であることが
定量的に裏付けられた．
...
本実験の主眼は TwoNN ID 推定の外部妥当性確認に限定される（AU/mknee 側の完全検証は将来課題）．
...
線形理論（定理\ref{thm:au_sat}）では，第$j$次元が活性となる条件は$\lambda_j > \beta \tau^2$である．
...
幾何学的に unbiased であることを直接証明するものではない．
...
Conv-VAE については「$\beta_{\max}=4$ での未飽和は $\beta$ 設定不足でも説明可能であることを MNIST 上で確認した」という範囲に実証が留まる——自然画像での AU/mknee 側の完全検証は引き続き未解決であり，より大きな$m$グリッドや専用学習条件の設計が次のステップとなる
