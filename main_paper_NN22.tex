...
%% AE/VAEにおける有効潜在次元の診断的解釈：多指標分析フレームワーク（版22）
%% コンパイル: platex main_paper_NN22.tex && dvipdfmx main_paper_NN22.dvi
...
AU$\approx 24$（$\approx \hat{d}_\mathrm{ID}$）付近で AU plateau-like behavior が観測された
（表\ref{tab:natimg_sweep}，太字行）．
ただし，後述の stable identification 5条件（§\ref{sec:natimg_stable_id}）は全て未達であり，
この観測を「安定した飽和」と断言することは避ける（conditional/preliminary な観測として扱う）．
...
\paragraph{幾何的正則化（実験 Exp-Syn-Iso・Exp-Iso-ID；補助実験）}
IsometricAE（$\kap \approx 1.04$）は潜在空間の等長性（ID 推定の前提条件）を実現できることを実験 Exp-Syn-Iso で確認した．
ただし，実験 Exp-Iso-ID（§\ref{sec:exp_iso_id}）では$\kap \in [1.1, 2.1]$での等長性改善が
