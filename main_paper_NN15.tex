...
\begin{itemize}
  \item 位相的に非自明な合成多様体において，$\AUsat \approx \demb$の傾向を観測
        （条件付き；実験 Exp-Syn-Top，§\ref{sec:disc_topology}）．
...
（条件付き知見；§\ref{sec:disc_topology}参照）．
これらの対応は付随的観察であり，
...
$\mknee = 12$とは区別する（§\ref{sec:disc_topology}）．
...
2段階パターンが観測された（単一シード，§\ref{sec:disc_dynamics}参照）．
...
\end{itemize}
これらは，MNIST の$\hat{d}_\mathrm{ID} \approx 10$〜12 が参照 AE の学習アーティファクトではなく，
データ固有の低次元構造を反映している蓋然性を高めるものである．
ただし，真の$d_\mathrm{ID}$との乖離（系統的バイアス）をゼロと証明することは，
真値が未知の実データでは原理的に不可能であり，
本論文の立場は「補助的参照推定量として運用する」という慎重なものを維持する
（理論的精度向上には IsometricAE 等の幾何的 AE が必要；§\ref{sec:disc_isometric}参照）．
...
$\AUsat = \demb$（$= 3$）が本研究条件下で観測された（実験 Exp-Syn-Top）．
...
\begin{itemize}
  ...
  \item \textbf{実データへの非適用性}：MNIST の$\AUsat = 36$は$\hat{d}_\mathrm{ID} \approx 10$〜12 を
        ...
        （Manifold Entanglement と capacity leak の両解釈が可能；実験 Exp-Mnist-Ent は予備的）．
\end{itemize}
...
実験 Exp-Mnist-Dyn の知見（Epoch 5 での大域的構造の急速形成，その後の TwoNN ID の漸進的増加）は，
...
IsometricAE（$\kap \approx 1.04$，実験 Exp-Syn-Iso）は，
...
Conv-VAE や自然画像ではより大きな$m$グリッドや専用学習条件の設計が次のステップとなる
