...
\textbf{汎化の範囲に関する注記}：
本フレームワークの実証的検証は MNIST・Fashion-MNIST（単純な画像）に集中しており，
Conv-VAE の十分収束した検証および CIFAR-10 等の自然画像への適用はまだ限定的である．
本論文の貢献は，新たな方法論的視点の提案と$\mknee$の限界条件の明確化に位置づけており，
広く汎用的な次元選択基準の確立を主張するものではない．
\end{abstract}
...
MNIST の畳み込み型 Conv-AE での TwoNN ID の一貫性も検証し，
さらに Conv-VAE を 300 エポックに延長した多シード実験（実験 EH；§\ref{sec:exp_conv_300ep}）も実施した．
EH の結果は FC-VAE との比較において AU Dynamics の一貫性を検討するものであり，
「Conv-VAE での $\mknee$ 数値の安定した検出」を主張するものではない（§\ref{sec:exp_conv_300ep}）．
...
明示的に体系的な検証を行った研究は限られている——本研究ではその対応と限界を実験的に検討する．
...
この区別なしには$\mknee$を誤用する（不安定な数値を設計根拠として扱う）リスクがあり，
...
\begin{itemize}
  \item \textbf{Conv-AE・Conv-VAE 検証}（実験 EG・EH）：
        本稿では畳み込み型 Conv-AE（実験 EG）の TwoNN ID 一貫性（FC-AE と誤差$\approx 10\%$以内）を確認した．
        さらに Conv-VAE 300 エポック多シード実験（実験 EH；§\ref{sec:exp_conv_300ep}）も実施し，
        FC-VAE との AU Dynamics の比較を行った．
        ただし，Conv-VAE においても$\mknee$の数値的安定性は FC-VAE と同様の制約を受けることに注意が必要である．
        CIFAR-10 等の自然画像への一般化（Pope ら\cite{pope2021intrinsic}：$d_\mathrm{ID} \approx 26$）
...
\textbf{本研究の位置づけと汎化スコープの明示}：
本論文の貢献は，「単一指標依存から多指標フレームワークへ」という方法論的視点の提案と，
$\mknee$の不安定性条件（$m$グリッド・シード依存）の定量的解明に位置づける．
検証は MNIST・Fashion-MNIST の全結合型 AE/VAE を中心としており，
Conv-VAE の完全収束実験および CIFAR-10 等の自然画像への適用は限定的である（§\ref{sec:exp_conv}，§\ref{sec:disc_limitation}）．
これらの領域における本フレームワークの有効性は，今後の追加検証を必要とする未解決の課題である．

\textbf{今後の課題}：
十分収束した Conv-VAE での AU Dynamics 検証（より多いエポック数），
自然画像（CIFAR-10 等）への拡張（深い Conv-AE/VAE が必要），
