...
線形理論から非線形実験への定性的継承と定量的乖離を整理した（§\ref{sec:theory_bridge}・§\ref{sec:exp_em}，実験 EM）．

\textbf{主要な実験的知見}（合成多様体・MNIST・Fashion-MNIST・CIFAR-10・SVHN を対象とした実験群 E1〜EZ；詳細は表\ref{tab:exp_list}）：
\begin{enumerate}
  \item \textbf{頑健指標 AU 値の実証的確認，および TwoNN ID の数値安定性確認}：
        ...
        Conv-VAE の AU 飽和遅延については，拡張$m$グリッド実験（実験 EN；$m\in[4,256]$，3シード）と
        さらに$m\in\{384,512\}$まで延長した EN\_ext（§\ref{sec:exp_en}）で確認した：
        $m \leq 64$では AU$=m$が全次元活性として継続し，$m = 96$から AU$=90.7\pm1.2$となり
        飽和が始まるが，$m=512$まで拡張しても AU$\approx m$が維持され，
        明確な AU 飽和プラトーは観測されなかった（$\mknee$は全$m$範囲でグリッド端に張り付く）．
...
\begin{enumerate}
  \item \textbf{有効次元の多指標分析フレームワーク}（主要貢献）：
        ...
        CIFAR-10 実験（EI）では Pope ら\cite{pope2021intrinsic}の ID 報告値との整合により
  ...
  \item \textbf{AU 鈍化点$\mknee$の定式化・限界解明・安定化戦略}（補助的貢献）：
        ...
        \textbf{密グリッド実験（EL）ではグリッド密度が主要因の一つと示唆}：
...
$\mknee$の$\sigma$は疎グリッドの値；密グリッド（EL）では$\sigma=0.9$まで改善（グリッド設計が主要因の一つ；§\ref{sec:exp_el}）．
\normalsize
\end{table}
...
MNIST PCA スペクトル分析（実験 EM）で実証する（§\ref{sec:exp_em}）．
...
\midrule
E1 & MSE 肘点・ID 確認 & 合成多様体 & 原理確認 \\
EA & AU 飽和（AE vs VAE） & 合成多様体 & 原理確認 \\
EB & 位相的埋め込み次元と AU & Torus/Möbius 等 & 探索的 \\
EC & IsometricAE・等長性検証 & 合成多様体 & 副次 \\
EP & Stage 1 TwoNN 安定性の実証（新規） & Swiss Roll + MNIST (E4参照) & Stage 1 妥当性 \\
E4/E4-ext & MNIST VAE スイープ（主実験） & MNIST FC-VAE, $m=1$〜256 & 主要 \\
E4-multi & MNIST 多シード検証 & 3 シード×150/300ep & 主要 \\
EM & 理論-実験橋渡し（PCA スペクトル分析） & MNIST PCA+E4 対比 & 理論補足 \\
EL & 密グリッド検証（3因子分離） & MNIST FC-VAE, $m=8$〜64 密 & mknee 安定化 \\
EF & Fashion-MNIST 適用検証 & Fashion-MNIST FC-VAE & 異種実データ \\
EG & 畳み込み AE での ID 整合性確認 & MNIST Conv-AE & アーキテクチャ拡張 \\
EH & 固定$\beta$ Conv-VAE（未飽和の確認） & MNIST Conv-VAE & 限界確認 \\
EK & KL アニーリング Conv-VAE & MNIST Conv-VAE & 安定化探索 \\
EN & Conv-VAE 拡張 $m$ グリッド（新規） & MNIST Conv-VAE, $m=4$〜256 & Conv-VAE 飽和探索 \\
EN\_ext & Conv-VAE $m=384$・512 拡張 & MNIST Conv-VAE, 3シード & Conv-VAE 飽和限界確認 \\
EQ & MNIST AE $m$スイープ：$\kappa$・TwoNN 共変化 & MNIST AE, $m=4$〜128 & Stage 1 幾何的妥当性 \\
EI & CIFAR-10 TwoNN 外部妥当性確認 & CIFAR-10 Conv-VAE & 外部妥当性 \\
EJ & SVHN 部分崩壊モニタリング & SVHN Conv-VAE & 外部妥当性 \\
E6 & MNIST 学習ダイナミクス & MNIST FC-VAE, $m=16$ & 付随・単一シード \\
EY & Manifold Entanglement 予備検証 & MNIST クラス制限 & 付随・予備的 \\
EZ & $\tau_\mathrm{knee}$横断検証 & 複数データセット & 付録（感度分析） \\
\bottomrule
\end{tabular}
\end{table}
...
RQ & 主要実験 & 対応する命題 \\
\midrule
RQ1（$\mknee$の有用性と安定化条件） & MNIST 主実験(E4), 多シード検証(E4-multi), & \\
 & 密グリッド検証(EL), Fashion-MNIST 適用(EF) & 命題\ref{thm:lower_bound}，定理\ref{thm:linear_vae}，命題\ref{thm:mknee_instab} \\
RQ2（多指標フレームワーク安定性） & 原理確認(E1,EA,EB), MNIST 多シード(E4-multi) & 表\ref{tab:indicator_class} \\
RQ3（等長性） & IsometricAE 実験(EC) & 命題\ref{thm:pullback} \\
理論-実験橋渡し & PCA スペクトル分析(EM) & 定理\ref{thm:linear_vae} \\
付随（学習ダイナミクス） & 学習ダイナミクス(E6), Manifold Entanglement 予備(EY) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{TwoNN ID の数値安定性と等長性バイアスの実証（実験 EP）}\label{sec:exp_ep}

Stage 1 の参照 AE は$\kap \approx 10^9$（等長性なし）であるにもかかわらず，
TwoNN ID 推定が補助的参照推定量として数値的に安定に機能するかを検証する（実験 EP）．
...
\begin{table}[H]
\centering
\caption{EP Section A：Swiss Roll（$d_\mathrm{true}=2$）での TwoNN ID 比較．
...
\begin{table}[H]
\centering
\caption{EP Section A：Swiss Roll（$d_\mathrm{true}=2$）での TwoNN ID と $\kap$ の関係（概略値）．
...
（既存 E4 データより；$m \geq 64$で TwoNN$=9.16$，収束途上）．
...
\textbf{Section C — MNIST AE $m$-スイープ：$\kappa$と TwoNN の共変化（実験 EQ）}（表\ref{tab:eq_mnist}）：
...
\begin{table}[H]
\centering
\caption{実験 EQ：MNIST AE $m$スイープ——$\kap$と TwoNN ID の共変化（$\hat{d}_\mathrm{ID}\approx10$）．
...
\begin{figure}[H]
  ...
  \caption{実験 EP：（左）Swiss Roll での TwoNN ID 棒グラフ —
  ...
  実験 EQ（AE$m$スイープ）の結果は表\ref{tab:eq_mnist}を参照．}
  \label{fig:ep}
\end{figure}}{}
...
\subsubsection{線形理論―実験橋渡し：MNIST PCA スペクトル分析（実験 EM）}\label{sec:exp_em}

§\ref{sec:theory_bridge}で予告したように，MNIST PCA スペクトルを用いて
定理\ref{thm:linear_vae}（水充填原理）の定性的継承と定量的乖離を検証する（実験 EM）．
...
\begin{table}[H]
\centering
\caption{EM：MNIST PCA スペクトルに基づく線形理論予測（$\beta=4$）と非線形 VAE 観測値の比較}
...
\begin{table}[H]
\centering
\caption{EM：線形理論（定理\ref{thm:linear_vae}）と非線形 MNIST VAE（実験 E4，300ep, 3seed）の比較}
...
\begin{itemize}
  ...
  \item \textbf{グリッド密度因子（主要因の一つ）}：密グリッド（EL）では$\mknee = 10.3 \pm 0.9$（seed=[11,9,11]）—— 疎グリッド$43\pm15$から劇的に改善し，推定$\hat{d}_\mathrm{ID} \approx 10$と整合する
\end{itemize}
...
EL 密グリッドデータでの比較結果を表\ref{tab:el_knee_compare}に示す．

\begin{table}[H]
\centering
\caption{EL：$\tau$法（$\tau=0.25$）vs 二階差分法（d2法）の$\mknee$比較（密グリッド，300ep，3シード）．}
...
\paragraph{$\AUsat = 36$の解釈（実験 EY，予備検証）}
...
\subsubsection{Conv-VAE での AU 飽和遅延：固定$\beta$・KL アニーリング比較（実験 EH・EK）}
\label{sec:exp_conv_300ep}

EG の収束不足を受け，300ep・3シード（EH）および KL サイクリックアニーリング（EK）で検証した
（表\ref{tab:eh_conv_vae}）．

\begin{table}[H]
\centering
\caption{EH・EK：MNIST Conv-VAE の AU 比較（300ep，3シード平均$\pm$標準偏差）．
FC-VAE（E4-300ep-multiseed）は$m=32$で AU$=15.7$と大幅に飽和するのに対し，
...
\toprule
 & \multicolumn{2}{c}{Conv-VAE 固定$\beta$（EH）} & \multicolumn{1}{c}{Conv-VAE アニーリング（EK）} & \multicolumn{2}{c}{FC-VAE（E4-300ep）} \\
...
%% ===================================================================
\subsection{Conv-VAE 拡張$m$グリッド実験（実験 EN・EN\_ext）}\label{sec:exp_en}

EH・EK（§\ref{sec:exp_conv_300ep}）で$m \leq 64$では AU 飽和が観測されなかったことを受け，
$m$グリッドを 256（実験 EN）および 512（実験 EN\_ext）まで拡張した．
...
\textbf{実験設定}（EN）：
EK と同一の Conv-VAE アーキテクチャ（$28\times28$ MNIST，2層 Conv エンコーダ・
...
\begin{table}[H]
\centering
\caption{EN：Conv-VAE 拡張$m$グリッド（KL アニーリング，150 エポック，3シード平均$\pm$標準偏差）．
$m \leq 64$では AU$=m$（全次元活性），$m \geq 96$で AU$<m$（飽和開始）が現れる．
比較のため FC-VAE（実験 E4，300ep, $m=64$）の AU$=22.3\pm0.5$を掲載する．}
\label{tab:en}
\small
\begin{tabular}{rccc}
\toprule
...
\midrule
 ...
 64 & $64.0\pm0.0$  & $13.29\pm0.12$ & EK と同一（飽和なし） \\
...
\multicolumn{4}{l}{FC-VAE 比較（E4, 300ep, $m=64$）：AU$=22.3\pm0.5$，TwoNN ID$=10.03\pm0.12$} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{実験 EN：Conv-VAE 拡張$m$グリッド（$m\in[4,256]$，KL アニーリング，3シード）の
...
\textbf{$m=256$超への拡張（実験 EN\_ext）}：
EN の「$\mknee=256$（グリッド端）」という結果を受け，$m \in \{384, 512\}$まで拡張した実験
（EN\_ext，3シード，150ep）を実施した．

EN\_ext（3シード，150ep）の結果を表\ref{tab:en_ext}に示す．

\begin{table}[H]
\centering
\caption{実験 EN\_ext：MNIST Conv-VAE の$m \in \{384, 512\}$拡張結果（3シード平均；KL サイクリックアニーリング，$\beta_{\max}=4$，150ep）．
...
EN\_ext の結果から，MNIST の今回設定の Conv-VAE では$m \leq 512$の全範囲で
...
AU 飽和の観測には$\beta$スケジュールやデコーダ容量の見直しが必要であり，今後の課題とする．
...
\begin{itemize}
  ...
  \item \textbf{数値的安定性は不支持}：MNIST 多シード実験（150・300 エポック）では$\mknee$が 32〜64 に変動し，
        数値的再現性は支持されなかった．密グリッド実験（EL）の結果から，
...
\textbf{コアメッセージ}：
AU 値（$\sigma \leq 0.5$）・TwoNN ID（MNIST $m=64$: $10.03 \pm 0.12$，EQ 実験で補強）は
...
\begin{enumerate}
  \item \textbf{密なグリッド設計（実験 EL で実証済み）}：推定 $\hat{d}_\mathrm{ID}$（TwoNN ID から得られる）
        ...
        EL 実験の結果：疎グリッド（$\mknee=43\pm15$）→密グリッド（$\mknee=10.3\pm0.9$）
...
$\mknee$が$\hat{d}_\mathrm{ID}$と定性的に整合した（実験 E4，EL）．
一方，Conv-VAE では$m \leq 64$の全範囲で AU$\approx m$が継続し，
KL アニーリング（実験 EK）を適用しても検証範囲内でのAU 飽和プラトーは観測されなかった（実験 EH）．
...
実験 EM で推定した FC-VAE の$\tau^2_\mathrm{eff} \approx 0.30$に対して，
Conv-VAE では$\tau^2_\mathrm{eff} \ll 0.30$となり，
$d^*(\beta) \gg 64$が期待される．すなわち，Conv-VAE のフレームワーク適用には
$m$グリッドを$d^*(\beta)$に到達するまで大幅に拡張する必要がある．

\textbf{実験 EN・EN\_ext による検証}（§\ref{sec:exp_en}）~~~
$m \in [4,256]$（EN）・$m \in \{384,512\}$（EN\_ext）の実験で，
$m = 96$以上で AU$<m$となり飽和傾向が確認された（$m=256$: 活性率 94\%）一方，
$m=512$まで拡張しても AU$\approx m$が全3シードで維持されており，
明確な AU 飽和プラトーは観測されなかった（$\mknee$はグリッド端の$m=512$に張り付く）．
この結果は，Conv-VAE の$d^*(\beta)$が少なくとも$m=512$を超えることを示し，
MSE ベース強力デコーダの$\tau^2_\mathrm{eff}$低下という理論的予測を支持する
（$\beta$スケジュールや decoder 容量の最適化が今後の課題）．


\textbf{自然画像への含意}~~~
CIFAR-10・SVHN（実験 EI・EJ）では TwoNN ID の外部妥当性（Pope ら\cite{pope2021intrinsic}との整合）は確認されたが，
...
\begin{itemize}
  \item \textbf{Conv-AE・Conv-VAE 検証}（実験 EG・EH・EK・EN）：
        Conv-AE（EG）の TwoNN ID 一貫性を確認した．
        EH では固定$\beta$Conv-VAE が 300 エポックでも AU$\approx m$を示し，
        EK（KL アニーリング）でも$m\in[4,64]$内では明確な AU 飽和プラトーは観測されなかった．
        EN・EN\_ext 実験（§\ref{sec:exp_en}）により拡張$m$グリッドでの定性的傾向は確認されたが，
...
\begin{itemize}
  \item \textbf{Stage 1 の ID 推定：等長性バイアスの厳格な評価}：
        ...
        \textbf{実験 EP（§\ref{sec:exp_ep}）の追加観察}：Swiss Roll（$d_\mathrm{true}=2$，単一条件）での制御実験において，
...
\begin{enumerate}
  \item \textbf{$\tau_{\mathrm{knee}}$の適応的設定}：
        ...
        また，EL 密グリッドでの比較（§\ref{sec:exp_el}，表\ref{tab:el_knee_compare}）では，
