...
本論文では，VAE の Active Unit（AU）増加ダイナミクスに基づく
有効潜在次元の解釈フレームワークを提案し，内在次元（ID）推定との関係を検討する．
中心的な提案は，AU 増加率が初めて顕著に鈍化するボトルネック次元
$m^{\mathrm{knee}}$（\textbf{AU 鈍化点}）を有効表現次元の実用的指標として用いることである．
$m^{\mathrm{knee}}$は$m$グリッド・AU 閾値$\delta$・検出閾値$\tau$に依存する
\textbf{経験的ヒューリスティック}であり，
本稿では，この指標の有用性と限界を合成多様体・MNIST・Fashion-MNIST
（全結合型・畳み込み型 AE/VAE）を通じて実験的に検証する．

主要な実験的知見（本研究条件下）：
\begin{enumerate}
  \item \textbf{$m^{\mathrm{knee}}$と ID 推定の整合性}：
        Fashion-MNIST（100 エポック）では$m^{\mathrm{knee}} = 12$が
        TwoNN 推定値（$\hat{d}_\mathrm{ID} \approx 9$〜10）と対応する傾向が観測された．
        MNIST では，E4 拡張実験（seed 42，特定$m$グリッド）で$\mknee = 12$が得られたが，
        150・300 エポック多シード実験では$\mknee$が 32〜64（平均$43 \pm 15$）に変動し，
        $\mknee$の精確な値は$m$グリッドや乱数シードに依存することが判明した（§\ref{sec:exp_multiseed2}）．
        一方，AU 値・TwoNN ID（$m=64$: $10.03 \pm 0.12$）はシード間で高い再現性を示す．
  \item \textbf{畳み込みアーキテクチャでの ID 推定の一貫性}：
        Conv-AE の TwoNN ID 推定値（$\approx 11$）が全結合型（$\approx 10$）と整合し，
        ID 推定がアーキテクチャに依存しにくいことが示された．
        ただし，Conv-VAE における$\mknee$の信頼ある検出には，100 エポックでは収束不足であった
        （探索的知見；より多いエポック数が必要）．
\end{enumerate}
...
本論文では，この問いに対し，
\textbf{内在次元（ID）推定}と\textbf{Active-Unit（AU）Dynamics}の組み合わせによる
分析フレームワークを提案する．

\textbf{ID 推定の役割}：TwoNN\cite{facco2017twonn}・MLE\cite{levina2005mle} は
局所的な距離関係からデータの有効自由度を推定する手法であり，
本研究条件下では AE/VAE の有効潜在次元の事前候補を与える実用的な指標として機能する
（ただし，参照 AE を介した推定は近似であり，等長性バイアスを含む；§\ref{sec:disc_limitation}参照）．

\textbf{AU Dynamics の役割}：VAE の AU パターン，特に AU 増加率の鈍化点$\mknee$は，
KL 正則化（情報ボトルネック\cite{tishby2000ib}）が初めて有意な次元圧縮を行う点を示し，
実データにおける有効表現次元の実用的な観測指標として機能する可能性を実験的に示す．
$\mknee$は設計パラメータであると同時に，
「VAE がデータから\textbf{何次元の有効表現}を学習するか」を外から読む解釈ツールである．

本論文の\textbf{貢献}は以下のとおりである：
\begin{enumerate}
  \item \textbf{AU 鈍化点$\mknee$の定式化と実証的検証}（主要貢献）：
        AU 正規化増加率$r(m)$と鈍化点$\mknee$による有効潜在次元の読み取り方法を定式化する
        （§\ref{sec:framework}）．
        Fashion-MNIST では$\mknee = 12$が TwoNN 推定値（$\hat{d}_\mathrm{ID} \approx 9$〜10）と
        対応する傾向を観測した（§\ref{sec:exp}，実験 EF）．
        多シード実験（§\ref{sec:exp_multiseed2}）では，AU 値・TwoNN ID（$\approx 10$）の
        シード間再現性は高いが，$\mknee$の精確な値は実験設定（$m$グリッド・シード）に依存し，
        32〜64 の範囲で変動することが明確になった．
        $\mknee$は$m$グリッド・$\delta$・$\tau$・$\beta$・学習収束に依存する
        \textbf{経験的ヒューリスティック}であり，その限界を§\ref{sec:disc_limitation}で明示する．
  \item \textbf{有効次元の多指標分析フレームワーク}（主要貢献）：
        ID 推定（TwoNN/MLE），AU Dynamics（$\mknee$，$\AUsat$），MSE エルボー，CKA の
        組み合わせにより，設計者が単一指標に依存せず有効次元を解釈するフレームワークを提供する
...
なお，本論文の主役は$\mknee$であり，この AE/VAE と$\dID$/$\demb$の対応は
副次的観察として位置づけている．
...
\begin{description}
  \item[RQ1] VAE の AU 鈍化点$\mknee$は，本研究条件下において，
             ...
             安定して観測されるか．
  \item[RQ2] AE/VAE の有効潜在次元を多指標（AU，TwoNN/MLE，MSE エルボー，PCA）で
...
\textbf{検証範囲の明示}：
本論文の全定量的結果は，合成多様体（$d_{\mathrm{true}} \leq 3$）・MNIST・Fashion-MNIST
（いずれも全結合型 AE/VAE）を対象とした限定的な検証に基づく．
CIFAR-10 等の自然画像（Conv-AE が必要）への一般化は§\ref{sec:disc_limitation}で議論する．
...
\begin{description}
  ...
  \item[漸増型（実データ，MNIST 等）]
        ...
        $\hat{d}_\mathrm{ID}$との整合が設計の信頼性を支持する．
\end{description}
...
\begin{tabular}{c}
  ...
  \fbox{\parbox{0.70\linewidth}{\centering
    ...
    \textbf{解釈}：$\mknee \approx \hat{\dID}$ → KL 正則化が$\hat{\dID}$次元で圧縮開始
  }} \\[3pt]
  $\Big\downarrow$ \\[1pt]
  \fbox{\parbox{0.70\linewidth}{\centering
...
この鈍化点$\mknee$が，データの有効内在次元$\hat{d}_\mathrm{ID}$と整合する
（本研究条件下での経験的観測；§\ref{sec:exp}参照）．
...
\textbf{$\mknee$の解釈}：
$\mknee = 12$は，AE の TwoNN ID 収束値（$\hat{d}_\mathrm{ID} \approx 10$〜12）と整合する．
これは KL 正則化が MNIST の有効内在次元（$\approx 10$〜12）を「感知」して
次元圧縮を開始したことを示す——すなわち，
\textbf{VAE は$\mknee$付近の次元数でデータの本質的な変動を効果的に表現している}と解釈できる．
...
\textbf{$\mknee$（VAE，$\tau=0.25$）} & \textbf{12} & E4-VAE & KL ダイナミクス直接観察，$\tau$依存 \\
\bottomrule
\end{tabular}
\end{table}
...
300 エポックの完全収束版（実験 E4）では$\mknee = 12$が得られており，
\textbf{$\mknee$の信頼性はエポック数（収束度）に強く依存する}ことを本結果は直接示す．
この収束依存性は§\ref{sec:disc_limitation}で限界として記載した内容を定量的に裏付けるものである．
...
\subsection{$\mknee$ の有効性と限界：経験的ヒューリスティックとしての位置づけ}\label{sec:disc_mknee}

本研究の中心的な実験的観察は，MNIST・Fashion-MNIST において，
$\mknee$（$\tau = 0.25$）が TwoNN 推定値（$\hat{d}_\mathrm{ID} \approx 10$〜12）と
おおむね一致する傾向が観測されたという点である．
この観察は 300 エポック（E4，単一シード）での結果であり，
全結合・畳み込み型の両アーキテクチャでの TwoNN ID 推定の一貫性（Conv-AE: 11.29 vs FC-AE: 10.02）は確認された．
ただし，あくまで\textbf{本研究の限定的な条件下での経験的観測}であり，普遍的な性質を主張するものではない．

$\hat{d}_\mathrm{ID}$（参照 AE 経由）は等長性バイアスを含む近似値であるため（$\kap \approx 10^9$），
$\mknee$と$\hat{d}_\mathrm{ID}$の「整合」は厳密な意味での理論的一致を意味しない．
むしろ，$\mknee$は「KL 正則化が有意な次元圧縮を開始する転換点」として
実用的な参照値を与える観測量として解釈するのが妥当である．

一方，$\mknee$は以下の要因に依存する経験的ヒューリスティックであることに留意が必要である：
\begin{itemize}
  ...
  \item $\beta$の値（予備実験：$\beta \in [2, 8]$で$\mknee = 12$が安定；付録\ref{app:delta}）
...
本論文では，AE/VAE の有効潜在次元を外から定量的に読む分析フレームワークを提案し，
合成多様体・MNIST・Fashion-MNIST での実験的検証を行った．

\textbf{主要実験知見のまとめ（本研究条件下）}：
\begin{enumerate}
  \item \textbf{$\mknee$の実験的知見と収束依存性}：
        300 エポック（単一シード）の MNIST 実験では$\mknee = 12$が得られ，
        TwoNN 推定値（$\hat{d}_\mathrm{ID} \approx 10$〜12）と対応する傾向が観測された．
        一方，3シード多重検証（150 エポック）では，各$m$での AU 値はシード間で一致（$\sigma \leq 0.5$）するが，
        $\mknee$は 32〜64 に変動（平均$53 \pm 15$）し，収束依存性が明確に示された．
        $\mknee$は$\tau$・$\beta$・$m$グリッド・収束度に依存する\textbf{経験的ヒューリスティック}であり，
        信頼性ある推定には十分な収束（300 エポック以上を推奨）が必要である．
  \item \textbf{多指標フレームワークの有用性}：
        ID 推定，AU Dynamics，MSE エルボー，PCA の組み合わせにより，
        各指標単独では得られない相補的な情報が得られることを示した
...
\textbf{今後の課題}：
より広いデータ・アーキテクチャ（Conv-AE，自然画像 CIFAR-10 等）への拡張，
全実験の多シード系統的検証，
...
$\beta \in [2, 8]$では$\mknee = 12$が安定して観測されることを確認した（本研究条件下）．
