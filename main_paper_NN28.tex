...
%% AE/VAEにおける有効潜在次元の診断的解釈（版28）
%% コンパイル: platex main_paper_NN28.tex && dvipdfmx main_paper_NN28.dvi
...
実験面では，MNIST・Fashion-MNIST において AU 値と TwoNN ID が複数シードにわたり
高い再現性を示す一方（AU $\sigma \leq 0.5$，TwoNN ID $10.03 \pm 0.12$），
$\mknee$は疎グリッドでは不安定であり，密グリッドと複数シードを用いた場合にのみ
TwoNN ID 近傍の探索的目安として機能することを示す．
さらに Conv-VAE および CIFAR-10/SVHN 実験を通じて，
AU 診断が困難となる条件を定量化し，本フレームワークの適用可能範囲と限界を明確化する．
\end{abstract}
...
本論文の\textbf{主な貢献は以下の3点}である（補助的貢献を第4点として付加する）：
\begin{enumerate}
  \item \textbf{多指標診断フレームワークの提案}（主要貢献）：
        AU 値，TwoNN/MLE ID，MSE エルボー，PCA を組み合わせ，
        AU 値を頑健な直接観測指標，TwoNN ID を等長性バイアスを含む補助的参照推定量，
        $\mknee$をグリッド・シード依存性をもつ探索的指標として明確に区別する
        フレームワークを提供する（§\ref{sec:framework}，§\ref{sec:discussion}）．
        MNIST・Fashion-MNIST での3シード検証により AU 値（$\sigma \leq 0.5$）と
        TwoNN ID（$10.03 \pm 0.12$）の高い再現性を確認した（§\ref{sec:exp_multiseed2}）．
  \item \textbf{線形ガウス近似に基づく AU 活性化条件の導出}（理論的貢献）：
        ...
        本定理は実際の非線形 VAE の厳密定理ではなく，線形ガウス rate-distortion 近似に
        基づく解析結果として非線形実験との定性的対応を与えるものである．
  \item \textbf{フレームワークの適用可能範囲と限界の定量化}（実験的貢献）：
        MNIST/Fashion-MNIST（FC-VAE），Conv-VAE，CIFAR-10/SVHN を対象に，
        AU 値・TwoNN ID が比較的高い再現性を示す一方，$\mknee$は疎グリッドでは不安定であることを
        3要因（グリッド密度・シード・エポック）分離実験で定量化した（§\ref{sec:exp_el}）．
        Conv-VAE・自然画像では stable identification の5条件（operational criteria）を定義し，
        AU 診断が困難となる境界条件を定量化する（§\ref{sec:disc_convvae}，§\ref{sec:disc_limitation}）．
  \item \textbf{TwoNN 適用条件の実証と理論化}（補助的貢献）：
        MNIST・Fashion-MNIST における実データ幾何比較実験（Exp-Real-Geom）により，
        「等長性改善より$m \gg \hat{d}_\mathrm{ID}$が TwoNN 安定性の支配的条件」を示し，
        命題\ref{thm:twonn_stability}として条件付き安定性の十分条件を理論的に整理した（§\ref{sec:exp_real_geom}）．
\end{enumerate}
...
strict な stable identification は今後の目標である（付録\ref{app:convvae_natimg}に5条件の詳細判定を示す）．
...
\textbf{$\mknee$検出法比較（Exp-NatImg-Knee，詳細は付録\ref{app:convvae_natimg}）}：
自然画像では AU 曲線が MNIST より滑らかで固定$\tau$法が最適とは限らない．
4手法（固定$\tau$，二階差分，平滑化曲率，区分線形）の比較では，
\textbf{平滑化後曲率法}が最も再現性が高かった（$\sigma=96$，詳細は付録\ref{app:convvae_natimg}）．
...
\paragraph{学習ダイナミクス（実験 Exp-Mnist-Dyn，単一シード；詳細は本稿では割愛）}
MNIST（$m=16$，200ep）での2フェーズ学習ダイナミクス（大域構造の急速形成後に局所的有効次元が漸進的増加）は
単一シードの探索的観察であり（詳細は付録（本稿では割愛），多シード検証は今後の課題である．
...
\begin{itemize}
  \item \textbf{FC-VAE + MNIST/Fashion-MNIST}：AU 値（$\sigma\leq0.5$）・TwoNN ID（$\sigma\leq0.12$）は複数シードにわたり安定．$\mknee$は密グリッド・複数シードの条件下でのみ TwoNN ID 近傍の探索的目安として機能する（Exp-Mnist-Base，Exp-Mnist-Dense-300，Exp-Fashion）．
  \item \textbf{Conv-VAE + MNIST}：TwoNN ID は安定；AU の$m<m$開始は$m\geq96$で確認．$\mknee$は$\beta_{\max}\geq10$で同定可能だが，$\beta_{\max}=4$では検証範囲内に$d^*(\beta)$が収まらない（Exp-Conv-M256，Exp-ConvV-HiBeta）．
