...
%  MDPI MAKE submission — main_paper_MAKE36.tex
%  English version of main_paper_NN36.tex (Japanese draft, version 36)
%  Version 36: revisions in response to Review_codex35.txt (Major 1--8, Minor 1--10)
%    - central claim reframed as a conditional, diagnostic protocol (Major 1)
%    - reproducible pass/fail rule for Step 3 (V1: AU/m <= 0.9; V2: dhat <= AU <=
%      3*dhat) introduced and applied as explicit verdicts across all experiments
%      (Major 2, 8)
%    - evidence-scope limitations (single-seed Fashion-MNIST Step 3 etc.) mirrored
%      in the abstract and contributions (Major 3)
%    - d_ID / d_emb / AU distinction reiterated where synthetic results are
%      interpreted (Major 4)
%    - new experiment Exp-Boot-CI: subsampling CIs, subsample-size sensitivity,
%      and DANCo/ESS estimator cross-checks for Step 1 (Major 5)
%    - new experiment Exp-Downstream: linear-probe downstream accuracy and
%      search-cost comparison against a full m-grid sweep (Major 6)
%    - "guarantees"/"self-repairs" wording weakened to surrogate predictions
%      (Major 7)
%    - abstract shortened; single- vs multi-seed evidence separated in tables;
%      knee labels removed from figures; ARD/Bayesian-PCA related work added;
%      supplementary-file inventory stated (Minor 1--10)
%  Compiled with official mdpi.cls
%  Compile: pdflatex main_paper_MAKE36.tex  (x3 for cross-references)
...
%=================================================================
\Title{Choosing the Bottleneck Dimension of Autoencoders and Variational
Autoencoders from the Intrinsic Dimension of the Data: An ID-Guided
Protocol with Active-Unit Verification}
...
%=================================================================
\abstract{%
The bottleneck dimension $m$ of autoencoders (AEs) and variational
autoencoders (VAEs) is the most fundamental design parameter governing the
quality of learned representations, yet in practice it is usually chosen by
trial and error or convention.
Under the manifold hypothesis, this paper proposes a conditional,
diagnostic ID-guided protocol---a three-step procedure of estimation,
setting, and verification---in which \textbf{a candidate range for $m$ is
derived from an estimate $\hatdID$ of the intrinsic dimension of the input
data (AE: $m \in [\hatdID, 2\hatdID]$; VAE: $m \geq 2\hatdID$), and the
active units (AU) of the trained VAE diagnose whether the surplus dimensions
were actually truncated}.
The rationale is the topological lower bound $m \geq \demb \geq \dID$
required for self-intersection-free reconstruction, and a
water-filling-type activation condition $\lambda_j > \beta\tau^2$ in a
linear-Gaussian surrogate.
On synthetic manifolds with known ground truth and on fully connected VAEs
on MNIST (3 seeds each), all three steps pass a reproducible decision rule
(Swiss Roll: $\AUsat = \dtrue = 2$; Torus: $\AUsat = \demb = 3$; MNIST:
$\hatdID = 10.03 \pm 0.12$ consistent with the AU slowdown region), and the
ID-guided candidate range reduces the training runs of a full grid search by
roughly 60--80\% while attaining the grid-best downstream linear-probe
accuracy inside the range.
We also quantify the limits of applicability: intrinsic-dimension estimation
is stable only through reference representations with $m \gg \hatdID$ and
inflates beyond $m \approx 2\hatdID$ on natural images, and for Conv-VAEs
the AU truncation fails at $\beta = 4$ (strengthening $\beta$ restores
truncation, but only as a single-seed exploratory observation).
The protocol is thus not a general solution: \textbf{$\hatdID$ provides a
candidate range, and AU diagnoses the success or failure of truncation under
a suitable $\beta$ and architecture}.}

\keyword{variational autoencoder; bottleneck dimension; intrinsic dimension;
manifold hypothesis; active units; TwoNN; model selection;
representation learning; knowledge extraction}
...
From this viewpoint, \textbf{the appropriate size of the bottleneck
dimension $m$ should be determined not by model-side convention but by a
data-side quantity---the manifold dimension}.
...
\begin{quote}
\textbf{A candidate range for the bottleneck dimension $m$ can be derived
from an estimate $\hatdID$ of the intrinsic dimension of the input data as
``AE: $m \in [\hatdID,\, 2\hatdID]$; VAE: $m \geq 2\hatdID$,'' and the
active units (AU) of a VAE---the number of coordinates actually used under
KL regularization---serve, under a suitable $\beta$ and architecture, as a
post-training diagnostic of whether the surplus dimensions were actually
truncated.}
\end{quote}
This is not a claim of a general solution to choosing $m$; it is the
proposal of a \textbf{conditional, diagnostic workflow}
(applicability conditions are summarized in Table~\ref{tab:applicability}).
The claim is supported both theoretically and experimentally.
On the theoretical side we use (i) the topological lower bound
$m \geq \demb \geq \dID$ required to reconstruct a $d$-dimensional manifold
without self-intersection (Proposition~\ref{thm:lower_bound}), and (ii) the
activation condition $\lambda_j > \beta\tau^2$ obtained from a water-filling
analysis of the linear-Gaussian setting
(Proposition~\ref{thm:linear_vae}).
The latter \textbf{predicts, within the linear-Gaussian surrogate}, that
even if $m$ is set above the required dimension, the KL regularization of
the VAE truncates the surplus dimensions (posterior collapse) and the AU
count reports the effective number of used dimensions---in other words,
\textbf{whereas for an AE the choice of $m$ itself fixes the representation
dimension, for a VAE the safe-side operation ``generous $m$ + AU
diagnosis'' can be expected} (whether this expectation actually holds is
itself checked by the Step-3 decision rule).

The contributions of this paper are threefold:
\begin{enumerate}
  \item \textbf{An ID-guided bottleneck-dimension selection protocol}
        (Section~\ref{sec:protocol}):
        a diagnostic workflow consisting of
        ...
        (Equations~\eqref{eq:rule_v1}--\eqref{eq:rule_v2}), failure modes,
        and reporting requirements.
  \item \textbf{Theoretical grounding of the selection principle}
        (Section~\ref{sec:theory}):
        we organize the topological lower bound
        (Proposition~\ref{thm:lower_bound}) and the surrogate activation
        condition $\lambda_j > \beta\tau^2$ under a linear-Gaussian
        rate--distortion approximation (Proposition~\ref{thm:linear_vae};
        $\tau^2$ is the isotropic Gaussian observation-noise variance, and
        the correspondence between the KL term and mutual information is an
        approximation that neglects the aggregated-posterior mismatch---both
        stated in the claim body), and derive the operational implication
        that ``the surrogate model predicts truncation of an excessive $m$,
        but the truncation threshold depends on $\beta$.''
  \item \textbf{Multi-seed validation and quantified applicability
        boundaries} (Section~\ref{sec:exp}):
        multi-seed validation of the full protocol is given on synthetic
        manifolds with known ground truth (the protocol returns $\dtrue$ and
        $\demb$; 3 seeds) and on fully connected VAEs on MNIST
        ($\hatdID \approx 10$--12 consistent with the AU slowdown region;
        3 seeds).
        Step 1 is accompanied by subsampling confidence intervals and
        cross-checks with estimators of different principles (DANCo, ESS).
        We also show with a linear probe that the ID-guided range reduces
        search cost while retaining downstream classification accuracy
        (Section~\ref{sec:exp_downstream}).
        We further quantify the limits of applicability:
        (a) the estimation of $\hatdID$ is stable only through reference
        representations with $m \gg \hatdID$, while isometry improvements
        are secondary (four-model comparison on MNIST and Fashion-MNIST,
        3 seeds; Proposition~\ref{thm:twonn_stability});
        (b) on natural images the TwoNN estimate agrees with reported
        values~\cite{pope2021intrinsic} up to $m \approx 2\hatdID$ but
        inflates beyond;
        (c) for Conv-VAEs at $\beta = 4$ the truncation threshold $\dstar$
        exceeds 512, so AU verification requires a strengthened $\beta$.
        The Step-3 evidence for Fashion-MNIST, the $\beta$-strengthened
        Conv-VAE recovery, and the natural-image AU plateau are single-seed
        exploratory observations (Appendix~\ref{app:exploratory}), and
        Step 3 on natural images remains unresolved---the contributions are
        claimed inclusive of these limitations.
\end{enumerate}
...
Our validation is restricted to synthetic manifolds with known ground truth
...
An estimate $\hatdID$ (TwoNN/MLE) that is ``numerically stable'' is not
necessarily ``unbiased with respect to the true intrinsic dimension''
(stability vs.\ validity; Section~\ref{sec:disc_twonn}); this paper treats
$\hatdID$ as a \textbf{reference value} for selection and does not claim to
estimate the true value.
Automatic detection of change points (knees) on the AU curve is strongly
grid- and seed-dependent, so it is not a named output of the protocol and is
placed as an exploratory analysis in Appendix~\ref{app:knee}.
Full application to large-scale natural images remains future work.

The remainder of the paper covers related work (Section~\ref{sec:related}),
the protocol (Section~\ref{sec:protocol}), its theoretical grounding
...
MLE~\cite{levina2005mle} (DANCo~\cite{ceruti2014danco} is more precise but
computationally expensive and left as future work).
...
derives a candidate range from the data-geometry side ($\hatdID$) and
...
%% ===================================================================
\section{ID-Guided Protocol for Choosing the Bottleneck Dimension}\label{sec:protocol}
...
\begin{tabular}{c}
  ...
  \fbox{\parbox{0.78\linewidth}{\centering
    ...
    AE: candidate range $m \in [\hatdID,\, 2\hatdID]$ (includes topological margin) \\
...
for stable estimation is supported by
Proposition~\ref{thm:twonn_stability} and the experiments
(Section~\ref{sec:exp_real_geom}).
...
\textbf{For AEs}: for $m < \demb$, self-intersection-free reconstruction is
impossible in principle (Proposition~\ref{thm:lower_bound}), and any excess
in $m$ remains as redundant dimensions (an AE has no automatic truncation).
We therefore take $m \in [\hatdID,\, 2\hatdID]$ as the candidate range,
absorbing the possibility of a topological obstruction
($\demb > \dID$; Torus type) into the margin.
Choose the upper side of the range if reconstruction accuracy matters, the
...
\textbf{setting $m \geq 2\hatdID$ generously is the safe-side operation}
...
For a VAE trained at a verification point $m_{\mathrm{ver}} \geq 2\hatdID$,
evaluate the following two conditions on the multi-seed mean AU:
\begin{align}
...
reflecting the manifold dimension of the data);
...
(Table~\ref{tab:failure}).
The threshold $0.9$ and factor $3$ are \textbf{conservative working
defaults}; in all settings of this paper the verdict is invariant over a
wide range of thresholds (activation ratios of passing cases are
$0.35$--$0.67$, clearly separated from failing cases at $0.94$--$1.0$;
Section~\ref{sec:exp}).
Simple manifolds show sharp saturation ($\AUsat = \dtrue$ or $\demb$), while
real data show a gradual slowdown (Section~\ref{sec:exp_mnist}).

\textbf{AE: MSE verification (qualitative diagnostic)}.
...
\toprule
\textbf{Symptom} & \textbf{Likely cause} & \textbf{Remedy / interpretation} \\
\midrule
...
All experiments in this paper follow these requirements.
...
\midrule
Synthetic manifolds ($\dtrue \leq 3$, 3 seeds) & Direct input-space estimate; matches $\dtrue$ & \textbf{Pass} (V1/V2): sharp saturation at $\dtrue$ / $\demb$ ($\sigma = 0$) & Fully applicable (ground-truth validated) \\
\addlinespace
FC-VAE on MNIST (3 seeds) / Fashion-MNIST (Step 1: 3 seeds; Step 3: single seed) & Stable: $10.03 \pm 0.12$ / $\approx 9$ & \textbf{Pass} (V1/V2) on MNIST; Fashion-MNIST exploratory & Applicable (MNIST fully validated) \\
\addlinespace
Conv-VAE on MNIST & Stable (inflates while AU $= m$) & \textbf{Fail} at $\beta_{\max} = 4$ (V1); $\beta_{\max} \geq 10$ restores V1 only (single seed) & Applicable only with strong $\beta$ (exploratory) \\
\addlinespace
Natural images (CIFAR-10 / SVHN, 3 seeds) & Consistent with Pope et al.~\cite{pope2021intrinsic} at $m_{\mathrm{ref}} \approx 2\hatdID$; inflates beyond & \textbf{Unresolved}: no V1/V2 pass within tested range & Step 1 usable with care; Step 3 open problem \\
\bottomrule
\end{tabular}
\end{table}

%% ===================================================================
\section{Theoretical Grounding of the Selection Principle}\label{sec:theory}
...
Proposition~\ref{thm:twonn_stability} is a sketch of conditional stability
that grounds the design of Step 1.
...
For a continuous AE to reconstruct a smooth $d$-dimensional manifold
$\mathcal{M} \subset \R^N$ without self-intersection and with ideally zero
reconstruction error, it is necessary that $m \geq \demb \geq d$, where
$\demb$ is the topological minimum embedding dimension of $\mathcal{M}$.
\end{Proposition}

\begin{proof}
Zero reconstruction error requires $f|_\mathcal{M}: \mathcal{M} \to \R^m$
to be a continuous injection, and no continuous injective embedding of a
$d$-dimensional manifold into $\R^m$ with $m < \demb$ exists by the
definition of the topological embedding
dimension~\cite{hatcher2002algebraic}.
\end{proof}

This is the rationale for the lower end of Step 2.
For real data $\demb$ is unknown, but since $\demb \geq \dID$, the estimate
$\hatdID$ provides the reference point ``below this is insufficient in
principle,'' and the possibility $\demb > \dID$ (a Torus-type topological
obstruction) is one reason for the margin ``set $m$ above $\hatdID$.''
The experiments confirm that this lower bound appears as the MSE elbow for
the Swiss Roll ($\demb = 2$) and the Torus ($\demb = 3 > \dID = 2$)
...
\begin{Remark}[Gap to empirical AU]
...
\subsection{Grounding of Step 1: Conditional Stability of Intrinsic-Dimension Estimation}\label{sec:theory_twonn}

\begin{Proposition}[Conditional stability of TwoNN (sketch)]\label{thm:twonn_stability}
...
(with $C \to 0$ as $\varepsilon, \delta_T \to 0$), and the deviation between
the input-space and latent-space TwoNN estimates is governed by the same
bound.
This proposition is not a guarantee of unbiasedness but a sketch of
sufficient conditions for conditional stability (proof sketch in
...
and Fashion-MNIST, and does it have practical value in search cost and
...
\textbf{All results in the main text are multi-seed experiments with at
least 3 seeds (42, 123, 777)}; single-seed exploratory observations are
separated into Appendix~\ref{app:exploratory} (exceptions are stated
...
\textbf{MNIST / Fashion-MNIST}: input dimension 784, hidden layers
...
Fashion-MNIST: 300 epochs (Exp-Real-Geom; $n_{\mathrm{train}} = 20{,}000$).
...
(iii) the training logs (JSON; all seeds); and
(iv) environment information (random seeds, library versions).
...
even when $m$ is set above $\demb$, the AU of the VAE ($\beta = 4$)
saturates at $\AUsat = 2 = \dtrue$ for the Swiss Roll and at
$\AUsat = 3 = \demb$ for the Torus in all seeds (standard deviation 0.0).
...
empirical observation}---under sufficient $\beta$, convergence, and
standardization preprocessing, the number of coordinates remaining after KL
...
\textbf{Verdict} (Step-3 rule): both Swiss Roll
($m_{\mathrm{ver}} = 4 = 2\hatdID$: AU $= 2.0$, activation ratio $0.50$) and
Torus ($m_{\mathrm{ver}} = 6 = 3\hatdID$: AU $= 3.0$, activation ratio
$0.50$) \textbf{pass} V1 and V2 (3 seeds, $\sigma = 0$).
...
TwoNN converges to around $10.0$ for $m_{\mathrm{ref}} \geq 64$
(the 3-seed value on the VAE-side latents is $10.03 \pm 0.12$;
Table~\ref{tab:mnist_s3}), and MLE
...
variance and 86 for 90\%, showing that linear methods grossly overestimate
the nonlinear manifold structure (Figure~\ref{fig:e4_dualaxis}, left).
...
\begin{tabular}{rcccc}
\toprule
$m_{\mathrm{ref}}$ & \textbf{MSE} & \textbf{TwoNN ID} & \textbf{MLE ID} & \textbf{CKA} \\
\midrule
...
Since $\hatdID$ provides the center of the candidate range, we quantify its
...
candidate-range center $\hatdID \approx 10$--$12$ is robust to sampling
variability.
...
In the raw input space, by contrast, DANCo degenerates and returns the
ambient dimension $784$ (and ESS inflates to $18.7$), supporting
\textbf{the necessity of the reference-AE route} also from the estimator
side.
...
\subsubsection{Step 2: Setting the Candidate Range}

From $\hatdID \approx 10$--$12$, the candidate range is $m \in [10, 24]$
for the AE and a generous $m \geq 20$ for the VAE.

\subsubsection{Step 3: Verification by AU and MSE (3 Seeds)}

Table~\ref{tab:mnist_s3} shows AU$(m)$ of the VAE ($\beta = 4$, 300 epochs,
3 seeds).
AU is highly reproducible across seeds ($\sigma \leq 0.5$); nearly all
...
plain-AE $m$-sweep), consistent with the candidate range $[10, 24]$.

\textbf{Verdict} (Step-3 rule): at $m_{\mathrm{ver}} = 20 = 2\hatdID$,
AU $= 13.3 \pm 0.5$ with activation ratio $0.67 \leq 0.9$ (V1 holds) and
$\hatdID = 10 \leq 13.3 \leq 30 = 3\hatdID$ (V2 holds): \textbf{pass}.
The verdict also passes at $m_{\mathrm{ver}} \in \{32, 64\}$
...
vertical dotted lines mark the PCA 80/90/95\% dimensions (43/86/152), far
above the nonlinear estimate $\hatdID \approx 10$.
...
(3 seeds, 300 epochs). Left: per-seed AU$(m)$ with the ID-guided candidate
range $[\hatdID, 2\hatdID]$ shaded; right: mean$\pm$std band with the Step-1
...
300 epochs) gives $\hatdID \approx 9$ (TwoNN converging to $9.9$--$10.1$
for $m_{\mathrm{ref}} = 5\hatdID$--$10\hatdID$).
A full multi-seed application of the protocol including VAE-side AU
verification was confirmed only in a single-seed preliminary observation
with the same tendency as MNIST (Appendix~\ref{app:exploratory}); multi-seed
...
$91.6\%$).
\textbf{The best accuracy over the full grid (11 points), $92.3\%$, is
attained inside the ID-guided candidate range $[10, 24]$ (at $m = 12$)},
...
holds on the VAE side: the linear-probe accuracy attains its best value over
the grid $\{4, \ldots, 64\}$, $89.1 \pm 0.4\%$, at $m = 12$ inside the
candidate range, and is nearly flat for $m \geq 12$
(at the VAE-path verification point $m = 20 = 2\hatdID$ it is
$88.2 \pm 0.4\%$, within 1 point of the best).
...
The retained performance is nearly identical to the grid-best as described
above.
This comparison is, however, limited to a linear probe on MNIST
...
mean$\pm$std, 3 seeds). Bold $m$ values lie in the ID-guided candidate
range $[10, 24]$, which contains the best accuracy over the full grid.
...
($d = \hatdID$: 10 for MNIST, 9 for Fashion-MNIST), comparing TwoNN, MLE,
...
That is, \textbf{the dominant condition for Step-1 stability is not
``improved isometry'' but the ``bottleneck margin
$m_{\mathrm{ref}} \gg \hatdID$''} (consistent with the $\delta_T$-dominance
implication of Proposition~\ref{thm:twonn_stability}), justifying the design
...
AU $\approx m$---that is, at $\beta_{\max} = 4$ the truncation threshold
satisfies $\dstar > 512$, and \textbf{the AU verification detects the state
...
experiments: at $\beta_{\max} = 10$, AU $= 93$ (36\%) at $m = 256$; at
$\beta_{\max} = 20$, AU $= 66$ (26\%)---a clear induced saturation
(Exp-ConvV-HiBeta; \textbf{single-seed exploratory observation}, details in
Appendix~\ref{app:exploratory}).
...
Step 2---candidate range obtained ($m \geq 20$);
Step 3---\textbf{fail} at $\beta_{\max} = 4$ (activation ratio
$0.94 > 0.9$ at $m = 256$; V1 violated).
At $\beta_{\max} \in \{10, 20\}$, V1 holds (activation ratios $0.36$ and
$0.26$) but V2 remains violated since AU $= 93$ and
$66 > 3\hatdID = 30$ (and the observation is single-seed exploratory);
...
For Conv-VAEs on CIFAR-10 and SVHN (3 Conv layers, $\beta_{\max} = 4$,
...
$28.8$--$30.9$---\textbf{on natural images the estimate is trustworthy only
near $m_{\mathrm{ref}} \approx 2\hatdID$, and ``the larger the better'' does
not hold} (third failure mode of Table~\ref{tab:failure}).
...
Step 1---usable only in the range $m_{\mathrm{ref}} \approx 2\hatdID$
(CIFAR-10: $25.21 \pm 0.41$, consistent with reported values; inflation
beyond);
Step 2---candidate range obtained conditionally (CIFAR-10: $m \geq 50$);
...
Step 1 agrees with Pope et al.~\cite{pope2021intrinsic} only near
$m \approx 2\hatdID$; Step 3 (AU saturation) is not reached in the tested
range.}
\label{tab:natural}
\small
...
Step 1 narrows the candidate range to the vicinity of $\hatdID$ before any
...
the linear-probe accuracy all supported the same candidate range $[10, 24]$
(Section~\ref{sec:exp_downstream}), and choosing $m$ inside the range
reduced the number of training runs by roughly 60--80\% relative to the
full grid search while nearly retaining the downstream classification
accuracy.
...
center of the candidate range'' and closes the loop with the Step-3
verification.
...
\begin{itemize}
  \item \textbf{Validation scope}: multi-seed validation of the full
        protocol is limited to synthetic manifolds and MNIST (Fashion-MNIST
        is multi-seed for Step 1 only).
        The recovery of Conv-VAE truncation by strengthened $\beta$
        (Exp-ConvV-HiBeta) is a single-seed exploratory observation and
        needs multi-seed replication.
...
To the question ``can the bottleneck dimension $m$ of AEs/VAEs be determined
from the manifold dimension of the input data,'' this paper gave an answer
in the form of a \textbf{conditional, diagnostic ID-guided protocol}:
(S1) intrinsic-dimension estimation through reference representations with
$m_{\mathrm{ref}} \gg \hatdID$,
(S2) setting $m$ as $\hatdID$ plus a topological margin
(AE: $m \in [\hatdID, 2\hatdID]$; VAE: $m \geq 2\hatdID$), and
(S3) post-training verification by AU and MSE equipped with the pass/fail
rule V1/V2.
The claim takes a conditional form rather than that of a general solution:
\textbf{$\hatdID$ provides a candidate range, and AU diagnoses whether the
VAE actually truncated the surplus dimensions under a suitable $\beta$ and
architecture}.
Theoretically, the topological lower bound and the water-filling surrogate
activation condition $\lambda_j > \beta\tau^2$ provide the rationale for
each protocol step;
experimentally, the full protocol passed the decision rule on synthetic
manifolds with known ground truth ($\AUsat = \dtrue$ or $\demb$; 3 seeds,
$\sigma = 0$) and on fully connected VAEs on MNIST
($\hatdID = 10.03 \pm 0.12$ consistent with the AU slowdown region;
3 seeds), with the ID-guided range reducing the training runs of a full
grid search by roughly 60--80\% while retaining downstream classification
accuracy.
At the same time we quantified the applicability boundaries:
intrinsic-dimension estimation is stable only with bottleneck margin
$m_{\mathrm{ref}} \gg \hatdID$ (isometry improvements are secondary;
four-model comparison), inflates beyond
$m_{\mathrm{ref}} \approx 2\hatdID$ on natural images, and for Conv-VAEs an
insufficient $\beta$ is detected as a V1 violation (AU $= m$); strengthening
$\beta$ restores truncation but order consistency with $\hatdID$ (V2)
remains unmet (a single-seed exploratory observation).
The main future tasks are multi-seed stable identification of Step 3 on
natural images and Conv-VAEs, nested validation of change-point detection,
and bias calibration of $\hatdID$.
...
Let $R_j \geq 0$ be the rate (KL contribution) allocated to direction $j$
and $D_j(R_j)$ the corresponding distortion reduction; the Gaussian
rate--distortion function gives $D_j(R_j) = \lambda_j e^{-2R_j}$ (nats), so
the relevant term of the $\beta$-ELBO is
\begin{equation}
  \mathcal{L}_j(R_j) = -\frac{1}{2\tau^2}\lambda_j e^{-2R_j} - \beta R_j + \text{const.}
\end{equation}
Optimization over $R_j$ is a concave program, and the KKT condition
...
Their difference is the aggregated-posterior mismatch
...
\subsection[\appendixname~\thesubsection]{Proof Sketch of Proposition~\ref{thm:twonn_stability} (Conditional Stability of TwoNN)}
...
\section[\appendixname~\thesection]{Single-Seed and Exploratory Experiments}\label{app:exploratory}

All experiments in this appendix are \textbf{single-seed (42) exploratory
observations} and are not used as direct evidence for the main-text claims.
...
$m \in \{16, 32, 64, 96, 128, 192, 256\}$).
At $\beta_{\max} = 10$ the activation rate drops to 36\% at $m = 256$ and
the change-point detector ($\tau$-method) returns $\approx 128$; at
$\beta_{\max} = 20$ it moves further down ($\approx 96$).
This is consistent with the qualitative prediction of the surrogate
condition that increasing $\beta$ moves the truncation to lower dimensions.

\begin{table}[H]
\caption{Exp-ConvV-HiBeta (single seed 42, exploratory): AU of the MNIST
Conv-VAE under strengthened $\beta_{\max}$ (activation rate in
parentheses).}
\label{tab:hibeta}
\small
\begin{tabular}{rccc}
...
\caption{Exp-ConvV-HiBeta (single seed, exploratory): increasing
...
side TwoNN converges to $\approx 9.4$ for $m_{\mathrm{ref}} \geq 32$, and an
AU slowdown region of the same order as $\hatdID \approx 9$--10 was
observed (same tendency as MNIST).
Multi-seed replication is future work (the multi-seed Step-1 values come
from Exp-Real-Geom; Section~\ref{sec:exp_real_geom}).
...
\begin{table}[H]
\caption{Experiment overview. All main-text results use 3 seeds
(42, 123, 777); appendix experiments marked ``single'' use seed 42 only.}
\label{tab:exp_list}
\small
...
Exp-ConvV-HiBeta & MNIST / Conv-VAE ($\beta_{\max}$ up) & 150 & single & App.~\ref{app:exploratory} \\
Exp-NatImg-Sweep & CIFAR-10 / Conv-VAE & 300 & partial & App.~\ref{app:exploratory} \\
Exp-Fashion & F-MNIST / AE, VAE & 100 & single & App.~\ref{app:exploratory} \\
