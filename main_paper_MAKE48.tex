...
autoencoders (VAEs) is usually set by convention or trial and error.
We propose a \textbf{conditional, diagnostic protocol} that uses an
intrinsic-dimension estimate $\hatdID$ of the input data as a
\textbf{search-window reference} for $m$: AEs are searched within
$m \in [\hatdID, 2\hatdID]$, whereas VAEs receive a generous candidate
$m \geq 2\hatdID$ whose surplus coordinates are verified after training
using active units (AU) and reconstruction error.
The protocol is motivated by a topological lower bound for zero-error
continuous reconstruction and by a linear-Gaussian rate--distortion
surrogate with activation condition $\lambda_j > \beta\tau^2$.
Under the tested settings, the protocol produced reproducible diagnostics
on synthetic manifolds (three to five seeds) and on fully connected VAEs
for MNIST and Fashion-MNIST (three seeds each): MNIST passes, Fashion-MNIST
is diagnosed as an over-pruning boundary case, and for the specific MNIST
grid and evaluation protocol used here the ID-guided window reduces
training runs by roughly 60--80\% while containing
the best observed linear-probe accuracy.
The method is conditional: intrinsic-dimension
estimates were stable only through reference representations
with $m \gg \hatdID$, latent-space TwoNN estimates on natural images
increased beyond a provisional consistency region, and the tested Conv-VAE
schedule ($\beta_{\max} = 4$) showed no truncation, with larger
$\beta_{\max}$ reducing the activation ratio without order consistency.
We therefore position $\hatdID$ as a search-window reference and AU as a
\textbf{model- and training-dependent diagnostic}---not as general
...
coordinates in active use under KL regularization---serve, under a suitable
$\beta$ and architecture, as a post-training diagnostic of whether the
surplus dimensions were indeed truncated.}
\end{quote}
...
(Proposition~\ref{thm:linear_vae}) motivates the expectation that even if $m$ is set above
...
candidate dimension followed by AU-based verification may be useful when
KL-induced truncation is effective}---a condition that is itself a
hypothesis to be checked by the Step-3 decision rule.
...
protocol.
A preliminary analysis of the relationship between manifold structure and
VAE latent-space capacity was presented by the authors
in~\cite{obata2025manifold}; the present paper substantially extends that
study with the full estimate--set--verify protocol, multi-seed validation,
and quantified applicability boundaries.
...
In our own prior work, we examined the role of latent variables in
encoder--decoder models on image datasets~\cite{okamoto2023latent}, studied
representation learning for multimodal processing~\cite{jinno2023multimodal},
and visualized and interpreted the latent spaces of deep
models~\cite{dai2024latent}; the present paper builds on these observations
by turning them into an explicit bottleneck-selection protocol.
...
In a complementary direction, Dai and Jin'no~\cite{dai2026island} analyzed
the geometric structure that emerges in deep classifiers whose
representation dimension is deficient relative to the data---the
degenerate counterpart of the over-provisioned bottlenecks studied here.
...
measures the number of coordinates in active use under KL regularization.
...
``mechanism that can truncate an over-generous $m$'' (whether truncation
has taken place is diagnosed in Step 3), with the water-filling activation
...
Section~\ref{sec:intro_dim}: what the bottleneck truly requires is
...
strategy}
(whether truncation has taken place is checked by V1 in Step 3).
...
\begin{align}
  ...
  \label{eq:rule_v2}
\end{align}
The verification \textbf{passes} when both hold (evidence that AU lies
within the operational range $[\hatdID, 3\hatdID]$ defined relative to the
Step-1 reference);
...
compressed and lies within the operational range defined relative to the
Step-1 reference under the
...
\midrule
AU $= m$ for all tested $m$ (no truncation) & Consistent with $\beta$ being too small relative to decoder capacity, or with the effective truncation threshold lying beyond the tested range & Increase $\beta$ or restrict decoder capacity; see Conv-VAE boundary (Section~\ref{sec:exp_boundary}) \\
...
\addlinespace
Conv-VAE on MNIST & Stable (inflates while AU $= m$) & \textbf{Fail} at $\beta_{\max} = 4$ (V1); $\beta_{\max} \geq 10$ restores V1 but not V2 (3 seeds) & Conditional applicability when sufficient truncation can be induced under the chosen architecture and training schedule; order consistency remains open \\
\addlinespace
...
\subsection{Surplus-Dimension Truncation in a Water-Filling Surrogate}\label{sec:theory_wf}
...
the surrogate---this motivates the conservative Step-2 strategy of using a
generous $m$ and verifying the resulting active-unit count, which can be
useful when KL-induced truncation is effective.
...
observed empirically for Conv-VAEs and natural images
...
the rate--distortion approximation vanishes on the truncated coordinates of
the globally optimized linear-Gaussian solution and is assumed negligible
on its active coordinates (Appendix~\ref{app:proofs}), and during nonlinear
optimization it is
...
This argument is not a guarantee of unbiasedness but a heuristic
description of conditions that may support conditional stability
...
Proposition~\ref{thm:linear_vae}: under this synthetic setting, the
observed behavior is consistent with the surrogate expectation that
\textit{a VAE can truncate the surplus dimensions of a generous $m$, with
AU reporting the result}.
...
agreement of the two estimators---are met: the pre-specified rounded-TwoNN
rule yields the \textbf{operational primary reference $\hatdID = 10$ for
MNIST}, while the MLE values of $11.3$--$11.9$ provide an
order-of-magnitude cross-check
(Section~\ref{sec:protocol_s1}).
...
(Section~\ref{sec:exp_mnist_ci}), and the three-seed candidate-VAE latent
cross-check (diagnostic only; not used to set $\hatdID$) is reported in
...
Table~\ref{tab:failure} arise in practice.
...
interpretation, this pattern is consistent with an effective threshold
$d^*_{\mathrm{eff}}(\beta)$---defined here only as an interpretive quantity
for the nonlinear experiment, not directly estimated from it---lying beyond
the tested range
($d^*_{\mathrm{eff}}(\beta) > 512$)---and \textbf{the near-diagonal
AU--$m$ relationship flags the diagnostic condition ``no substantial
truncation observed
within the tested range,'' which is consistent with $\beta$ being too small
relative to the decoder capacity} (first row of Table~\ref{tab:failure}).
...
moves the truncation to lower dimensions, suggesting that architecture- and
data-dependent strengthening of $\beta$ may be required before AU-based
verification can be informative for Conv-VAEs.
...
$\beta_{\max} = 4$ keeps AU $= m$ (no truncation) up to $m = 64$ and
remains highly active (94\%) even at $m = 256$.
The AU pattern indicates that no substantial truncation is observed within
the tested range, which is consistent with insufficient $\beta$ relative to
the decoder capacity (first row of Table~\ref{tab:failure}).
...
When TwoNN is applied globally to a dataset with heterogeneous local
geometry, such inhomogeneity can complicate the interpretation of
neighbor-distance ratios and tends to degrade the preservation of neighbor
ranks (the
...
Under the surrogate interpretation, this pattern would correspond to an
effective active-direction threshold larger than the tested $m$-range
($\dstar \gg m$), resulting in little or no observable truncation; in a
latent space where
...
of performance---how many dimensions are in use, directly
...
$m_{\mathrm{ref}} \approx 2\hatdID$ on natural images, and for the tested
Conv-VAE schedule with $\beta_{\max} = 4$
sufficient truncation cannot be assumed (a V1
violation with AU $= m$); strengthening $\beta_{\max}$ induces truncation
but order consistency with $\hatdID$ (V2) remains unmet (3 seeds).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\supplementary{The following supporting information is submitted with this
manuscript as an anonymized supplementary archive for peer review: source
code for all experiments listed in Table~\ref{tab:exp_list}, model and
metric implementations, configuration files, data-subset indices, random
seeds, seed-specific training logs (JSON), figure- and table-generation
scripts, and environment information. Upon acceptance, these materials will
be released in a public GitHub repository archived with a DOI (Zenodo), and
the repository URL and DOI will be added here.}

\authorcontributions{Conceptualization, C.O., M.D. and K.J.;
methodology, C.O. and K.J.;
software, C.O. and M.D.;
validation, C.O., M.D. and K.J.;
formal analysis, C.O. and K.J.;
investigation, C.O., M.D. and K.J.;
writing---original draft preparation, C.O. and K.J.;
writing---review and editing, C.O., M.D. and K.J.;
supervision, K.J.;
project administration, K.J.
All authors have read and agreed to the published version of the manuscript.}

\funding{This research was funded by JSPS KAKENHI Grant Numbers JP24K15115
...
Communication (RIEC), Tohoku University, and by a
...
\acknowledgments{During the preparation of this study, the authors used
Claude Code (Anthropic; Claude Fable 5 model) for the following purposes:
to assist in generating and refactoring the experimental program code,
because the experimental pipeline required a large number of structurally
similar training and analysis scripts whose consistency benefits from
automated generation; and to check English grammar and proofread the
manuscript, because none of the authors is a native English speaker. The
authors have reviewed, tested, and edited all outputs and take full
...
(i) For the particular globally optimized linear-Gaussian solution
considered here, the approximation is \textbf{exact on truncated
coordinates}: there $q_\phi(z_j|x) = p(z_j)$, so those coordinates
contribute nothing to $\mathrm{KL}(q_\phi(z)\,\|\,p(z))$.
On \textbf{active coordinates}, the aggregated-posterior mismatch is
\textbf{assumed to be negligible} at this optimum: the marginal variance of
the posterior mean plus the posterior variance equals the prior variance (a
property of PPCA-type
optima~\cite{tippingbishop1999ppca,lucas2019elbo}), and this
variance-matching property motivates the assumption but does not by itself
...
In \textit{Proceedings of the 28th International Conference on Machine
Learning (ICML)}, Bellevue, WA, USA, 28 June--2 July 2011; pp.~833--840.

\bibitem{okamoto2023latent}
Okamoto, S.; Jin'no, K.
A study on the role of latent variables in the encoder--decoder model using
image datasets.
\textit{Nonlinear Theory Appl.\ IEICE} \textbf{2023}, \textit{14},
652--676. https://doi.org/10.1587/nolta.14.652.

\bibitem{jinno2023multimodal}
Jin'no, K.; Izumi, M.; Okamoto, S.; Dai, M.; Takahashi, C.; Inami, T.
Fundamental considerations on representation learning for multimodal
processing.
In \textit{Human Interface and the Management of Information (HCII 2023)};
LNCS 14015; Springer: Cham, Switzerland, 2023; pp. 389--399.
https://doi.org/10.1007/978-3-031-35132-7\_29.

\bibitem{dai2024latent}
Dai, M.; Jin'no, K.
Visualization and interpretation of latent space in deep learning.
In \textit{HCI International 2024 (HCII 2024)}; LNCS; Springer: Cham,
Switzerland, 2024; pp. 14--23.
https://doi.org/10.1007/978-3-031-60114-9\_2.
...
\bibitem{dai2026island}
Dai, M.; Jin'no, K.
Analysis of the island class structure in dimension-deficient deep
classifiers.
\textit{Nonlinear Theory Appl.\ IEICE} \textbf{2026}, \textit{17},
1187--1212. https://doi.org/10.1587/nolta.17.1187.
...
Learning (ICML)}, \textit{PMLR} Vol.~202, 2023; pp.~25834--25857.
...
Learning (ICML)}, \textit{PMLR} Vol.~119, 2020; pp.~5166--5176.
