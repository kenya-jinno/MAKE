...
%  MDPI MAKE submission — main_paper_MAKE35.tex
%  English version of main_paper_NN35.tex (Japanese draft, version 35)
%  Version 35: complete restructuring in response to Review_codex34.txt (Major 1--8)
%    - central claim unified: choosing the bottleneck dimension from the intrinsic
%      (manifold) dimension of the input data
%    - ID-guided selection protocol (estimate -> set -> verify by AU) presented as
%      the main contribution (Major 1)
%    - m_knee demoted from a named diagnostic output to an exploratory appendix
%      analysis (Major 2)
%    - theorem renamed "surrogate activation condition" (Proposition); tau^2
%      definition and the KL/mutual-information approximation stated in the claim
%      body (Major 3)
%    - stability-vs-validity distinction placed early, as part of the method (Major 4)
%    - main text restricted to multi-seed experiments; single-seed material moved
%      to the appendix and labeled exploratory (Major 5)
%    - relation to conventional model-selection criteria added to Discussion (Major 6)
%    - length reduced to roughly half of version 34 (Major 7)
%  Compiled with official mdpi.cls
%  Compile: pdflatex main_paper_MAKE35.tex  (x3 for cross-references)
...
Under the manifold hypothesis, this paper proposes an ID-guided selection
protocol---a three-step procedure of estimation, setting, and
verification---in which \textbf{$m$ is chosen from an estimate $\hatdID$ of
the intrinsic (manifold) dimension of the input data, and the choice is
verified after training through the active units (AU) of a VAE}.
Theoretically, the protocol is grounded in (i) the topological lower bound
$m \geq \demb \geq \dID$ required for self-intersection-free reconstruction,
and (ii) a water-filling-type surrogate activation condition
$\lambda_j > \beta\tau^2$ in the linear-Gaussian setting
($\lambda_j$: eigenvalues of the input covariance; $\tau^2$: observation
noise variance), which together yield the selection principle: set $m$
comfortably above $\hatdID$, let the KL regularization of the VAE
automatically truncate the surplus dimensions, and read the truncation
result from the AU count.
Experimentally, the protocol returns the correct dimension on synthetic
manifolds with known ground truth (3 seeds; Swiss Roll:
$\AUsat = \dtrue = 2$; Torus: $\AUsat = \demb = 3$), and on MNIST (3 seeds)
the estimate $\hatdID = 10.03 \pm 0.12$ (TwoNN) is consistent with the
slowdown region of the AU growth.
We further quantify the limits of applicability: intrinsic-dimension
estimation is stable only through reference representations with
$m \gg \hatdID$ (four-model comparison on MNIST and Fashion-MNIST, 3 seeds),
the estimate inflates beyond $m \approx 2\hatdID$ on natural images, and for
Conv-VAEs the AU truncation fails when $\beta$ is too small
($\dstar > 512$ at $\beta = 4$).
Our validation is centered on mid-scale data and fully connected VAEs;
full application to large-scale natural images and independent validation of
change-point detection remain future work.}
...
\begin{quote}
\textbf{The bottleneck dimension $m$ can be selected from an estimate
$\hatdID$ of the intrinsic dimension of the input data as
``$m \gtrsim \hatdID$ (including a topological margin),'' and the active
units (AU) of a VAE---the number of coordinates actually used under KL
regularization---serve as a post-training verification signal for this
selection.}
\end{quote}
...
The latter guarantees, in an idealized setting, that even if $m$ is set
above the required dimension, the KL regularization of the VAE
automatically truncates the surplus dimensions (posterior collapse) and the
AU count reports the effective number of used dimensions---in other words,
\textbf{whereas for an AE the choice of $m$ itself fixes the representation
dimension, for a VAE the safe-side operation ``generous $m$ + AU read-out''
becomes possible}.
...
\begin{enumerate}
  \item \textbf{An ID-guided bottleneck-dimension selection protocol}
        ...
        an executable procedure consisting of
        ...
        margin; VAE: a generous setting $m \gtrsim 2\hatdID$), and
        (S3) post-training verification by AU and reconstruction error,
        together with decision rules, failure modes, and reporting
        requirements.
  \item \textbf{Theoretical grounding of the selection principle}
        ...
        that ``a VAE self-repairs an excessive $m$, but the truncation
        threshold depends on $\beta$.''
  \item \textbf{Multi-seed validation and quantified applicability
        boundaries} (Section~\ref{sec:exp}):
        we confirm that the protocol returns the correct dimension on
        synthetic manifolds with known ground truth (3 seeds) and show on
        MNIST (3 seeds) that $\hatdID \approx 10$--12 is consistent with the
        AU slowdown region.
...
``safety device that automatically repairs an over-generous $m$,'' with the
water-filling activation condition as its quantitative rationale
...
\begin{tabular}{c}
  ...
  \fbox{\parbox{0.78\linewidth}{\centering
    ...
    VAE: set generously, $m \gtrsim 2\hatdID$ (KL truncates surplus dimensions)
  }} \\[3pt]
  $\Big\downarrow$ \\[1pt]
  \fbox{\parbox{0.78\linewidth}{\centering
    \textbf{Step 3: Post-training verification} \\[1pt]
    VAE: does AU$(m)$ saturate/slow down near $\hatdID$? (multi-seed) \\
    AE: does the rapid MSE improvement stop around
    $m \approx \hatdID$--$2\hatdID$? \\
    On mismatch: diagnose with the failure-mode
...
\textbf{For VAEs}: by the implication of
Proposition~\ref{thm:linear_vae}, the KL regularization truncates surplus
dimensions even when $m$ exceeds the required dimension, so
\textbf{setting $m \gtrsim 2\hatdID$ generously is the safe-side
operation}.
...
seeds):
\begin{itemize}
  \item \textbf{VAE: AU verification}.
        If AU$(m)$ saturates (or clearly slows down) below $m$ and its level
        is of the same order as $\hatdID$, this is evidence that a
        compression reflecting the manifold dimension of the data has taken
        place.
        Simple manifolds show sharp saturation
        ($\AUsat = \dtrue$ or $\demb$), while real data show a gradual
        slowdown (Section~\ref{sec:exp_mnist}).
  \item \textbf{AE: MSE verification}.
...
\midrule
Synthetic manifolds ($\dtrue \leq 3$, 3 seeds) & Direct input-space estimate; matches $\dtrue$ & Sharp saturation at $\dtrue$ / $\demb$ ($\sigma = 0$) & Fully applicable (ground-truth validated) \\
\addlinespace
FC-VAE on MNIST / Fashion-MNIST (3 seeds) & Stable: $10.03 \pm 0.12$ / $\approx 9$ & Gradual slowdown consistent with $\hatdID$ & Applicable \\
\addlinespace
Conv-VAE on MNIST & Stable (inflates while AU $= m$) & Requires $\beta_{\max} \geq 10$; at $\beta_{\max} = 4$, $\dstar > 512$ & Applicable only with strong $\beta$ (exploratory) \\
\addlinespace
Natural images (CIFAR-10 / SVHN, 3 seeds) & Consistent with Pope et al.~\cite{pope2021intrinsic} at $m_{\mathrm{ref}} \approx 2\hatdID$; inflates beyond & No AU saturation within tested range & Step 1 usable with care; Step 3 open problem \\
\bottomrule
\end{tabular}
\end{table}
...
(i) Surplus dimensions with $m > \dstar$ are truncated at the optimum---this
is the rationale for Step 2, ``a generous $m$ is safe for VAEs.''
...
and Fashion-MNIST (Sections~\ref{sec:exp_mnist}
and~\ref{sec:exp_real_geom})?
...
experiment code, configurations, random seeds, training logs (JSON), and
regeneration scripts for the main tables and figures are attached as
supplementary material for review; upon acceptance they will be deposited in
...
automatic truncation)---consistent with implications (i) and (iii) of
Proposition~\ref{thm:linear_vae}, confirming on ground truth the core
mechanism of the protocol: \textit{the VAE self-repairs a generous $m$, and
AU reports the result}.
...
The correspondence $\AUsat = \demb$ on the Torus and the M\"obius band is a
conditional empirical observation depending on preprocessing
(standardization) and sufficient convergence, and we claim no theoretical
necessity (single-seed topological-manifold experiments are in
...
($10.03 \pm 0.12$ over 3 seeds at $m_{\mathrm{ref}} = 64$), and MLE
...
increasing $m_{\mathrm{ref}}$ (300 epochs, single seed 42 for the sweep; the
3-seed value at $m_{\mathrm{ref}} = 64$ is $10.03 \pm 0.12$).}
\label{tab:mnist_s1}
\small
\begin{tabular}{rcccc}
\toprule
...
for the AE and a generous $m \gtrsim 20$ for the VAE.
...
$m \approx 16$--20 (Table~\ref{tab:mnist_s1}), consistent with the candidate
range $[10, 24]$.
...
(3 seeds, 300 epochs). Left: per-seed AU$(m)$; right: mean$\pm$std band. AU
values are highly reproducible ($\sigma \leq 0.5$).}
...
Appendix~\ref{app:exploratory}).
The qualitative prediction---increasing $\beta$ moves the truncation to
lower dimensions---is borne out, identifying ``strengthening $\beta$
according to data and architecture'' as a necessary condition for applying
the protocol to Conv-VAEs.
...
Indeed, on MNIST the MSE verification, the AU verification, and $\hatdID$
all supported the same candidate range $[10, 24]$.
On the other hand, this paper does not perform a systematic comparison with
downstream-task accuracy or disentanglement metrics; verifying whether
ID-guided selection is also optimal in terms of downstream performance
remains future work.
...
(Table~\ref{tab:real_geom}), and (iv) consistency with reported values on
CIFAR-10---but this is not a direct proof of unbiasedness.
...
verification.
Remaining tasks include systematic comparison with estimators based on
different principles (DANCo~\cite{ceruti2014danco}, ESS) and reporting
...
order as $\hatdID$?''---the protocol is designed to rely only on this robust
part.
...
\begin{itemize}
  ...
  \item \textbf{$\beta$ dependence}: the AU read-out depends on $\beta$
...
from the manifold dimension of the input data,'' this paper gave an
affirmative and operational answer in the form of an
\textbf{ID-guided selection protocol}:
(S1) intrinsic-dimension estimation through reference representations with
$m_{\mathrm{ref}} \gg \hatdID$,
(S2) setting $m$ as $\hatdID$ plus a topological margin (generous $m$ with
KL auto-truncation for VAEs), and
(S3) post-training verification by AU and MSE.
Theoretically, the topological lower bound and the water-filling surrogate
activation condition $\lambda_j > \beta\tau^2$ ground each protocol step;
experimentally, the protocol was confirmed on synthetic manifolds with known
ground truth ($\AUsat = \dtrue$ or $\demb$; 3 seeds, $\sigma = 0$) and on
MNIST ($\hatdID = 10.03 \pm 0.12$ consistent with the AU slowdown region;
3 seeds).
...
insufficient $\beta$ is detected as the signal AU $= m$ with truncation
restored by strengthening $\beta$.
The main future tasks are stable identification of AU verification on
natural images, nested validation of change-point detection, and
cross-validation of Step 1 with intrinsic-dimension estimators based on
