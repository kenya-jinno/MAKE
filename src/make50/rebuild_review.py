"""Reaggregate immutable MAKE49 logs for MAKE50; no model training.

Run from the repository root with Python, NumPy, matplotlib and scikit-learn.
All analysis choices added in MAKE50 are retrospective and labelled as such.
"""
...
OUT = ROOT / 'results/make50'
FIG = ROOT / 'results/figures/MAKE50'
...
NAMES = {'B1_full_mse': 'Grid (MSE)', 'B2_full_elbo': 'Grid (ELBO proxy)',
 ...
 'proposed': 'ID-guided + Q', 'ascending_scan_Q': 'Ascending + Q',
...
MAIN = ['B1_full_mse','B6a_fondue','B6a_plus_Q','B7_geco_l0',
...
for block, datasets in enumerate([DS[:2],DS[2:]],1):
    ...
    tex_table(f'main{block}',
        ...
        '$m_{\\rm final}$ is the width of the evaluated ordinary VAE, including grid rounding for the pruning adaptations. '
        'MSE columns are multiplied by $10^3$; $Q$ counts validation target attainment. '
        '$C_{\\rm train}$ is the recorded training-time account including the quality anchor, '
        'not complete wall-clock cost (Section~\\ref{sec:cost}).',f'tab:main{block}',
        'lccccc',r'Method & $m_{\rm final}$ & Val. MSE & Test MSE & $Q$ & $C_{\rm train}$ (s)',lines[:-1], 'footnotesize')
...
lines=[]
for meth in SPAN_METHODS+['B7_geco_l0','B8_ard_vae']:
    vals=[]
    for ds in DS:
...
tex_table('span','Range of seed-mean returned dimensions across $\\beta\\in\\{0.25,0.5,1,2,4\\}$. GECO varies the anchor-derived constraint; it is not a direct $\\beta$ intervention. ARD selection was fixed at an implicit weight of one and is excluded. Fixed 32 is a design constant; fallback spans are not ID robustness. Gaussian likelihood except Bernoulli dSprites.','tab:span','lcccc',r'Method & MNIST & Fashion & dSprites & CIFAR-10',lines)
...
for ds in DS:
    for b in BETAS:
        ...
        lines.append(f"{DN[ds]} & {b:g} & {fe['epochs']} & {fe['search_seconds']:.2f} & {'yes' if fe['stable'] else 'no'} \\\\")
...
fig,axes=plt.subplots(2,2,figsize=(8,6),layout='constrained')
show=['proposed','ascending_scan_Q','B6a_fondue','B7_geco_l0']
for ax,ds in zip(axes.flat,DS):
    ...
    ax.set(xscale='log',xlabel=r'$\beta$ (GECO: anchor context)',ylabel='Returned dimension',title=DN[ds]);ax.set_xticks(BETAS,labels=['.25','.5','1','2','4']);ax.grid(alpha=.2)
