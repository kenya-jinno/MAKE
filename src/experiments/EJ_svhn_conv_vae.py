"""
実験 EJ: SVHN Conv-VAE with KL Cyclic Annealing
目的: 自然画像（SVHN）における多指標フレームワークの汎化性検証
     CIFAR-10との比較によるデータセット依存性の確認
Architecture: Conv-VAE (32x32x3 入力, EIと同一)
m: [16, 32, 64, 128, 256]
...
def load_svhn(n_train=20000, n_test=3000):
...
def main():
    ...
    for seed in seeds:
        ...
        with open('results/tables/EJ_svhn_conv_vae.json', 'w') as f:
            json.dump(all_results, f, indent=2)

    # ── Summary ───────────────────────────────────────────────────────────────
    print("\n=== SVHN Conv-VAE Summary ===")
    ...
    ax.set_title('SVHN Conv-VAE AU (Cyclic Annealing, 3 seeds)')
    ...
    ax2.set_title('SVHN: AU & TwoNN ID vs $m$ (Cyclic Annealing)')
    ax2.set_xscale('log')
    ax2.tick_params(axis='y', labelcolor='b'); ax2r.tick_params(axis='y', labelcolor='r')
    h1, l1 = ax2.get_legend_handles_labels()
    h2, l2 = ax2r.get_legend_handles_labels()
    ax2.legend(h1+h2, l1+l2, fontsize=7); ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('results/figures/EJ_svhn_conv_vae.png', dpi=150, bbox_inches='tight')
    plt.savefig('results/figures/EJ_svhn_conv_vae.pdf', bbox_inches='tight')
    plt.close()
    print("Saved: results/figures/EJ_svhn_conv_vae.{png,pdf}")
