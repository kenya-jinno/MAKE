"""
E4_300ep_multiseed 図の v43 再生成（MAKE投稿論文_修正指摘と改善案.md §11.3 対応）
- 参照線ラベルを "Rounded Step-1 reference d̂_ID = 10" に変更
  （dID = 10 が真の内在次元と誤読されるのを防ぐ）
- 線種・マーカー・候補帯は従来版（白黒印刷対応）を維持
- 旧図は残し，新図は *_v43 として保存
...
DHAT = 10
REF_LABEL = r'Rounded Step-1 reference $\hat{d}_{\mathrm{ID}} = 10$'
...
ax2.axhline(DHAT, color='purple', ls=':', lw=2, label=REF_LABEL)
...
plt.tight_layout()
plt.savefig('results/figures/E4_300ep_multiseed_v43.png', dpi=300, bbox_inches='tight')
plt.savefig('results/figures/E4_300ep_multiseed_v43.pdf', bbox_inches='tight')
print('Saved: results/figures/E4_300ep_multiseed_v43.{png,pdf}')
