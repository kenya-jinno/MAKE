"""Assemble the complete Japanese MAKE52 translation without changing results."""
...
SRC=ROOT/'src/nn52';OUT=ROOT/'results/nn52';OUT.mkdir(exist_ok=True)
english=(ROOT/'main_paper_MAKE52.tex').read_text()
template=(ROOT/'src/make52/manuscript_body.tex').read_text().strip()
template=re.sub(r'@@section:(\w+)@@',lambda m:(ROOT/'src/make52'/(m[1]+'.tex')).read_text().strip(),template)
...
assert len(original)==154 and set(blocks)==set(range(154))
body='\n\n'.join(original[k] if blocks[k]=='@@keep@@' else blocks[k] for k in range(154))
...
body=re.sub(r'(\\includegraphics(?:\[[^\]]*\])?\{)results/figures/MAKE(?:51|52)/',
            lambda m:m[1]+'results/figures/NN52/',body)
...
% MAKE52の全訳。数式・数値・引用・図表番号は英語版に対応する。
% 再コンパイル：lualatex main_paper_NN52.tex（参照確定まで2回）
...
ID由来の開始点と中点は全100条件で同じ次元数を返し，ID採用条件では前者が平均8.84候補を要するのに対し，後者は5.84候補である。
判定を持たない中点方策の2件の誤りは，ID方策が昇順fallbackを用いる条件で生じる。
事後的な係数・閾値の10,000設定では，局所探索の155出力が品質を満たす最小次元を逃す。
各データセット3シードの追加実験で，ARMによる剪定と厳密ヤコビアン関連度読み取りを評価する。
native再構成，次元転用，制約充足は異なる結果を与え，ローカルARM実装ではdSpritesに問題が残る。
別のMNIST・1シード実験ではMonte Carlo ELBOによる容量選択を比較する。
活性ユニットの予測は，天井に近い課題で追加の利益を確立しない。
これらの結果は，探索規則，実装上の選択，評価の定義が測定される品質と費用にどう影響するかを示す一方，普遍的な次元選択保証を与えない。
\end{abstract}
\noindent\textbf{キーワード：}変分自己符号化器，ボトルネック次元，内在次元，活性ユニット，モデル選択，再構成品質，再現性
\par\smallskip
{\small 本稿は英語版MAKE52の日本語版であり，節・図表・式番号は英語版に対応する。参考文献の書誌情報は原文表記を保持する。}
\par\medskip
'''
...
\paragraph{日本語版の生成}
日本語本文の組立ては\path{src/nn52/build_japanese.py}，
図の表示文字の日本語化は\path{src/nn52/render_figures.py}による。
日本語TeXのコンパイルには，プロジェクト直下で
\texttt{lualatex main\_paper\_NN52.tex}を参照が確定するまで繰り返す。
日本語図は\path{results/figures/NN52/}に保存する。
'''
final=preamble+body+translation_note+'\n\n'+bib+'\n\\end{document}\n'
(ROOT/'main_paper_NN52.tex').write_text(final)
(OUT/'translation_verification.json').write_text(json.dumps({
 'source':'main_paper_MAKE52.tex','source_sha256':hashlib.sha256(english.encode()).hexdigest(),
...
print(f'Wrote main_paper_NN52.tex; {len(blocks)} blocks, {len(maths)} equations and {len(table_checks)} tables checked.')
