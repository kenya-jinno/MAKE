%% main_paper_NN33.tex
%% MDPI MAKE 投稿用日本語草稿
%% AE/VAEにおける有効潜在次元の診断的解釈（版33）
%% 版33の変更点：Review_codex32.txt（Major 1--8, Minor 1--10）への対応と全体の分量削減
%% コンパイル: platex main_paper_NN33.tex && dvipdfmx main_paper_NN33.dvi
...
%% ===================================================================
\begin{abstract}
本論文では，AE/VAE が学習した潜在表現において実際に利用される有効潜在次元を診断的に解釈するため，
Active Unit（AU）Dynamics，内在次元（ID）推定（TwoNN/MLE），MSE エルボー，
および PCA を統合した多指標診断フレームワークを提案する．
本フレームワークは有効潜在次元を単一の数値として一意に決定するのではなく，
AU 値を KL 活性座標数の頑健な直接観測指標，
TwoNN/MLE ID を数値的に安定な補助的参照推定量
（潜在空間の安定な参照値であり，真の内在次元に対する不偏性は保証されない），
AU 鈍化点$\mknee$を TwoNN 誘導型の探索的変化点として明確に区別する．
理論面では，線形ガウス $\beta$-VAE の rate-distortion 近似という解釈的サロゲートのもとで，
データ共分散の第$j$固有方向が活性化される条件$\lambda_j > \beta\tau^2$を導出する
（学習済み非線形 VAE に関する定理ではない）．
実験では，MNIST の全結合 VAE において AU 値と TwoNN ID が複数シードにわたり
高い再現性を示す一方，$\mknee$は疎グリッドでは大きく変動すること，
および TwoNN 推定値近傍の密グリッド探索（TwoNN 誘導型局所精密化）により
$\mknee = 10.3 \pm 0.9$ に安定化することを示す——
ただしこの安定化の検証範囲は MNIST FC-VAE・$\beta = 4$・密グリッド・3シードに限られる．
さらに Fashion-MNIST，Conv-VAE，CIFAR-10/SVHN 実験を通じて，
本フレームワークが機能する条件と AU/$\mknee$ 診断が困難となる境界条件を定量化する．
\end{abstract}
...
$\mknee$ は TwoNN 誘導型の探索的変化点であり，主要な有効次元の読み取りは
頑健指標（AU 値）と数値安定な補助的参照推定量（TwoNN ID）が担う
（各指標の分類と信頼性は表\ref{tab:indicator_class}にまとめる）．

本論文の\textbf{貢献は以下の3点}である：
\begin{enumerate}
  \item \textbf{多指標診断フレームワークの提案}：
        本研究の主貢献は，$\mknee$そのものの提案ではなく，
        AU 値，TwoNN/MLE ID，MSE エルボー，PCA を役割の異なる診断指標として整理し，
        AE/VAE の有効潜在次元を多面的に解釈する枠組みを与える点にある．
        ...
        高い再現性を確認し，TwoNN 推定値近傍の密グリッド・独立初期化・複数シード条件での
        $\mknee = 10.3 \pm 0.9$（Exp-Mnist-Dense-300）を主要な安定化結果とする（§\ref{sec:exp_el}）．
        ただし，探索領域自体を TwoNN 推定値から選択しているため，
        この結果は「TwoNN 誘導型の局所精密化」であり，TwoNN と独立な検証ではない．
        検証範囲は MNIST FC-VAE・$\beta = 4$・3シードに限られる．
...
AU とデータの分散構造（有効次元）との関連を定性的に論じた．
...
誤診断を避けるため，本稿では以下の5つの対象量を明示的に区別する：
\begin{enumerate}[(i)]
  \item \textbf{データ多様体の内在次元}$d_\mathrm{ID}^\mathrm{data}$：
        入力分布そのものの局所自由度．合成多様体では既知（$\dtrue$），
        実データでは入力空間への TwoNN/MLE 適用で近似される．
  \item \textbf{潜在表現の内在次元}$d_\mathrm{ID}^\mathrm{latent}$：
        参照 AE の潜在表現に TwoNN/MLE を適用して得られる推定値（本稿の$\hat{d}_\mathrm{ID}$）．
        数値的には安定だが，参照 AE の埋め込みの歪みを介するため，
        （i）と一致する保証はない——安定な\textbf{潜在空間側の参照値}である（§\ref{sec:disc_twonn_bias}）．
  \item \textbf{KL 活性座標数}（AU 値）：
        VAE が実際に符号化に用いている潜在座標の数．
        $\beta$・アーキテクチャ・閾値$\delta$・学習条件に依存する\textbf{座標利用の観測量}であり，
        内在次元の推定量ではない．$\mknee$は AU$(m)$曲線の変化点，
        $\AUsat$はグリッド上の最大 AU 値である．
  \item \textbf{再構成に必要なボトルネック次元}：
        再構成誤差が許容水準に達する最小の$m$（MSE エルボーが対応）．
  \item \textbf{診断上の設計候補次元}$\dzopt$：
        上記（i）〜（iv）の複数指標を照合して設計者が選択する候補値．
        一意な最適解ではない．
\end{enumerate}
本稿の主張はこの区別の上に述べられる：
...
\subsection{Stable Identification の判定基準}\label{sec:natimg_stable_id}

新しいデータセット・アーキテクチャで「AU/$\mknee$ を安定同定できた」と主張するための
判定基準を，実験に先立ってここで定義する．
...
理論的に導出された普遍定数ではなく，\textbf{著者らが定めた operational criteria}である．
...
（PyTorch 2.0，CUDA；CUDA 不可時は CPU にフォールバック）で実行した．
...
実験コード・設定・生ログ（JSON）は査読時に補助資料として提供する．
...
\begin{table}[H]
\centering
\caption{Summary of experiment groups. Detailed per-experiment configurations (epochs, $\beta$ schedule, $m$ grid, seeds, sample size, role) are listed in Table~\ref{tab:exp_full} (\ref{app:exp_table}). Categories: main experiments (core framework validation), supplementary experiments (prerequisite verification), boundary experiments (validity range quantification).}
\label{tab:exp_list}
\small
...
\textbf{注意}：Swiss Roll の$\AUsat$は前処理依存性が高い（標準化条件で 2，別条件で 3）ため，
$\AUsat$と$\demb$の対応を支持する主証拠は Torus・Möbius Band（$\AUsat = \demb = 3$）に限定する．
この対応も本研究条件（標準化前処理，$\beta = 4$，十分な収束）に依存する経験的観測であり，
理論的必然性は保証されない（§\ref{sec:discussion}）．
...
グリッド設計と初期化経路に依存する予備的観察にとどまる．
本論文では TwoNN 推定値近傍の密グリッド・独立初期化・複数シード条件による
Exp-Mnist-Dense-300 の$\mknee = 10.3 \pm 0.9$を主要な安定化結果として扱う（§\ref{sec:exp_el}）．
...
\begin{itemize}
  ...
  \item \textbf{グリッド密度因子（主要因の一つ）}：密グリッド（Exp-Mnist-Dense）では$\mknee = 10.3 \pm 0.9$（シード別に 11, 9, 11）—— 疎グリッド$43\pm15$から大きく改善し，推定$\hat{d}_\mathrm{ID} \approx 10$と整合する（ただし密領域を TwoNN 推定値近傍に置いた TwoNN 誘導型精密化としての整合である点に注意）
\end{itemize}

\noindent
\textbf{グリッド密度の効果（命題\ref{thm:mknee_instab}の示唆との比較）}：
命題\ref{thm:mknee_instab}は理想化単調飽和モデルに対する補助的な上界であり，
非線形 VAE の経験的 AU 曲線に直接適用できるものではない．
ただし，推定 ID 近傍（$m = 8$〜16）を間隔 1 で密に探索した結果，
$\mknee \in \{9, 11\}$（3シード）となり，TwoNN ID（$\approx 10$）との乖離が大きく低下した．
これは，命題2が理想化モデルで示したグリッド設計の重要性という示唆と，定性的に一致する結果である．
ただし経験的な AU 曲線は非単調な揺動を示すため（例：表\ref{tab:el_au}で AU$=10$→$10$→$10$→$12$→$11$），
「上界以内に収まった」と主張するよりも，「グリッドを密にすることで経験的$\mknee$の分散が大きく低下した」
という観察として述べる方が正確である．
...
\subsection{Fashion-MNIST への適用（実験 Exp-Fashion，予備的外部確認）}\label{sec:exp_fashion}

Fashion-MNIST（衣類 10 クラス，28×28）は，フレームワークのシード・再現性の主張に対する
予備的な外部確認として位置づける（主実験は MNIST 3シード）．
単一シード（seed 42）での確認であり，MNIST（3シード）と同水準の再現性の主張はしない．
...
これらの差の解釈（データセット間の局所自由度の違い等）には
より厳密な分析が必要であり，本稿では単一シードの観察として報告するにとどめる．
...
実験 Exp-Conv-AE の結果（表\ref{tab:eg}）：
...
\begin{itemize}
  \item \textbf{安定化条件の特定（検証範囲の明示）}：MNIST 多シード疎グリッド実験では$\mknee$が
        32〜64 に大きく変動し，粗いグリッドでは信頼性のある診断補助量として使用できない．
        TwoNN 推定値近傍の密グリッド・独立初期化・複数シード条件（Exp-Mnist-Dense-300）では
        $\mknee = 10.3 \pm 0.9$と TwoNN ID（$\approx 10$）近傍に安定化した．
        ただしこの結果は\textbf{MNIST FC-VAE・$\beta = 4$・TwoNN 誘導型密グリッド・3シード}という
        条件に限定して確認されたものであり，探索領域自体が TwoNN 推定に依存するため
        独立検証ではない（§\ref{sec:exp_el}）．
        したがって$\mknee$の既定の記述は「TwoNN 誘導型の探索的変化点」であり，
        上記条件を明示せずに一般的な利用を推奨するものではない．
...
\begin{enumerate}
  \item \textbf{密なグリッド設計（TwoNN 誘導型局所精密化）}：推定 $\hat{d}_\mathrm{ID}$（TwoNN ID）の
        前後に間隔 1 の密な点を配置する（Exp-Mnist-Dense で実証；§\ref{sec:exp_el}）．
        命題\ref{thm:mknee_instab}より，グリッド間隔がそのまま$\mknee$の最大誤差になる．
        ただし探索領域が TwoNN 推定に依存する点，およびシード因子が残存する点に注意する．
  \item \textbf{アンサンブル$\mknee$}：複数シードでの AU 曲線から各$m^{\mathrm{knee}(s_i)}$を計算し，
        \textbf{メディアン}を採用する：
        \begin{equation}
          m^{\mathrm{knee,ens}} = \mathrm{median}\bigl\{m^{\mathrm{knee}(s_1)}, \ldots, m^{\mathrm{knee}(s_S)}\bigr\}
          \label{eq:mknee_ens}
        \end{equation}
        メディアンは$\mknee$がグリッド端に張り付く外れ値に対して頑健である
        （$S$が偶数の場合は隣接する2値の算術平均とする）．
...
要点のみ述べると，密グリッド条件（Exp-Mnist-Dense）では$\tau$法（$10.3 \pm 0.9$）が
二階差分法（$18.3 \pm 6.8$）より頑健であり，
$\tau$法が不安定になる条件（AU 増加率が境界値付近で複数の$m$にわたり一定）では
二階差分代替手法への切り替えが有効である．
...
\textbf{実験的検証}~~~
この解釈は§\ref{sec:exp_en}・§\ref{sec:exp_hibeta}の結果と整合する：
$\beta_{\max}=4$では$m=512$まで拡張しても飽和プラトーは現れず（$d^*(\beta) > 512$），
$\beta_{\max}$を 10・20 に増大させると$\mknee \approx 128$・$96$が同定された（表\ref{tab:hibeta}）——
$\beta$増大により$d^*(\beta)$が低次元側に移動するという理論的予測どおりである．
すなわち$\beta_{\max}=4$での飽和未観測はフレームワークの適用限界ではなく，
デコーダ容量に対する$\beta$不足に起因し，
Conv-VAE への適用では$\beta$の増強またはデコーダ容量の削減が必要である．
...
\begin{itemize}
  ...
  \item \textbf{$\mknee$の不安定性は第一級の限界である}：
        多シード検証（§\ref{sec:exp_multiseed}）が示すとおり，
        $\mknee$の疎グリッドでの不安定性はエポック延長では解消されず，
        密グリッドによる安定化も MNIST FC-VAE・$\beta=4$・TwoNN 誘導型探索という
        限定された条件でのみ確認されている．
        グリッド密度と多シードは一般に十分な安定化策とは言えず，
...
高い再現性を確認し，TwoNN 推定値近傍の密グリッド・独立初期化・複数シード条件で
$\mknee = 10.3 \pm 0.9$と安定化した（Exp-Mnist-Dense-300；§\ref{sec:exp_el}）．
この安定化は\textbf{TwoNN 誘導型の局所精密化}であり，
検証範囲は MNIST FC-VAE・$\beta = 4$・密グリッド・3シードに限られる——
条件を明示しない一般的な$\mknee$の利用を推奨するものではない．
...
MNIST の4モデル比較（表\ref{tab:real_geom_summary}）および
Fashion-MNIST での定性的確認（\ref{app:twonn_detail}）によって支持した
...
AU plateau-like behavior を本研究条件下で観察した（stable identification 5条件は全て未達；\ref{app:convvae_natimg}）．
