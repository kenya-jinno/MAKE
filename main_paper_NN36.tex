%% main_paper_NN36.tex
%% MDPI MAKE 投稿用日本語草稿
%% 入力データの内在次元に基づく AE/VAE ボトルネック次元の選定（版36）
%% 版36の変更点：Review_codex35.txt（Major 1--8, Minor 1--10）への対応
%%   - 中心主張を「条件付き・診断型プロトコル」として再フレーミング（Major 1）
%%   - Step 3 に再現可能な合否判定規則（V1: AU/m <= 0.9, V2: dhat <= AU <= 3dhat）を導入し，
%%     全実験に判定を明記（Major 2, 8）
%%   - 検証範囲の限定（Fashion 単一シード等）を要旨・貢献に明記（Major 3）
%%   - d_ID / d_emb / AU の区別を合成実験の解釈に再掲（Major 4）
%%   - 新実験 Exp-Boot-CI：サブサンプリング CI・サイズ感度・DANCo/ESS 相互検証（Major 5）
%%   - 新実験 Exp-Downstream：線形プローブ下流精度と探索コスト比較（Major 6）
%%   - 「保証」等の表現をサロゲート予測に弱化（Major 7）
%%   - 要旨短縮，表・図の単一/多シード区別明示，mknee ラベルを図から除去，
%%     ARD/Bayesian PCA への言及追加，補助資料インベントリ明記（Minor 1--10）
%% コンパイル: platex main_paper_NN36.tex && platex main_paper_NN36.tex && dvipdfmx main_paper_NN36.dvi
...
本論文は，多様体仮説のもとで
\textbf{$m$の候補範囲を入力データの内在次元の推定値$\hatdID$から導き
（AE：$m \in [\hatdID, 2\hatdID]$，VAE：$m \geq 2\hatdID$），
学習後に VAE の Active Units（AU）で「余剰次元が実際に遮断されたか」を診断する}，
条件付き・診断型の ID 誘導プロトコル（推定→設定→検証の3ステップ）を提案する．
根拠は，自己交差のない再構成に$m \geq \demb \geq \dID$を要する位相的下界と，
線形ガウスサロゲートの水充填型活性化条件$\lambda_j > \beta\tau^2$である．
真の次元既知の合成多様体と MNIST の全結合 VAE（いずれも3シード）では，
3ステップすべてが再現可能な合否判定規則のもとで合格し
（Swiss Roll：$\AUsat = \dtrue = 2$，Torus：$\AUsat = \demb = 3$；
MNIST：$\hatdID = 10.03 \pm 0.12$と AU 鈍化域が整合），
ID 誘導の候補範囲はフルグリッド探索の学習ランを約 6〜8 割削減しつつ，
グリッド全域での最良の下流分類精度（線形プローブ）を候補範囲内で達成した．
適用境界も定量化する：内在次元推定は$m \gg \hatdID$の参照表現でのみ安定で
自然画像では$m \approx 2\hatdID$を超えると膨張し，
Conv-VAE では$\beta = 4$で AU 遮断が働かない（$\beta$増強で遮断は回復するが，
単一シードの探索的観察にとどまる）．
すなわち本プロトコルは$m$選定の一般解ではなく，
\textbf{$\hatdID$が候補範囲を与え，適切な$\beta$とアーキテクチャのもとで
...
本論文の中心的主張は一文で述べられる：
\begin{quote}
\textbf{ボトルネック次元$m$の候補範囲は入力データの内在次元の推定値$\hatdID$から
...
\begin{enumerate}
  ...
  \item \textbf{多シード実証と適用境界の定量化}（§\ref{sec:exp}）：
        ...
        （プロトコルが$\dtrue$・$\demb$を返す；3シード）と MNIST の全結合 VAE
        （$\hatdID \approx 10$--12 と AU 鈍化域の整合；3シード）で与える．
        Step 1 にはサブサンプリング信頼区間と原理の異なる推定量
        （DANCo・ESS）による相互検証を付す．
        ...
        なお，Fashion-MNIST の Step 3・Conv-VAE の$\beta$増強・自然画像の
        AU プラトーは単一シードの探索的観察であり（\ref{app:exploratory}），
...
本研究はモデル側の自動決定に頼らず，データ幾何側の$\hatdID$から候補範囲を導き，
...
検証点$m_{\mathrm{ver}} \geq 2\hatdID$で学習した VAE の AU（多シード平均）に対し，
次の2条件を判定する：
\begin{align}
...
（ii）\textbf{実データ検証}——MNIST・Fashion-MNIST でプロトコル全体が機能し，
実用価値（探索コスト・下流性能）があるか
...
単一シードの探索的観察は\ref{app:exploratory}に分離した（例外は本文中に明記する）．
...
Torus での$\AUsat = \demb$は，十分な$\beta$と収束・標準化前処理のもとで
...
\textbf{判定}（Step 3 規則）：Swiss Roll（$m_{\mathrm{ver}} = 4 = 2\hatdID$：
AU $= 2.0$，活性率$0.50$），Torus（$m_{\mathrm{ver}} = 6 = 3\hatdID$：
AU $= 3.0$，活性率$0.50$）とも V1・V2 成立で\textbf{合格}（3シード，$\sigma = 0$）．
...
$\hatdID$は候補範囲の中心を与えるため，その統計的不確かさを定量化する
...
$0.5$（MLE）と狭く}，$\hatdID \approx 10$--$12$という候補範囲の中心の読み取りは
標本変動に対して頑健である．
第二に，\textbf{原理の異なる DANCo は$10.0$--$10.9$と TwoNN に密接に一致し}，
ESS は$13$--$15$とやや高いが同一オーダーにある——
Step 1 の読み取りが TwoNN の挙動に固有のものではないことを示す．
一方，生入力空間では DANCo が退化して環境次元$784$を返し（ESS も$18.7$へ上振れ），
\textbf{参照 AE を経由する設計の必要性}が推定量側からも裏づけられる．
...
AE の候補範囲は$m \in [10, 24]$，VAE は$m \geq 20$の大きめ設定となる．
...
単一シードの予備観察（\ref{app:exploratory}）で MNIST と同傾向を確認したが，
多シード再現は今後の課題である．
...
ID 誘導候補範囲$[10, 24]$の内部（$m = 12$）で達成され}，
...
候補範囲内の$m = 12$で達成し，$m \geq 12$ではほぼ横ばいである
...
（a）AE 経路：候補範囲$[10, 24]$内のグリッド点$\{12, 16, 20\}$を
3 シードで学習（9 ラン）——計 12 ラン（\textbf{約 64\%削減}）；
（b）VAE 経路：$m = 2\hatdID = 20$の1点を 3 シードで学習し AU で検証（3 ラン）
——計 6 ラン（\textbf{約 82\%削減}），
のいずれでも探索コストを大幅に削減する．
このとき保持される性能は上記のとおり最良値とほぼ同等である．
ただしこの比較は線形プローブ・MNIST に限られる（§\ref{sec:disc_model_selection}）．
...
seeds). Bold $m$ values lie in the ID-guided candidate range $[10, 24]$,
which contains the best accuracy over the full grid.
...
（Exp-ConvV-HiBeta；\textbf{単一シードの探索的観察}，詳細は\ref{app:exploratory}）．
...
Step 2——候補範囲取得可（$m \geq 20$）；
Step 3——$\beta_{\max} = 4$では\textbf{不合格}（$m = 256$で活性率$0.94 > 0.9$，V1 不成立）．
$\beta_{\max} \in \{10, 20\}$では V1 は成立する（活性率$0.36$・$0.26$）が，
AU $= 93$・$66 > 3\hatdID = 30$のため V2 は不成立のままであり
（かつ単一シードの探索的観察），
...
Step 2——候補範囲は条件付きで取得可（CIFAR-10：$m \geq 50$）；
...
同一の候補範囲$[10, 24]$を支持し（§\ref{sec:exp_downstream}），
候補範囲内の$m$の選択はフルグリッド探索に対して学習ラン数を約 6〜8 割削減しつつ
下流分類精度をほぼ保持した．
...
\textbf{$\hatdID$は候補範囲を与え，適切な$\beta$とアーキテクチャのもとで
...
フルプロトコルが判定規則に合格し，
ID 誘導範囲がフルグリッド探索の学習ランを約 6〜8 割削減しつつ
下流分類精度を保持することを確認した．
同時に，適用境界を定量化した：
