...
%  MDPI MAKE submission — main_paper_MAKE33.tex
%  English version of main_paper_NN33.tex (Japanese draft, version 33)
%  Version 33: revisions addressing Review_codex32.txt (Major 1--8, Minor 1--10) + overall shortening
%  Compiled with official mdpi.cls
%  Compile: pdflatex main_paper_MAKE31.tex
%           pdflatex main_paper_MAKE31.tex
%           pdflatex main_paper_MAKE31.tex
...
(VAEs).
Rather than determining the effective latent dimensionality as a single
unique number, the framework explicitly separates the diagnostic roles of the
indicators: AU values serve as robust direct observations of the number of
KL-active latent coordinates; TwoNN/MLE ID estimates serve as numerically
stable auxiliary reference estimators (stable latent-space references whose
unbiasedness with respect to the true intrinsic dimension is not guaranteed);
and the AU slowdown point $\mknee$ is treated as a TwoNN-guided exploratory
change point.
Theoretically, under a rate--distortion surrogate of the linear-Gaussian
$\beta$-VAE---an interpretive surrogate, not a theorem about trained
nonlinear VAEs---we derive the condition $\lambda_j > \beta\tau^2$ for the
$j$-th eigendirection of the data covariance to be activated.
...
$\mknee$ fluctuates strongly on sparse $m$-grids, and that a dense grid
search around the TwoNN estimate (TwoNN-guided local refinement) stabilizes
$\mknee$ at $10.3 \pm 0.9$---with the scope of this stabilization limited to
MNIST FC-VAEs with $\beta = 4$, a dense grid, and three seeds.
Furthermore, through Fashion-MNIST, Conv-VAE, and CIFAR-10/SVHN experiments,
we quantify the conditions under which the framework functions and the
boundary conditions under which AU/$\mknee$ diagnostics become inconclusive.}
...
bottleneck dimension''---$\mknee$ is a TwoNN-guided exploratory change
point, and the primary readout of effective dimensionality is carried by the
robust indicator (AU values) and the numerically stable auxiliary
reference estimator (TwoNN ID)
(the classification and reliability of the indicators are summarized in
Table~\ref{tab:indicator_class}).
...
\begin{enumerate}
  \item \textbf{Proposal of a multi-indicator diagnostic framework}:
        ...
        present $\mknee = 10.3 \pm 0.9$ under a dense grid placed around the TwoNN
        estimate, with independent initialization and multiple seeds
        (Exp-Mnist-Dense-300), as the main stabilization result
        (Section~\ref{sec:exp_el}).
        However, because the search region itself is chosen from the TwoNN
        estimate, this result is a \textbf{TwoNN-guided local refinement}, not a
        validation independent of TwoNN.
        Its verified scope is limited to MNIST FC-VAEs with $\beta = 4$ and three
        seeds.
...
occurs in ELBO minimization, and qualitatively discussed the relationship
between AU and the variance structure (effective dimensionality) of the data.
...
To avoid misdiagnosis, we explicitly distinguish the following five target
quantities throughout this paper:
\begin{enumerate}[(i)]
  \item \textbf{Intrinsic dimension of the data manifold} $d_\mathrm{ID}^\mathrm{data}$:
        the local degrees of freedom of the input distribution itself.
        Known for synthetic manifolds ($\dtrue$); approximated for real data by
        applying TwoNN/MLE in input space.
  \item \textbf{Intrinsic dimension of the latent representation}
        $d_\mathrm{ID}^\mathrm{latent}$:
        the estimate obtained by applying TwoNN/MLE to the latent representation of a
        reference AE (our $\hat{d}_\mathrm{ID}$).
        Numerically stable, but mediated by the distortion of the reference-AE
        embedding, so agreement with (i) is not guaranteed---it is a stable
        \textbf{latent-space reference} (Section~\ref{sec:disc_twonn_bias}).
  \item \textbf{Number of KL-active coordinates} (AU values):
        the number of latent coordinates the VAE actually uses for encoding.
        An \textbf{observable of coordinate usage} that depends on $\beta$, the
        architecture, the threshold $\delta$, and the training conditions---not an
        intrinsic-dimension estimator.
        $\mknee$ is a change point of the curve $\mathrm{AU}(m)$, and $\AUsat$ is the
        maximum AU value over the grid.
  \item \textbf{Bottleneck dimension required for reconstruction}:
        the smallest $m$ at which the reconstruction error reaches an acceptable
        level (captured by the MSE elbow).
  \item \textbf{Diagnostic design dimension} $\dzopt$:
        the candidate value chosen by the designer by cross-checking the indicators
        of (i)--(iv).  Not a unique optimum.
\end{enumerate}
...
\subsection{Criteria for Stable Identification}\label{sec:natimg_stable_id}

We define here, ahead of the experiments, the criteria required to claim that
...
\textbf{author-defined operational criteria}.
...
RTX~3070 GPU (PyTorch~2.0, CUDA; falling back to CPU when CUDA is unavailable).
...
of GPU time.
Experiment code, configurations, and raw logs (JSON) are provided as
...
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\caption{Summary of experiment groups. Detailed per-experiment configurations (epochs, $\beta$ schedule, $m$ grid, seeds, sample size, role) are listed in Table~\ref{tab:exp_full} (Appendix~\ref{app:exp_table}). Categories: main experiments (core framework validation), supplementary experiments (prerequisite verification), boundary experiments (validity range quantification).}
\label{tab:exp_list}
\small
...
\textbf{Caveat}: the $\AUsat$ of the Swiss Roll is strongly preprocessing-dependent
(2 under standardized conditions, 3 under other conditions), so we restrict the main
evidence supporting the correspondence between $\AUsat$ and $\demb$ to the Torus and
the M\"obius Band ($\AUsat = \demb = 3$).
Even this correspondence is an empirical observation that depends on our experimental
conditions (standardized preprocessing, $\beta = 4$, sufficient convergence) and is not
guaranteed as a theoretical necessity (see \S\ref{sec:discussion}).
...
on the grid design and initialization trajectory.
In this paper we treat $\mknee = 10.3 \pm 0.9$ from Exp-Mnist-Dense-300---obtained with a
dense grid around the TwoNN estimate, independent initialization, and multiple
seeds---as the main stabilization result (Section~\ref{sec:exp_el}).
...
\begin{itemize}
  ...
  \item \textbf{Grid-density factor (one of the main causes)}: with the dense grid (Exp-Mnist-Dense), $\mknee = 10.3 \pm 0.9$ (per-seed values 11, 9, 11) --- a large improvement over the sparse-grid $43\pm15$, consistent with the estimate $\hat{d}_\mathrm{ID} \approx 10$ (note, however, that this is agreement as a TwoNN-guided refinement, since the dense region was placed around the TwoNN estimate)
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Exp-Fashion: Fashion-MNIST AE and VAE results (100 epochs, $n_\mathrm{train}=10{,}000$, representative $m$ values; \textbf{single seed 42}).}
\label{tab:ef}
\small
...
AU remains at 25 even at $m = 256$ (unlike 36 for MNIST), and the TwoNN ID estimates
($\approx 9$--$10$) are also somewhat smaller than those for MNIST ($\approx 10$--$12$).
Interpreting these differences (e.g., in terms of dataset-level local degrees of freedom)
would require a more rigorous analysis; here we report them only as single-seed observations.
...
Results of Experiment Exp-Conv-AE (Table~\ref{tab:eg}):
...
\begin{itemize}
  \item \textbf{Identification of stabilization conditions (with explicit scope)}:
        In the MNIST multi-seed sparse-grid experiments, $\mknee$ fluctuated widely
        between 32 and 64, revealing that on coarse grids it cannot be used as a
        reliable diagnostic aid.
        Under a dense grid placed around the TwoNN estimate, with independent
        initialization and multiple seeds (Exp-Mnist-Dense-300), $\mknee = 10.3 \pm 0.9$
        stabilized in the vicinity of the TwoNN ID ($\approx 10$).
        However, this result is confirmed only under the conditions
        \textbf{MNIST FC-VAE, $\beta = 4$, TwoNN-guided dense grid, three seeds},
        and since the search region itself depends on the TwoNN estimate, it is not an
        independent validation (\S\ref{sec:exp_el}).
        The default description of $\mknee$ is therefore ``a TwoNN-guided exploratory
        change point,'' and we do not recommend its general use without stating these
        conditions.
...
\begin{enumerate}
  \item \textbf{Dense grid design (TwoNN-guided local refinement)}: place dense
        step-1 points around the estimate $\hat{d}_\mathrm{ID}$ (obtained from the TwoNN
        ID), as demonstrated in Exp-Mnist-Dense (\S\ref{sec:exp_el}).
  ...
  \item \textbf{Ensemble $\mknee$}: compute each $m^{\mathrm{knee}(s_i)}$ from the AU
        curves for multiple seeds and adopt the \textbf{median}:
        \begin{equation}
          m^{\mathrm{knee,ens}} = \mathrm{median}\bigl\{m^{\mathrm{knee}(s_1)}, \ldots, m^{\mathrm{knee}(s_S)}\bigr\}
          \label{eq:mknee_ens}
        \end{equation}
        The median is robust to outliers in which $\mknee$ is stuck at the grid
        boundary (when $S$ is even, we take the arithmetic mean of the two adjacent
        values).
...
In brief, under dense-grid conditions (Exp-Mnist-Dense) the $\tau$ method
($10.3 \pm 0.9$) is more robust than the second-difference method
($18.3 \pm 6.8$); when the $\tau$ method becomes unstable (the AU growth rate
staying near the boundary value across multiple $m$), switching to the
second-difference alternative is effective.
...
This interpretation is consistent with the results of \S\ref{sec:exp_en} and
\S\ref{sec:exp_hibeta}: at $\beta_{\max}=4$, no saturation plateau appears even when
extending to $m=512$ ($d^*(\beta) > 512$), whereas increasing $\beta_{\max}$ to 10 and
20 yields identifiable $\mknee \approx 128$ and $96$ (Table~\ref{tab:hibeta})---exactly
the theoretical prediction that increasing $\beta$ shifts $d^*(\beta)$ toward smaller
values.
The non-observation of saturation at $\beta_{\max}=4$ is thus not an applicability
limit of the framework but a consequence of $\beta$ being insufficient relative to the
decoder capacity; application to Conv-VAEs requires strengthening $\beta$ or reducing
decoder capacity.
...
\begin{itemize}
  ...
  \item \textbf{The instability of $\mknee$ is a first-class limitation}:
        as the multi-seed validation (\S\ref{sec:exp_multiseed}) shows, the sparse-grid
        instability of $\mknee$ is not resolved by extending training, and its
        dense-grid stabilization has been confirmed only under the restricted setting
        of MNIST FC-VAE, $\beta=4$, and a TwoNN-guided search region.
        Grid density and multiple seeds cannot in general be regarded as sufficient
        stabilization measures, and we do not recommend using $\mknee$ without stating
        these conditions.
...
stabilized $\mknee = 10.3 \pm 0.9$ under a dense grid placed around the TwoNN
estimate, with independent initialization and multiple seeds
(Exp-Mnist-Dense-300; Section~\ref{sec:exp_el}).
This stabilization is a \textbf{TwoNN-guided local refinement}, and its
verified scope is limited to MNIST FC-VAEs with $\beta = 4$, a dense grid, and
three seeds---we do not recommend general use of $\mknee$ without stating
these conditions.
...
(Table~\ref{tab:real_geom_summary}) and a qualitative confirmation on
Fashion-MNIST (Appendix~\ref{app:twonn_detail})
...
\begin{table}[H]
\centering
\caption{Three-tier applicability of the diagnostic framework (summary of evidence).}
...
Exp-Fashion & Fashion-MNIST / AE, VAE & 100 / 4 & $\{1,4,\ldots,256\}$ & 1 & 10,000 & Prelim.\ external check \\
