...
%% AE/VAEにおける有効潜在次元の診断的解釈：多指標分析フレームワーク（版21）
%% コンパイル: platex main_paper_NN21.tex && dvipdfmx main_paper_NN21.dvi
...
\title{AE/VAE における有効潜在次元の診断的解釈：\\
       Active-Unit Dynamics・内在次元推定・再構成指標の役割と限界に基づく\\
       多指標分析フレームワーク}
\author{（著者名）\\
{\small （所属機関）}\\
{\small \texttt{（メールアドレス）}}}
...
\textbf{本フレームワークの目的は，有効潜在次元を一意に決定することではない}．
複数の指標が何に対して頑健で，何に対して探索的（グリッドや初期化に依存）かを
定量的に明確化し，設計者が単一指標への過信を避けながら診断的に活用する解釈的基盤を提供する．
とりわけ，本論文が定義する AU 鈍化点$\mknee$は，主要指標（AU 値・TwoNN/MLE ID）を補完する
\textbf{探索的指標}に過ぎず，ボトルネック次元の一意な同定のために用いるものではない——
この区別は本アブストラクトの段階から明示する．
...
\textbf{探索的指標}（$\mknee$：グリッド・シード依存，主指標ではない）に明示的に分類した
解釈論を構築した点にある（表\ref{tab:indicator_class}）．
...
\begin{enumerate}
  \item \textbf{頑健指標 AU 値の実証的確認，および TwoNN ID の数値安定性確認}：
        ...
        実データ幾何比較実験（実験 Exp-Real-Geom，§\ref{sec:exp_real_geom}）では，
        等長性改善より「$m \gg \hat{d}_\mathrm{ID}$」が TwoNN 安定性の支配的条件であることを
        MNIST・Fashion-MNIST の複数モデル比較で示し，命題\ref{thm:twonn_stability}（§\ref{sec:theory_stability}）として理論化した．
  ...
  \item \textbf{自然画像・Conv-VAE における破綻条件の定量的解明}（限界検証実験群の貢献）：
        自然画像での AU/$\mknee$ 安定同定は現時点で「未達」だが，\textbf{本論文はこれを弱点でなく貢献として位置づける}——
        「どの条件（$\beta$水準，デコーダ容量，グリッド設計）でフレームワークが機能し，
        どの条件で破綻するか」を定量的に解明することが本研究の知見である．
        Conv-VAE での高$\beta$実験（Exp-ConvV-HiBeta，§\ref{sec:exp_hibeta}）は，
        $\beta_{\max}=4$での AU 飽和未観測が「フレームワーク破綻」ではなく「$\beta$設定不足」に起因することを実証した．
        CIFAR-10 拡張スイープ（Exp-NatImg-Sweep，§\ref{sec:exp_natimg_sweep}）では
        $\beta_{\max}=80$・薄型デコーダ条件で AU 飽和プラトーを初観測し，
        stable identification（5項目基準，§\ref{sec:natimg_stable_id}）への必要条件を定量化した——
        これは「限界がどこにあるかを明確化した知見」であり，FC-VAE で確立したフレームワークの
        自然画像への拡張に向けた境界条件の特定という研究上の意義を持つ．
\end{enumerate}

本論文の方法論的貢献は，「各指標が何に対して頑健で，何に対して探索的か」を
実証的かつ理論的に明確化した点にあり，AE/VAE の設計・診断のための
実用的な多指標解釈ガイドラインを確立する．
特に自然画像・Conv-VAE 設定での未達事項は，フレームワークの弱点ではなく
「どこから先が未解決か」を切り分けた研究の正直な姿勢として提示する．
\end{abstract}
...
\begin{enumerate}
  \item \textbf{有効次元の多指標診断フレームワーク}（主要貢献）：
        ID 推定（TwoNN/MLE），AU Dynamics（AU 値，$\mknee$，$\AUsat$），MSE エルボー，PCA の
        組み合わせにより，設計者が単一指標への過信を避け，有効次元を診断的・相補的に解釈する
        ...
        自然画像・Conv-VAE での AU/$\mknee$ 安定同定が未達な設定については，
        Exp-NatImg-AU（必要$\beta$・アーキテクチャ条件の定量化）と
        Exp-NatImg-Sweep（$\beta_{\max}=80$・薄型デコーダ条件での飽和プラトー初観測）により
        \textbf{「どの条件でフレームワークが機能し，どの条件で破綻するか」を解明した貢献}として位置づける——
        これは弱点の開示ではなく，適用領域の境界条件を定量化した知見である（§\ref{sec:disc_convvae}，§\ref{sec:disc_limitation}）．
  ...
  \item \textbf{実データ幾何比較実験と TwoNN 条件付き安定性命題}：
        ...
        および$\mknee$検出法比較（実験 Exp-NatImg-Knee）を実施した．
...
\begin{description}
  ...
  \item[\textbf{限界検証実験}（「どの条件で破綻するか」の定量化）]
        Exp-Conv-Fix，Exp-Conv-Ann，Exp-Conv-M256，Exp-Conv-M512，Exp-ConvV-HiBeta，Exp-NatImg-AU，Exp-NatImg-Sweep，Exp-NatImg-Knee．
        Conv-VAE・自然画像での AU 飽和未達は「フレームワーク破綻」ではなく，
        「どの条件（$\beta$水準，デコーダ容量，グリッド設計）で機能・不機能か」を定量化した貢献として位置づける．
\end{description}
...
位置づけの凡例：\textbf{主実験}（フレームワーク中核の実証），補助実験（前提条件・解釈論の検証），\textit{限界検証}（破綻条件の定量化）．}
\label{tab:exp_list}
\small
\begin{tabular}{lllll}
\toprule
...
\midrule
Exp-Syn-1 & MSE 肘点・ID 確認 & 合成多様体 & \textbf{主実験} & RQ1, RQ2 \\
Exp-Syn-AU & AU 飽和（AE vs VAE） & 合成多様体 & \textbf{主実験} & RQ1, RQ2 \\
Exp-Syn-Top & 位相的埋め込み次元と AU & Torus/Möbius 等 & 補助実験（探索的） & --- \\
Exp-Syn-Iso & IsometricAE・等長性検証 & 合成多様体 & 補助実験（RQ3） & RQ3 \\
Exp-Stab & TwoNN 数値安定性の実証 & Swiss Roll + MNIST & 補助実験（Stage 1 妥当性） & RQ2 \\
Exp-Geom & MNIST AE $m$スイープ：$\kappa$・TwoNN 共変化 & MNIST AE, $m=4$〜128 & 補助実験（幾何的妥当性） & RQ2, RQ3 \\
\textbf{Exp-Iso-ID} & \textbf{等長性改善が TwoNN ID に与える影響} & \textbf{Swiss Roll, AE vs IsometricAE} & 補助実験（等長性バイアス検証） & \textbf{RQ2, RQ3} \\
Exp-Mnist-Base & MNIST VAE スイープ（主実験） & MNIST FC-VAE, $m=1$〜256 & \textbf{主実験} & RQ1, RQ2 \\
Exp-Mnist-Multi & MNIST 多シード検証 & 3 シード×150/300ep & \textbf{主実験} & RQ1, RQ2 \\
Exp-Mnist-PCA & 理論-実験橋渡し（PCA スペクトル） & MNIST PCA+Exp-Mnist-Base 対比 & 補助実験（理論補足） & RQ1 \\
Exp-Mnist-Dense & 密グリッド検証（3因子分離） & MNIST FC-VAE, $m=8$〜64 密 & \textbf{主実験}（$\mknee$安定化） & RQ1 \\
Exp-Fashion & Fashion-MNIST 適用検証 & Fashion-MNIST FC-VAE & \textbf{主実験} & RQ1, RQ2 \\
Exp-Conv-AE & 畳み込み AE での ID 整合性確認 & MNIST Conv-AE & 補助実験 & RQ2 \\
Exp-Conv-Fix & 固定$\beta$ Conv-VAE & MNIST Conv-VAE & \textit{限界検証} & RQ1 \\
Exp-Conv-Ann & KL アニーリング Conv-VAE & MNIST Conv-VAE & \textit{限界検証} & RQ1 \\
Exp-Conv-M256 & Conv-VAE 拡張 $m$ グリッド & MNIST Conv-VAE, $m=4$〜256 & \textit{限界検証} & RQ1 \\
Exp-Conv-M512 & Conv-VAE $m=384$・512 拡張 & MNIST Conv-VAE, 3シード & \textit{限界検証} & RQ1 \\
Exp-ConvV-HiBeta & 高$\beta$による AU 飽和の誘発 & MNIST Conv-VAE, $\beta_{\max}\in\{10,20\}$ & \textit{限界検証（破綻条件の解明）} & RQ1 \\
Exp-Nat-Cifar & CIFAR-10 TwoNN 外部妥当性確認 & CIFAR-10 Conv-VAE & \textbf{主実験} & RQ2 \\
Exp-Nat-SVHN & SVHN 部分崩壊モニタリング & SVHN Conv-VAE & \textit{限界検証} & RQ1 \\
\textbf{Exp-NatImg-AU} & \textbf{CIFAR-10 AU 飽和条件の探索} & \textbf{CIFAR-10 Conv-VAE, $\beta_{\max}\in\{10,20,40\}$} & \textbf{\textit{限界検証（必要条件の特定）}} & \textbf{RQ1} \\
\textbf{Exp-Real-Geom} & \textbf{実データ幾何比較：複数モデル TwoNN 安定性} & \textbf{MNIST+Fashion-MNIST, AE/IsometricAE/CAE/DAE} & \textbf{主実験} & \textbf{RQ2, RQ3} \\
\textbf{Exp-NatImg-Sweep} & \textbf{自然画像拡張スイープ} & \textbf{CIFAR-10, $m\in[32,1024]$, $\beta_{\max}\in\{10,20,40,80\}$} & \textbf{\textit{限界検証（境界条件の定量化）}} & \textbf{RQ1} \\
\textbf{Exp-NatImg-Knee} & \textbf{$\mknee$検出法比較（自然画像）} & \textbf{CIFAR-10, 4手法比較} & \textbf{\textit{限界検証（手法比較）}} & \textbf{RQ1} \\
Exp-Mnist-Dyn & MNIST 学習ダイナミクス & MNIST FC-VAE, $m=16$ & 補助実験（単一シード） & --- \\
Exp-Mnist-Ent & Manifold Entanglement 予備検証 & MNIST クラス制限 & 補助実験（予備的） & --- \\
Exp-Tau-Cross & $\tau_\mathrm{knee}$横断検証 & 複数データセット & 補助実験（感度分析） & RQ1 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Section A の示唆}：
AE と IsometricAE は$m=2=d_\mathrm{true}$でほぼ同程度に TwoNN$\approx1.55$と過小推定する（$\kap$の差 1.06 vs 1.79 にほぼ依存しない）
一方，$m=4>d_\mathrm{true}$の AE（$\kap=2.155$）は TwoNN$\approx2.03$へ回復する．
すなわち$\kap \in [1.06, 2.16]$の範囲では，ボトルネック余裕（$m - d_\mathrm{true}$）の不足が
TwoNN 過小推定の主因であり，$m \gg d_\mathrm{ID}$での TwoNN 利用の有用性を支持する（実験 Exp-Iso-ID で$m \in \{2,3,4,6,8\}$全範囲にわたり確認；§\ref{sec:exp_iso_id}）．
...
一方，$m=2=d_\mathrm{true}$では両モデルとも TwoNN$\approx1.78$〜$1.80$と過小推定となり，
入力空間 TwoNN（2.427）を下回る．
以上から，本設定における$\kap \in [1.1, 2.1]$での等長性改善は TwoNN ID の系統的バイアスを有意には低減せず，
「$m \gg d_\mathrm{ID}$という十分なボトルネック余裕の確保」が TwoNN 推定の数値的安定化の支配的指針であることが確認された．
...
\textbf{実データへの含意}：
本実験結果は Exp-Iso-ID（合成多様体；§\ref{sec:exp_iso_id}）の知見を
...
前節（Exp-NatImg-AU，$\beta_{\max} \in \{10,20,40\}$，$m \in [32,384]$）を大幅に拡張する：
\begin{itemize}
...
\midrule
4  & Standard & 384 & $208\pm12$ & $25.4\pm0.5$ & Exp-NatImg-AU（§\ref{sec:exp_natimg_au}）と一致 \\
...
$\mknee$を設計に用いる際は$m$グリッド粒度・$\tau_\mathrm{knee}$・$\beta$・収束度への注意が必要であり，
密グリッドと複数シードのアンサンブルを推奨する（付録\ref{app:tau_knee}，\ref{app:beta}）．
...
実験 Exp-Iso-ID（§\ref{sec:exp_iso_id}）では$\kap \in [1.1, 2.1]$の範囲で等長性改善が TwoNN ID の系統的バイアスを有意に低減しないことが判明し，
TwoNN 過小推定の主因はボトルネック余裕の不足（$m < d_\mathrm{true}$）であることが確認された（詳細は§\ref{sec:exp_iso_id}）．
...
\textbf{自然画像への含意}~~~
CIFAR-10・SVHN では TwoNN ID の外部妥当性は確認されたが，AU/$\mknee$ 側の安定同定には至らなかった（§\ref{sec:exp_natural}，§\ref{sec:exp_natimg_au}）．
...
Exp-NatImg-Sweep での初観測（§\ref{sec:exp_natimg_sweep}）はこの条件下（$\beta_{\max}=80$，薄型デコーダ）で達成されている．
...
\begin{itemize}
  \item \textbf{Conv-AE・Conv-VAE 検証}（実験 Exp-Conv-AE・Exp-Conv-Fix・Exp-Conv-Ann・Exp-Conv-M256）：
        ...
        CIFAR-10・SVHN 実験では TwoNN の外部妥当性を確認し（§\ref{sec:exp_natural}），
        拡張スイープ（Exp-NatImg-Sweep，§\ref{sec:exp_natimg_sweep}）で AU 飽和プラトーの初観測に成功したが，
...
\begin{itemize}
  \item \textbf{Stage 1 の ID 推定：等長性バイアスの厳格な評価}：
        ...
        （Exp-Stab・Exp-Iso-ID での$\kap \in [1.06, 2.16]$での等長性改善が TwoNN に有意な影響を与えないことも，$m \gg d_\mathrm{ID}$という運用条件の重要性を裏付ける；§\ref{sec:exp_ep}，§\ref{sec:exp_iso_id}）．
...
本論文では，AE/VAE の有効潜在次元を定量的に診断する
\textbf{多指標診断フレームワーク}を提案し，
合成多様体・MNIST・Fashion-MNIST での主実験と，CIFAR-10・SVHN での外部妥当性確認，
および Conv-VAE・自然画像での限界検証実験を行った．
\textbf{本フレームワークはボトルネック次元を一意に決定するものではなく，
各指標の頑健性・限界・適用条件を定量的に明確化した診断的解釈論を提供するものである}．
フレームワークの安定動作は FC-VAE と MNIST/Fashion-MNIST の範囲で最もよく実証されており，
Conv-VAE や自然画像への適用については「どの条件で機能し，どの条件で破綻するか」を
限界検証実験（主実験群 (C)）により定量化した——これを弱点としてではなく，
フレームワークの適用領域の境界条件を解明した貢献として提示する．
本論文の主要貢献として，
（i）実データ幾何比較実験（Exp-Real-Geom）と TwoNN 条件付き安定性命題（命題\ref{thm:twonn_stability}）により，
「$m \gg \hat{d}_\mathrm{ID}$が TwoNN 安定性の支配的条件」を理論・実験の両面から示した；
（ii）自然画像拡張スイープ（Exp-NatImg-Sweep）と stable identification 判定基準の明文化（§\ref{sec:natimg_stable_id}）により，
AU 飽和プラトーの初観測（$\beta_{\max}=80$，薄型デコーダ，CIFAR-10）を達成し，自然画像での stable identification への必要条件を定量化した（strict な stable identification は未達であり，今後の目標として残す）．
...
主要な有効次元の読み取りは Stage 2 の AU Dynamics（頑健指標）に委ねる（§\ref{sec:framework_pipeline}）．
$\mknee$は AU 曲線形状から読む\textbf{探索的指標}に過ぎず，主要指標の代替ではない（§\ref{sec:framework}，表\ref{tab:indicator_class}）．
...
\textbf{自然画像（CIFAR-10/SVHN）}
  & ✓（Pope値と整合）
  & △/✓（$\beta_{\max}=80$薄型デコーダでプラトー初観測）
  & △（5条件中0達成；飽和プラトーは観測，stable IDは未達）
...
この3段階整理は，フレームワークの「弱点の開示」ではなく，
\textbf{どこまで確かめ，どの条件で破綻し，どこから未解決かを切り分けた研究}として位置づける枠組みである．
本論文は「有効潜在次元を一意に決定する方法の提案」ではなく，
\textbf{「どの指標が頑健で，どの条件でどの指標が限界を迎えるかを定量化した診断フレームワーク」の確立}を目指す——
この姿勢が，自然画像・Conv-VAE での未達事項を弱点ではなく貢献として提示できる根拠である．
FC-VAE + MNIST/Fashion-MNIST では解釈論が最もよく検証されており，
Conv-VAE については「$\beta_{\max}=4$ での未飽和は $\beta$ 設定不足でも説明可能であることを MNIST 上で確認した」という範囲に実証が留まる．
自然画像での AU/$\mknee$ 安定同定は引き続き未解決だが，
Exp-NatImg-AU・Exp-NatImg-Sweep により「必要$\beta$条件，デコーダ容量条件，stable identification 5基準の現達成状況」を定量化した——これが境界条件の解明という本論文の知見である
...
\begin{enumerate}
  \item \textbf{$\tau_{\mathrm{knee}}$の適応的設定}：
        ...
        実験 Exp-NatImg-Knee（§\ref{sec:exp_natimg_knee}）では自然画像での平滑化後曲率法が最安定であることを確認したが，
  ...
  \item \textbf{$\mknee$安定化の定量的検証}：
        ...
        stable identification の 5 条件判定枠組み（§\ref{sec:natimg_stable_id}）により，
        条件の達成状況を定量的に記述できる．
  ...
  \item \textbf{自然画像での $\beta_{\max}$ 設定と薄型デコーダの組み合わせ最適化}：
        Exp-NatImg-Sweep の結果（§\ref{sec:exp_natimg_sweep}）から，$\beta_{\max}=80$・薄型デコーダという
        設定でのみ AU 飽和が初観測された．stable identification の達成には，
        $\beta_{\max} \in [40,120]$の密スイープと，デコーダ容量・シード安定性の同時最適化が次のステップとなる．
        具体的には，AU プラトー値のシード間$\sigma \leq 0.05 \cdot \AUsat$（条件4）と
        $\beta_{\max}$変動±20\%での$\mknee$安定性（条件3）の達成が最優先課題である．
\end{enumerate}
