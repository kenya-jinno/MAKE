...
%  MDPI MAKE submission — main_paper_MAKE45.tex
...
$m \geq 2\hatdID$ and verifying after training whether the surplus
...
diagnostically detects an over-pruning boundary case), and for the specific
MNIST grid and evaluation protocol used here the ID-guided window reduces
the training runs of a
...
$m \gg \hatdID$---on natural images, latent-space TwoNN estimates increased
when the reference bottleneck exceeded the provisional consistency
region---and for the tested Conv-VAE schedule with $\beta_{\max} = 4$,
AU-based truncation was not observed within the tested range, whereas
increasing $\beta_{\max}$ reduced the activation ratio (3 seeds) without
establishing order consistency.
...
($d^*_{\mathrm{eff}}(\beta) > 512$)---and \textbf{the clear AU $= m$
pattern flags the diagnostic condition ``no substantial truncation observed
