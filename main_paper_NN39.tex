%% main_paper_NN39.tex
%% MDPI MAKE 投稿用日本語草稿
%% 入力データの内在次元に基づく AE/VAE ボトルネック次元の選定（版39）
%% 版39の変更点：Review_codex38.txt（accept with minor corrections;
%% Minor 1--4, 体裁 1--4）への対応
...
%% コンパイル: platex main_paper_NN39.tex && platex main_paper_NN39.tex && dvipdfmx main_paper_NN39.dvi
...
Conv-VAE では$\beta = 4$で AU 遮断が働かず，
$\beta$増強は遮断を回復するがオーダー整合は未達である
（単一シードの探索的観察）．
...
表示した V2 域は保守的に丸めた値であり，平均判定側の計算値の下端は$2.24$）．
...
単一シードの探索的観察（$\beta$増強 Conv-VAE・Fashion-MNIST の Step 3）は
...
したがって Fashion-MNIST では Step 1 は多シードで確認済みである一方，
VAE 側の AU 検証（Step 3）は MNIST と同傾向を示す単一シードの
予備観察（\ref{app:exploratory}）にとどまり，
フルプロトコルの多シード再現は今後の課題である．
...
\begin{itemize}
  \item \textbf{検証範囲}：フルプロトコルの多シード検証は合成多様体と
        MNIST の AU 検証に限られる（MNIST の Step 1 スイープは
        不確かさを別途定量化した単一シード；Fashion-MNIST は Step 1 のみ多シード）．
