...
\begin{enumerate}
  \item \textbf{頑健指標 AU 値の実証的確認，および TwoNN ID の数値安定性確認}：
        ...
        Conv-VAE の AU 飽和遅延については，拡張$m$グリッド実験（実験 EN；$m\in[4,256]$，3シード，§\ref{sec:exp_en}）と
        強力デコーダによる$\tau^2_\mathrm{eff}$低下という理論的観点（§\ref{sec:disc_convvae}）から
        実証した：$m \leq 64$では AU$=m$（全次元活性）が継続するが，$m = 96$から AU$=90.7\pm1.2$となり
        飽和が始まる（$m=256$: AU$=241.3$，$\mknee=256$；完全飽和検出には$m>256$が必要）．
...
E6 & MNIST 学習ダイナミクス & MNIST FC-VAE, $m=16$ & 副次・探索的 \\
EY & Manifold Entanglement 予備検証 & MNIST クラス制限 & 副次・探索的 \\
EZ & $\tau_\mathrm{knee}$横断検証 & 複数データセット & 副次 \\
\bottomrule
\end{tabular}
\end{table}
...
RQ3（等長性，副次） & IsometricAE 実験(EC) & 命題\ref{thm:pullback} \\
理論-実験橋渡し & PCA スペクトル分析(EM) & 定理\ref{thm:linear_vae} \\
副次（学習ダイナミクス） & 学習ダイナミクス(E6), Manifold Entanglement 予備(EY) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  $m=d_\mathrm{true}=2$では AE・IsometricAE ともに TwoNN$\approx1.55$と過小推定（$\kap$の差より
  ボトルネック余裕の不足が主因）；$m=4>d_\mathrm{true}$で TwoNN が$2.03$へ回復．
  （右）MNIST での TwoNN vs $m$ 収束曲線（参照 AE，$\kap\approx10^9$）：
  $m\geq128$で TwoNN が$\approx10$に収束し，$m \gg d_\mathrm{ID}$条件下での数値的安定性を示す．}
  \label{fig:ep}
\end{figure}}{}
...
%% ===================================================================
\subsection{Conv-VAE 拡張$m$グリッド実験（実験 EN）}\label{sec:exp_en}

EK 実験（§\ref{sec:exp_annealing}）で$m \leq 64$では AU 飽和が観測されなかったことを受け，
$m$グリッドを 256 まで拡張した実験（EN）を実施した．
§\ref{sec:disc_convvae}で論じる$\tau^2_\mathrm{eff}$の低下仮説のもとでは，
Conv-VAE の$d^*(\beta)$は FC-VAE のそれ（$\approx 10$）より大幅に大きいため，
...
\textbf{自然画像実験（EI・EJ）の位置づけ}：
本実験の主眼は\textbf{フレームワーク全体の自然画像での有効性検証ではなく}，
TwoNN ID 推定（数値安定な補助的参照推定量）の外部妥当性確認に限定される．
CIFAR-10・SVHN での AU 飽和プラトーは今回のグリッド範囲・アーキテクチャでは未到達であり，
AU/mknee 側の完全なフレームワーク適用は将来課題として残る（詳細は以下）．

\textbf{（確認された成果）}
\begin{itemize}
  \item \textbf{TwoNN ID 推定の外部妥当性（EI；主要確認事項）}：
        ...
        これにより，TwoNN 推定（補助的参照推定量として用いる $\hat{d}_\mathrm{ID}$）の
        自然画像への外部妥当性が確認された．
        ただし，この知見は TwoNN 推定の信頼性に関する確認であり，
        フレームワーク全体の自然画像への汎化を意味するものではない．
  \item \textbf{SVHN の部分的崩壊の AU モニタリング（EJ）}：
        ...
        整合する部分崩壊が観測されたが，AU 飽和プラトーには至らず，
        AU/mknee 側の解釈的フレームワーク適用は今回の設定では未達である．
\end{itemize}
...
頑健指標（AU 値），数値安定な補助的参照推定量（TwoNN ID），探索的指標（$\mknee$）を
明確に区別して用いる点にある．
...
\textbf{拡張$m$グリッド実験（実験 EN）による検証}~~~
この予測を検証するために，$m \in [4, 256]$（9点，KL サイクリックアニーリング，150 エポック，3シード）
の Conv-VAE 実験（EN）を実施した．
実験 EN の結果（表\ref{tab:en}，図\ref{fig:en}；§\ref{sec:exp_en}）を参照する．
$m \leq 64$では AU$=m$（全次元活性）が3シードで一致するが，
$m = 96$で初めて AU$=90.7\pm1.2$（$<96$）が現れ，以降$m$の増加に伴い活性率が徐々に低下する
（$m=256$: AU$=241.3\pm2.9 \approx 0.94m$）．
FC-VAE ではすでに$m=12$付近で AU 飽和が始まり$m=64$で AU$=22.3$（活性率 35\%）に留まるのと対照的に，
Conv-VAE では$m=256$時点でも活性率 94\%と高い．
これは「Conv-VAE の$d^*(\beta)$が FC-VAE より大幅に大きい」という$\tau^2_\mathrm{eff}$理論と整合し，
フレームワーク自体の普遍性（Conv-VAE でも水充填原理が定性的に成立すること）を支持する証拠となる．
ただし，$\mknee$（$\tau=0.25$）は全3シードで 256（範囲上限）となり，
明確な飽和プラトーの同定には$m > 256$が必要であることも確認された．
...
\begin{itemize}
  \item \textbf{Conv-AE・Conv-VAE 検証}（実験 EG・EH・EK・EN）：
        ...
        EN 実験（§\ref{sec:exp_en}）により拡張$m$グリッドでの定性的傾向は確認されたが，
        明確な$\mknee$同定には$m>256$が必要であり，
