...
%  MDPI MAKE submission — main_paper_MAKE43.tex
...
The contributions of this paper are organized into three contributions and
one theoretical motivation:
\begin{enumerate}
  ...
  \item \textbf{Empirical contribution: multi-seed validation and
        ...
        inflates beyond;
        (c) for Conv-VAEs at $\beta_{\max} = 4$ the truncation threshold
        $\dstar$ exceeds 512, so AU verification requires a strengthened
...
In this spirit, $\hatdID$ provides the reference point ``below this is
insufficient in principle,'' and the possibility $\demb > \dID$ (a
...
of the ID, and Allegra et al.~\cite{allegra2020data} model exactly this
heterogeneous-local-ID structure).
...
We emphasize that this exactness is a property of the \textbf{particular
globally optimized linear-Gaussian solution considered here}---the
aggregated-posterior mismatch vanishes on truncated coordinates and is zero
at this optimum on active ones---and should not be generalized to arbitrary
linear or nonlinear VAEs.
