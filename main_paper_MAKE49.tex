...
The latent dimension of a variational autoencoder~\cite{kingma2014vae} has to be set before
...
place. Some estimate the intrinsic dimension of the data~\cite{facco2017twonn,levina2005mle}
and use it to place a search window~\cite{bonheme2022fondue}. Others train a single large model briefly and read
off how many latent coordinates remain in use, either from the variance of
the posterior means~\cite{burda2016iwae} or from a classification of the
latent variables into types~\cite{bonheme2023active}. Others again prune during training, under an explicit constraint on
reconstruction quality~\cite{deboom2020geco} or under a prior that
suppresses irrelevant axes~\cite{saha2025ardvae}.
