...
\begin{enumerate}
  \item \textbf{頑健指標の実証的確認}：
        ...
        TwoNN 推定の\textbf{外部妥当性}を示す（§5.8）——ただし，自然画像での AU/mknee 側の検証は
...
TwoNN ID（$\hat{d}_\mathrm{ID}$） & \textbf{頑健指標}$^\dagger$ & $\sigma \leq 0.12$（$m=64$） & 内在次元の参照推定 \\
...
TwoNN ID 推定（頑健指標）の外部妥当性確認と，AU モニタリングの基礎的有用性の確認にある．
