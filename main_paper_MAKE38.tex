...
%  MDPI MAKE submission — main_paper_MAKE38.tex
%  English version of main_paper_NN38.tex (Japanese draft, version 38)
%  Version 38: revisions in response to Review_codex37.txt (Minor revision;
%  Minor 1--8, presentation 1--5)
%    - primary Step-1 estimator pre-registered as the converged TwoNN value
%      (rounded to integer); MLE/DANCo/ESS are cross-checks; when the MLE
%      cross-check shifts 2*dhat past the next grid point, verify there too
%      (MNIST: m=32 also passes) (Minor 1, 6)
%    - Torus margin unified as "m >= 5 (fine grid); m = 6 = 2 d_emb is the
%      first verified point on the coarse grid" in abstract / Step 2 /
%      conclusion (Minor 2)
%    - "all main-text results are multi-seed" narrowed to VAE verification
%      results; single-seed reused sweeps (Step-1 reference-AE sweep, AE
%      downstream baseline) listed explicitly (Minor 3)
%    - threshold-invariance ranges split into mean-verdict (V1 0.67--0.94)
%      and per-seed (V1 0.70--0.93) ranges, computed by the extended
%      Exp_AU_Rule.py (Minor 4)
%    - Fashion-MNIST Step-1 estimate unified to dhat ~= 10 (grid design
%      value d=9 marked as preliminary) (Minor 5)
%    - reporting-requirements compliance claim softened (Minor 7)
%    - Proposition-3 references kept as "conditional-stability sketch" (Minor 8)
%    - "restores V1 but not V2" wording unified (presentation 2);
%      Table 8 caption separates search window vs. verification point
%      (presentation 3); torus 3x MSE improvement annotated with m=4 -> m=6
%      (presentation 4); abstract lightly tightened (presentation 5)
%  Compiled with official mdpi.cls
%  Compile: pdflatex main_paper_MAKE38.tex  (x3 for cross-references)
...
autoencoders (VAEs) is their most fundamental design parameter, yet in
practice it is usually chosen by trial and error or convention.
...
$\AUsat = \demb = 3$ for $m \geq 5$, with boundary over-pruning at
$m \leq 4$ detected by the MSE cross-check; MNIST:
...
the AU truncation fails at $\beta = 4$ ($\beta$-strengthening restores
truncation but not order consistency with $\hatdID$, in single-seed
exploratory runs).
...
\begin{enumerate}
  ...
  \item \textbf{Multi-seed validation and quantified applicability
        ...
        3 seeds; conditional-stability sketch in
        ...
        Conv-VAE recovery (V1 only, not V2), and the natural-image AU
        plateau are single-seed
...
for stable estimation is consistent with the conditional-stability sketch
of Proposition~\ref{thm:twonn_stability} and supported by the experiments
(Section~\ref{sec:exp_real_geom}).
...
$(2.3, 6.6)$; recomputed per seed, the verdicts are unanimous at the
default thresholds and invariant for a V1 threshold in $(0.70, 0.93)$ and
a V2 factor in $(2.3, 6.6)$---the narrower per-seed lower edge is set by
one MNIST seed with AU $= 14$ at $m_{\mathrm{ver}} = 20$
(Section~\ref{sec:exp}; the supplementary analysis script recomputes both
ranges).
...
\addlinespace
FC-VAE on MNIST (3 seeds) / Fashion-MNIST (Step 1: 3 seeds; Step 3: single seed) & Stable: $10.03 \pm 0.12$ / $\approx 10$ & \textbf{Pass} (V1/V2) on MNIST; Fashion-MNIST exploratory & Applicable (MNIST fully validated) \\
\addlinespace
Conv-VAE on MNIST & Stable (inflates while AU $= m$) & \textbf{Fail} at $\beta_{\max} = 4$ (V1); $\beta_{\max} \geq 10$ restores V1 but not V2 (single seed) & Applicable only with strong $\beta$ (exploratory) \\
\addlinespace
...
\textbf{All VAE verification results in the main text are multi-seed
experiments with at least 3 seeds (42, 123, 777)}; two reused training
sweeps are single-seed and are labeled as such where they appear (the
Step-1 reference-AE sweep of Table~\ref{tab:mnist_s1}, also re-analyzed in
Tables~\ref{tab:boot_ci} and \ref{tab:downstream}, and the AE side of the
downstream comparison), and single-seed exploratory observations are
separated into Appendix~\ref{app:exploratory}.
...
fourth row of Table~\ref{tab:failure}, the margin is enlarged and all
...
$[10, 24]$, which contains the best accuracy over the full grid; for the
VAE, $m = 20$ is in addition the pre-specified verification point
...
manifolds with known ground truth ($\AUsat = \dtrue$ or $\demb$ under
sufficient margin---$m \geq 5$ on the Torus---with the boundary
over-pruning at $m \leq 4$ detected by the MSE cross-check; 3--5 seeds,
...
\begin{table}[H]
\caption{Experiment overview. Main-text VAE verification results use 3 seeds
(42, 123, 777); single-seed sweeps are marked in the Seeds column; appendix
experiments marked ``single'' use seed 42 only.}
\label{tab:exp_list}
\small
