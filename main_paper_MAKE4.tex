...
%% Compile: pdflatex main_paper_MAKE4.tex
...
Effective Dimensionality via Active-Unit Dynamics and Intrinsic-Dimension
Estimates}
...
variational activity reflect different aspects of a learned representation.
This paper proposes a multi-indicator diagnostic framework for extracting
actionable knowledge from autoencoder latent spaces.
...
identification is not always possible and clarify the boundary conditions under
which AU/$\mknee$ diagnostics should be interpreted as inconclusive.
...
reproducible manner.
From the viewpoint of knowledge extraction, the objective of this study is not
merely to estimate a single latent dimensionality value.
Rather, we aim to extract model-design knowledge from trained autoencoder
representations.
The proposed diagnostics answer three practical questions: which latent
coordinates are actually used by the model; whether the geometry of the learned
representation is consistent with intrinsic-dimension estimates; and whether
increasing the nominal bottleneck dimension leads to meaningful representational
gains.
This knowledge is useful for selecting latent-space sizes, detecting
over-parameterized or under-regularized representations, and identifying cases
in which dimensionality diagnostics should be regarded as inconclusive.
In this sense, the proposed framework provides a knowledge-extraction protocol
for representation learning, rather than a standalone intrinsic-dimension
estimator.
...
intrinsic dimension from nearest-neighbour distances;
DANCo~\cite{ceruti2014danco} offers higher accuracy at greater computational cost.  Pope et
...
\subsection{Design Principle: Bottleneck Margin is a Major TwoNN Stabilizing Factor}
\label{sec:theory_dp}
...
$d_{\mathrm{emb}}=3$), the reconstruction MSE decreases substantially from
$m=1$ to $m=2$, and further at $m=3$ (Table~\ref{tab:e1}).
For Swiss Roll, the decrease from $m=1$ to $m=2$ is consistent with the idea
that a two-dimensional bottleneck is sufficient to represent this manifold in
practice.
For Torus, the largest decrease occurs at $m=3$, consistent with the
three-dimensional embedding requirement.
However, the anomalous increase at $m=4$ for Swiss Roll ($0.041\pm0.015$,
higher than at $m=2$) illustrates that MSE elbows are affected by
optimization variance, network capacity, and coordinate representation, not
only by intrinsic dimensionality.
In addition, simple connectedness alone does not guarantee that the embedding
...
direct estimate of intrinsic dimension (Appendix~\ref{app:proofs}).
...
Even with dense grids the AU curve fluctuates non-monotonically
...
Figure~\ref{fig:au_dynamics} visualises the sparse-grid instability and
dense-grid stabilization.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\linewidth]{results/figures_make/fig2_au_dynamics.pdf}
\caption{MNIST VAE AU dynamics.
\textbf{Left}: Sparse grid ($m\in\{4,8,\ldots,64\}$, 3 seeds, 300 epochs)---
$\mknee$ varies between 32 and 64 ($\sigma=15$).
\textbf{Right}: Dense grid ($m\in\{8,9,\ldots,64\}$, independent initialization,
3 seeds, 300 epochs)---$\mknee=10.3\pm0.9$, consistent with TwoNN ID ($\approx10$,
dashed red).
The non-monotone AU fluctuations visible in both panels confirm that $\mknee$
is an exploratory heuristic requiring controlled evaluation conditions.}
\label{fig:au_dynamics}
\end{figure}

%% -------------------------------------------------------------------
\subsection{\texorpdfstring{TwoNN Stability: Bottleneck Margin as a Major Factor}{TwoNN Stability: Bottleneck Margin as a Major Factor}}\label{sec:exp_stability}

Three complementary experiments support Design Principle~1 (Figure~\ref{fig:twonn_stab}).
...
(TwoNN: 10.17→10.98, $\Delta<0.82$; color-coded in Figure~\ref{fig:twonn_stab}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.65\linewidth]{results/figures_make/fig3_twonn_stability.pdf}
\caption{TwoNN ID vs.\ bottleneck dimension $m$ for MNIST AE
(color = $\log_{10}\kappa$, anisotropy indicator).
TwoNN stabilizes above $m\approx5\hatdID$ (vertical dotted line),
while $\kappa$ increases substantially beyond $m=64$ without affecting stability.
This supports Design Principle~1: bottleneck margin, not isometry, is the major
stabilizing factor in the tested settings.}
\label{fig:twonn_stab}
\end{figure}
...
Extending to 300 epochs with fixed $\beta$ or cyclic KL annealing~\cite{fu2019cyclical} does not
...
Figure~\ref{fig:applicability} provides a visual summary of the diagnostic
decisions across all tested settings.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\linewidth]{results/figures_make/fig4_applicability.pdf}
\caption{Applicability-boundary summary across all tested settings.
Green rows indicate identifiable conditions; yellow rows indicate conditional
or inconclusive results; red rows indicate boundary or unreliable regimes.
The framework does not only produce a dimension estimate---it also identifies
cases where the diagnosis should be treated as inconclusive, which is itself
useful knowledge for model selection.}
...
MNIST~\cite{lecun1998}, Fashion-MNIST~\cite{xiao2017fashion},
CIFAR-10~\cite{krizhevsky2009learning}, and SVHN~\cite{netzer2011reading}
are publicly available benchmark datasets.
The source code, configuration files, random seeds, trained-model logs, and
numerical results supporting all tables and supplementary analyses are provided
as supplementary material for peer review.
Upon acceptance, these materials will be deposited in a public repository
(GitHub or Zenodo), and the repository URL and DOI will be added to the final
version of the manuscript.
The repository will include the scripts required to reproduce the AU, TwoNN/MLE,
reconstruction-error, and grid-sensitivity analyses reported in this paper.
...
independently.  The activation threshold follows a two-case analysis:

\textbf{Case 1 ($\lambda_j \leq \beta\tau^2$, inactive).}
The water-filling solution $D_j^*=\beta\tau^2$ is infeasible
(requires $D_j^* \leq \lambda_j$), so the optimum is the inactive solution
with zero rate ($R_j=0$, $D_j=\lambda_j$).

\textbf{Case 2 ($\lambda_j > \beta\tau^2$, potentially active).}
The water-filling solution is feasible.
The gain from activating direction $j$ is
$\Delta L_j = L_j^{\mathrm{active}}-L_j^{\mathrm{dead}}
= (\beta/2)\,h(\lambda_j/\beta\tau^2)$
where $h(t)=t-1-\log t$.  Since $h(t)>0$ for all $t>0,\,t\neq1$, and here
$t=\lambda_j/\beta\tau^2>1$, we have $\Delta L_j>0$: activating is strictly
better than inactivating.

Combining both cases: direction $j$ is active if and only if $\lambda_j>\beta\tau^2$.
