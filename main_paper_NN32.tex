...
%% AE/VAEにおける有効潜在次元の診断的解釈（版31）
%% コンパイル: platex main_paper_NN31.tex && dvipdfmx main_paper_NN31.dvi
...
Active Unit（AU）Dynamics，内在次元（ID）推定（TwoNN/MLE），再構成誤差に基づく MSE エルボー，
および PCA を統合した多指標診断フレームワークを提案する．
本フレームワークの目的は，有効潜在次元を単一の数値として一意に決定することではない．
むしろ，AU 値を VAE が実際に利用する潜在座標数の直接観測指標，
TwoNN/MLE ID を数値的に安定な補助的参照推定量，
AU 鈍化点$\mknee$をグリッド・シード依存性をもつ探索的変化点として明確に区別し，
複数指標の整合性から潜在次元利用を診断する点に特徴がある．

理論面では，線形ガウス $\beta$-VAE の rate-distortion 近似のもとで，
データ共分散の第$j$固有方向が活性化される条件$\lambda_j > \beta\tau^2$を導出し，
KL 正則化が有効潜在次元を抑制する機構を水充填原理の観点から説明する．
ただし，本解析は非線形 VAE に対する厳密定理ではなく，
実験結果を解釈するための線形近似モデルとして位置づける．

実験では，MNIST の全結合 VAE において AU 値と TwoNN ID が複数シードにわたり
高い再現性を示す一方，$\mknee$は疎グリッドでは大きく変動することを確認した．
推定 ID 近傍を密に探索し，独立初期化・複数シードで評価することで
$\mknee = 10.3 \pm 0.9$ と TwoNN ID 近傍に安定化することを示す．
さらに Fashion-MNIST，Conv-VAE，CIFAR-10/SVHN 実験を通じて，
本診断フレームワークが有効に機能する条件と，
AU/$\mknee$ 診断が困難となる境界条件を定量化する．
\end{abstract}
...
\multicolumn{4}{l}{平均$\mknee = 53 \pm 15$（150 エポック）} \\
\multicolumn{4}{l}{主結果は密グリッド・独立初期化の Exp-Mnist-Dense-300: $\mknee = 10.3 \pm 0.9$} \\
\bottomrule
\end{tabular}
\end{table}
...
（平均$53 \pm 15$）．
これは，正規化増加率$\rho(m)$が閾値 $\tau = 0.25$ に近い複数の$m$値で微妙に変動するためであり，
...
$\mknee = 10.3 \pm 0.9$ を主結果として採用する（実験設定の詳細は§\ref{sec:exp_el}）．
...
\begin{itemize}
  ...
  \item \textbf{グリッド密度因子（主要因の一つ）}：密グリッド（Exp-Mnist-Dense）では$\mknee = 10.3 \pm 0.9$（シード別に 11, 9, 11）—— 疎グリッド$43\pm15$から劇的に改善し，推定$\hat{d}_\mathrm{ID} \approx 10$と整合する
\end{itemize}
...
Fashion-MNIST（$\hat{d}_\mathrm{ID}=9$）でも同様のパターンが観測された（詳細表は\ref{app:twonn_detail}）．
...
\begin{itemize}
  ...
  \item \textbf{主要実験の多シード検証}：
        ...
        いずれも単一シードのみであった．
        本稿では Exp-Mnist-Multi-150（150 エポック，3シード；§\ref{sec:exp_multiseed}）および
        Exp-Mnist-Multi-300（300 エポック，3シード；§\ref{sec:exp_multiseed2}）を追加した．
...
\begin{itemize}
  \item \textbf{Stage 1 の ID 推定：等長性バイアスの厳格な評価}：
        ...
        （Exp-Stab・Exp-Iso-ID での$\kap \in [1.06, 2.16]$での等長性改善が TwoNN に有意な影響を与えないことも，$m \gg d_\mathrm{ID}$という運用条件の重要性を裏付ける；§\ref{sec:exp_ep}）．
...
（300 エポック多シード実験 Exp-Mnist-Multi-300 および密グリッド因子分離実験 Exp-Mnist-Dense-300；
§\ref{sec:exp_multiseed2}，§\ref{sec:exp_el}）．
...
CIFAR-10 では Pope ら\cite{pope2021intrinsic}の報告値（$\approx 26$）と整合する推定値を得た
（CIFAR-10 Conv-VAE 実験 Exp-Nat-Cifar）；
