%% main_paper_MAKE2.tex
%% MDPI MAKE (Machine Learning and Knowledge Extraction) submission
%% Revised from MAKE1: see change log below
%% NOTE for submission: replace \documentclass with MDPI's mdpi.cls:
%%   \documentclass[make,article,submit,pdftex,moreauthors]{Definitions/mdpi}
%% Compile: pdflatex main_paper_MAKE2.tex
...
\usepackage{lmodern}
\usepackage{graphicx}  % pdflatex-compatible (dvipdfmx removed)
\usepackage{booktabs}
\usepackage{url}
...
\newtheorem{analytical}[theorem]{Analytical Result}
...
conditions under which AU/$\mknee$ identification fails;
Fashion-MNIST is used as a preliminary external check only. These results support the proposed
...
clustering, and visualisation~\cite{bengio2013representation}.
...
dimension, and reconstruction-side behaviour is therefore directly useful for
...
shows a sharp elbow at $m=d_{\mathrm{true}}=2$ (Swiss Roll) and
$m=d_{\mathrm{emb}}=3$ (Torus).
This is explained by the topological embedding dimension: although both manifolds
have intrinsic dimension~2, the Torus cannot be embedded in $\R^2$ without
self-intersections, whereas a standard smooth embedding requires $\R^3$.
The reconstruction lower bound therefore depends on~$d_{\mathrm{emb}}$, not
only~$\dID$ (Appendix~\ref{app:proofs}).
...
optimisation variance when the extra dimension is unused: higher std indicates
...
4 & $0.041\pm0.015$ & $0.019\pm0.002$ & high std: optimisation variance \\
\bottomrule
\end{tabular}
\end{table}
...
consistent with non-linear stochastic optimisation (e.g., AU$=29$ at $m=64$
but AU$=26$ at $m=128$).
...
result}; the main stabilised result is $\mknee=10.3\pm0.9$ from dense-grid,
...
TwoNN stabilisation is driven by $m/\hatdID$ across all four models; inter-model
...
regularisation and decoder-capacity control than the tested settings provide.
...
Conceptualisation, (Author Initials); Methodology, (A.I.);
Software, (A.I.); Validation, (A.I.) and (Co-author Initials);
Formal Analysis, (A.I.); Investigation, (A.I.);
Writing---Original Draft Preparation, (A.I.);
Writing---Review \& Editing, (A.I.) and (C.I.);
Supervision, (C.I.); Project Administration, (C.I.).
All authors have read and agreed to the published version of the manuscript.
\emph{(Please fill in initials before submission.  Use CRediT taxonomy:
...
public repository (GitHub/Zenodo) upon publication.
...
($m\in\{1,4,8,\ldots,256\}$) and initialisation, and is \emph{not} used as a
main result.
...
reflect stochastic optimisation, not a stable signal.
The main stabilised result is $\mknee=10.3\pm0.9$ (Table~\ref{tab:el_mknee}).}
\label{tab:e4_vae}
\small
\begin{tabular}{rcccc}
\toprule
