...
\begin{enumerate}
  \item \textbf{頑健指標 AU 値の実証的確認，および TwoNN ID の数値安定性確認}：
        ...
        TwoNN 推定の\textbf{外部妥当性}を確認した（§\ref{sec:exp_natural}）——ただし，自然画像での AU/mknee 側の検証は
        アーキテクチャ依存性が高く，今回のグリッド範囲内では AU 飽和プラトーは未到達である．
...
\begin{table}[H]
\centering
\caption{実験一覧：各実験の目的・データ・位置づけ}
\label{tab:exp_list}
\small
\begin{tabular}{llll}
\toprule
実験ラベル & 名称・目的 & データ/設定 & 位置づけ \\
\midrule
Exp-Syn-1 & MSE 肘点・ID 確認 & 合成多様体 & 原理確認 \\
Exp-Syn-AU & AU 飽和（AE vs VAE） & 合成多様体 & 原理確認 \\
Exp-Syn-Top & 位相的埋め込み次元と AU & Torus/Möbius 等 & 探索的 \\
Exp-Syn-Iso & IsometricAE・等長性検証 & 合成多様体 & 副次 \\
Exp-Stab & Stage 1 TwoNN 安定性の実証（新規） & Swiss Roll + MNIST (Exp-Mnist-Base参照) & Stage 1 妥当性 \\
Exp-Mnist-Base & MNIST VAE スイープ（主実験） & MNIST FC-VAE, $m=1$〜256 & 主要 \\
Exp-Mnist-Multi & MNIST 多シード検証 & 3 シード×150/300ep & 主要 \\
Exp-Mnist-PCA & 理論-実験橋渡し（PCA スペクトル分析） & MNIST PCA+Exp-Mnist-Base 対比 & 理論補足 \\
Exp-Mnist-Dense & 密グリッド検証（3因子分離） & MNIST FC-VAE, $m=8$〜64 密 & mknee 安定化 \\
Exp-Fashion & Fashion-MNIST 適用検証 & Fashion-MNIST FC-VAE & 異種実データ \\
Exp-Conv-AE & 畳み込み AE での ID 整合性確認 & MNIST Conv-AE & アーキテクチャ拡張 \\
Exp-Conv-Fix & 固定$\beta$ Conv-VAE（未飽和の確認） & MNIST Conv-VAE & 限界確認 \\
Exp-Conv-Ann & KL アニーリング Conv-VAE & MNIST Conv-VAE & 安定化探索 \\
Exp-Conv-M256 & Conv-VAE 拡張 $m$ グリッド（新規） & MNIST Conv-VAE, $m=4$〜256 & Conv-VAE 飽和探索 \\
Exp-Conv-M512 & Conv-VAE $m=384$・512 拡張 & MNIST Conv-VAE, 3シード & Conv-VAE 飽和限界確認 \\
Exp-ConvV-HiBeta & 高$\beta$による AU 飽和の人為的誘発 & MNIST Conv-VAE, $\beta_{\max}\in\{10,20\}$ & Conv-VAE 飽和メカニズム検証 \\
Exp-Geom & MNIST AE $m$スイープ：$\kappa$・TwoNN 共変化 & MNIST AE, $m=4$〜128 & Stage 1 幾何的妥当性 \\
Exp-Nat-Cifar & CIFAR-10 TwoNN 外部妥当性確認 & CIFAR-10 Conv-VAE & 外部妥当性 \\
Exp-Nat-SVHN & SVHN 部分崩壊モニタリング & SVHN Conv-VAE & 外部妥当性 \\
Exp-Mnist-Dyn & MNIST 学習ダイナミクス & MNIST FC-VAE, $m=16$ & 付随・単一シード \\
Exp-Mnist-Ent & Manifold Entanglement 予備検証 & MNIST クラス制限 & 付随・予備的 \\
Exp-Tau-Cross & $\tau_\mathrm{knee}$横断検証 & 複数データセット & 付録（感度分析） \\
\bottomrule
\end{tabular}
\end{table}
...
RQ & 主要実験 & 対応する命題 \\
\midrule
RQ1（$\mknee$の有用性と安定化条件） & MNIST 主実験(Exp-Mnist-Base), 多シード検証(Exp-Mnist-Multi), & \\
 & 密グリッド検証(Exp-Mnist-Dense), Fashion-MNIST 適用(Exp-Fashion) & 命題\ref{thm:lower_bound}，定理\ref{thm:linear_vae}，命題\ref{thm:mknee_instab} \\
RQ2（多指標フレームワーク安定性） & 原理確認(Exp-Syn-1,Exp-Syn-AU,Exp-Syn-Top), MNIST 多シード(Exp-Mnist-Multi) & 表\ref{tab:indicator_class} \\
RQ3（等長性） & IsometricAE 実験(Exp-Syn-Iso) & 命題\ref{thm:pullback} \\
理論-実験橋渡し & PCA スペクトル分析(Exp-Mnist-PCA) & 定理\ref{thm:linear_vae} \\
付随（学習ダイナミクス） & 学習ダイナミクス(Exp-Mnist-Dyn), Manifold Entanglement 予備(Exp-Mnist-Ent) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
グリッド密度を変えた Exp-Mnist-Dense 実験を実施した．
Exp-Mnist-Base と Exp-Mnist-Multi では既にエポック因子（150ep vs. 300ep）とシード因子（3シード）が分離されている．
Exp-Mnist-Dense 実験はグリッド密度因子を単離する：$m \in \{8,9,10,11,12,13,14,15,16,18,20,24,28,32,40,48,64\}$
...
\begin{itemize}
  ...
  \item \textbf{数値的安定性は不支持}：MNIST 多シード実験（150・300 エポック）では$\mknee$が 32〜64 に変動し，
        数値的再現性は支持されなかった．密グリッド実験（Exp-Mnist-Dense）の結果から，
        $m$グリッドの設計が不安定性の\textbf{主要因の一つ}と考えられるが，
        シード（初期化）による変動も残存しており，単一の要因に帰着するものではない．
\end{itemize}
...
のが本研究の正直な結論であり，多指標フレームワークの AU 値の頑健性と TwoNN ID の数値安定性が
本研究の主要な貢献となる．
...
（§\ref{sec:disc_incidental}で触れる IsometricAE は理論的精度改善の一方向だが，本稿の主評価指標は AU である）．
...
\paragraph{幾何的正則化（実験 Exp-Syn-Iso）}
IsometricAE（$\kap \approx 1.04$）は潜在空間での ID 推定の前提条件（等長性）を実現できることを示した．
...
$m=512$まで拡張しても明確な AU 飽和プラトーは観測されなかった（$\mknee$はグリッド端の$m=512$に張り付く，$\beta_{\max}=4$）．
...
AU/mknee 側の安定同定には至っていない（§\ref{sec:exp_natural}）．
自然画像は$d_\mathrm{ID} \approx 26$（CIFAR-10）と MNIST より実質的に高次元であり，
より深い Conv-VAE アーキテクチャと広い$m$グリッド（例：$m \in [64, 512]$）が必要である．
これらの系統的検証は今後の重要課題である．
...
\begin{itemize}
  \item \textbf{Conv-AE・Conv-VAE 検証}（実験 Exp-Conv-AE・Exp-Conv-Fix・Exp-Conv-Ann・Exp-Conv-M256）：
        ...
        Exp-Conv-Ann（KL アニーリング）でも$m\in[4,64]$内では明確な AU 飽和プラトーは観測されなかった．
        Exp-Conv-M256・Exp-Conv-M512 実験（§\ref{sec:exp_en}）により拡張$m$グリッドでの定性的傾向は確認されたが，
        $m=512$まで拡張しても AU 飽和プラトーは観測されず，
        より深い Conv-VAE アーキテクチャと広い$m$グリッドでの系統的検証が課題として残る．
