...
% TODO(text): 以下の survey 段落
%   - 次元削減: PCA / Isomap / LLE を背景として（R1-6）
%   - ID 推定: TwoNN, MLE, DANCo, graph-based (Costa & Hero)。
%     指定 DOI の MST 論文は異常検知が主題のため次元削減法として紹介しない
%   - 次元選択: FONDUE (Alg.1), FONDUE-VAR (Alg.3, Appendix G)
%   - 剪定: GECO + L0, ARD-VAE, MaskAAE
%   - 事後崩壊と active units の系譜
%   - 先行会議論文 NOLTA2025 との 3 列差分（会議版 / 既存 / 本改訂）
...
% TODO(text): アーキテクチャ、候補グリッド、比較法 B1-B9 と提案法・昇順走査の定義表、
%             B7/B8 の原著からの逸脱（hard concrete / Gaussian 近似）を明記
...
% TODO(text): 数値は Table D2 / MAKE49_段階4_主比較結果.md §2.1, §4 から転記
...
% TODO(text): 数値は MAKE49_段階5_結果.md E3-1〜E3-5 から転記
...
% TODO(text): 数値は Table F から転記。4 データ揃い次第確定
...
% TODO(text): 数値は MAKE49_段階3_E7a_E2結果.md, 段階5 E4-1/E4-5 から転記
...
% TODO(text): 数値は MAKE49_段階5_結果.md E5-1〜E5-4 から転記
...
% TODO(text): 数値は MAKE49_段階1_監査結果.md A1, 段階7 §1, 段階4 §2.6 から転記
