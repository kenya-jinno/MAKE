...
TwoNN ID 推定（数値安定な補助的参照推定量）の外部妥当性確認と，AU モニタリングの基礎的有用性の確認にある．
...
\begin{itemize}
  ...
  \item \textbf{SVHN の部分的崩壊の AU モニタリング（EJ）}：
        ...
        整合する部分崩壊が観測され，AU 監視によるモデル診断の有用性を示す基礎的確認を得た．
\end{itemize}
...
\textbf{本フレームワークの適用範囲と限界の整理}：

本稿では特に以下の点について，適用条件と現状の限界を明示する．

\begin{description}
  \item[FC-VAE と MNIST/Fashion-MNIST（安定動作域）]
        AU 値・TwoNN ID の頑健性，$\mknee$の安定化条件，線形理論との定性的対応は，
        FC-VAE と MNIST/Fashion-MNIST において最もよく検証されており，
        本フレームワークの主要な適用域である．
  \item[Conv-VAE（部分的・条件付き）]
        Conv-VAE では，固定$\beta$の場合は 300 エポックでも $m \leq 32$ で AU$\approx m$ が続き（実験 EH），
        KL アニーリング（EK）でも MNIST の tested range（$m \leq 64$）内では AU 飽和プラトーは観測されなかった．
        これは Conv-VAE の実効 $d^*(\beta)$ が FC-VAE より高次元であることを示唆する．
        Conv-AE での TwoNN ID は FC-AE と整合する（実験 EG；TwoNN: 11.29 vs 10.02）が，
        AU Dynamics を用いた$\mknee$の安定検出にはより大きな$m$グリッドが必要であり，
        Conv-VAE でのフレームワーク完全適用は今後の課題である．
  \item[自然画像（TwoNN 外部妥当性の確認に限定）]
        CIFAR-10・SVHN では TwoNN 推定の外部妥当性（Pope らの報告値との整合）と
        AU モニタリングによる部分崩壊検出は確認されたが，
        AU 飽和に基づく$\mknee$・$\AUsat$の同定には至っていない．
        自然画像でのフレームワーク全体の適用性確認は将来課題である．
  \item[Stage 1 の$\hat{d}_\mathrm{ID}$（補助的参照推定量として）]
        Stage 1 で得られる$\hat{d}_\mathrm{ID}$は参照 AE（$m=256$）の非等長性（$\kap \approx 10^9$）に
        由来するバイアスを含む近似値であり，主要な有効次元の読み取りは Stage 2 に委ねる設計としている．
        等長性バイアスの定量的解消には IsometricAE 等の等長的アーキテクチャの活用が必要である．
\end{description}
