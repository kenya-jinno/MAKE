...
MSE エルボー・PCA を組み合わせた\textbf{多指標分析フレームワーク}を提案する．
本フレームワークの中核的価値は，各指標が持つ頑健性と探索的性格を明示した上で
相補的に組み合わせる点にあり，単一指標への過信が招く誤診断を体系的に回避できる．
理論面では，線形$\beta$-VAE において AU 飽和しきい値を$\lambda_j > \beta\tau^2$として
厳密に証明し（定理\ref{thm:linear_vae}），$\mknee$のグリッド感度についても形式的に解析する
（命題\ref{thm:mknee_instab}）．

主要な実験的知見（合成多様体・MNIST・Fashion-MNIST・CIFAR-10・SVHN）：
\begin{enumerate}
  \item \textbf{頑健な指標：AU 値と TwoNN ID}：
        AU 値・TwoNN ID は3シード・複数データセットにわたって高い再現性を示す
        （MNIST$\sigma \leq 0.5$，$m=64$ TwoNN ID$= 10.03 \pm 0.12$）．
        自然画像（CIFAR-10・SVHN）では，本フレームワークの TwoNN ID 推定が
        Pope ら\cite{pope2021intrinsic}の報告値（CIFAR-10: $d_\mathrm{ID} \approx 26$）と整合し，
        フレームワークの汎化性を実証する（§\ref{sec:exp_natural}）．
  \item \textbf{KL アニーリングによる Conv-VAE AU 動態の変化}：
        固定$\beta$では$m=64$で約10次元が崩壊（AU$=54.3$）するのに対し，
        サイクリック KL アニーリング\cite{fu2019cyclical}導入により崩壊が抑制（AU$=64$）される
        ことを実験的に示す（§\ref{sec:exp_annealing}）．
        いずれも$m \in [4,64]$内では AU 飽和プラトーは観測されず，
        Conv-VAE の AU 飽和観測には$m > 64$が必要であることを示す．
  \item \textbf{$m^{\mathrm{knee}}$の有用性と安定化条件}：
        Fashion-MNIST では$m^{\mathrm{knee}} = 12$が$\hat{d}_\mathrm{ID} \approx 9$〜10 と対応し（RQ1 部分的支持），
        MNIST 多シード実験では$\mknee \in \{32, 64\}$に変動（数値的安定性：不支持）．
        命題\ref{thm:mknee_instab}はこの不安定性の理論的根拠を与え，
        アンサンブル化による安定化戦略を提案する（§\ref{sec:disc_mknee_improve}）．
\end{enumerate}

本論文の貢献は，「単一指標依存から多指標フレームワークへ」という方法論的視点の提案，
AU 飽和の理論的解析，および有効次元読み取りの実用的ガイドラインの確立にある．
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{AU 鈍化点$\mknee$の定式化と探索的検証}（補助的貢献）：
        ...
        $\mknee$の不安定性の定量的解明（命題\ref{thm:mknee_instab}）と
        KL アニーリングによる安定化（実験 EK；§\ref{sec:exp_annealing}）が本貢献に含まれる．
\end{enumerate}
...
RQ & 実験 & 対応する命題 \\
\midrule
RQ1（$\mknee$と ID 整合性） & E4, E4-multi, EF, EG & 命題\ref{thm:lower_bound}，定理\ref{thm:linear_vae} \\
...
\textbf{自然画像実験（EI・EJ）の主要知見}：
表\ref{tab:ei_ej}から次の3点が観測される．

第1に，CIFAR-10 実験（EI）では$m=64$における潜在 TwoNN ID が$25.21\pm0.41$となり，
生データ TwoNN ID（25.20）と一致した．
これは Pope ら\cite{pope2021intrinsic}の報告値（$d_\mathrm{ID} \approx 26$）の
\textbf{独立した再現}であり，本フレームワークの TwoNN ID 推定の外部妥当性を示す．
CIFAR-10 では AU$=m$が$m \leq 256$の全範囲で維持され，AU 飽和には$m > 256$が必要と考えられる．

第2に，SVHN 実験（EJ）では$m \geq 128$で AU$<m$の部分的崩壊が観測された
（$m=128$: AU$=125.3\pm0.5$，$m=256$: AU$=206.7\pm1.2$）．
SVHN の生データ TwoNN ID（10.09）は CIFAR-10（25.20）より低く，
SVHN の方が相対的に低次元な潜在表現を要求することと整合する．

第3に，両データセットとも$m \in [16,256]$内では $\tau=0.25$基準による$\mknee$検出には至らなかった（全シード$\mknee=256$）．
SVHN では$\tau$を小さく設定するか，$m$グリッドを拡張することで$\mknee$が特定できる可能性がある．
...
本研究の知見から，$\mknee$を実際の設計に用いる際の以下の推奨事項を提案する：
\begin{enumerate}
  \item \textbf{密なグリッド設計}：推定 $\hat{d}_\mathrm{ID}$（TwoNN ID から得られる）
        の前後に密な点を配置する（例：ID$\approx 26$なら$m \in \{20, 24, 28, 32, 36\}$を含む）．
...
\begin{itemize}
  \item \textbf{参照 AE の等長性バイアスと TwoNN ID への影響}：
        命題\ref{thm:pullback}より，等長性$(\kap = 1)$は ID 推定精度の理論的前提であるが，
        $m = 256$ の参照 AE の条件数$\kap \approx 10^9$は等長性からの大きな逸脱を示す．
        実験 EH では，Conv-VAE の AU 未飽和時（AU$\approx m$）の TwoNN ID（13.10）が
        FC-VAE の AU 飽和時（10.03）より大幅に膨張しており，
        \textbf{等長性バイアスは KL 飽和不足と相乗的に ID 推定値を歪める}ことが示された．
        IsometricAE（実験 EC）による$\kap \approx 1$の達成，または
        複数$m$での ID 収束確認（$m$増加に対して TwoNN ID が安定する点の確認）が実用的な緩和策である．
        ランダム射影との比較による歪みの定量的評価は今後の課題である．
...
\textbf{主要知見のまとめ}：
\begin{enumerate}
  \item \textbf{多指標フレームワークの有用性と頑健性}：
        TwoNN/MLE ID，AU 値，MSE エルボー，PCA の相補的活用により，
        単一指標では得られない信頼性の高い有効次元解釈が可能である．
        特に AU 値（$\sigma \leq 0.5$）・TwoNN ID（MNIST $m=64$: $10.03 \pm 0.12$）は
        3シード・300 エポックにわたって高再現性を示す（§\ref{sec:exp_multiseed2}）．
  \item \textbf{定理\ref{thm:linear_vae}と実験の整合}：
        線形$\beta$-VAE の AU 飽和しきい値$\lambda_j > \beta\tau^2$は，
        KL 正則化のない Vanilla AE が AU$=m$（全次元活性）となる実験的観測（実験 EA）と整合する．
        実験 EK では，固定$\beta=4$では$m=64$で部分的崩壊（AU=54.3），
        サイクリックアニーリングでは AU=64（崩壊抑制）となり，
        アニーリングが AU 動態に異なる影響を与えることを示す．
        いずれも MNIST Conv-VAE での$m \in [4,64]$内の明確な飽和プラトーは観測されず，
        Conv-VAE の有効$d^*(\beta)$が FC-VAE より高次元であることが示唆される．
  \item \textbf{自然画像への汎化}：
        CIFAR-10・SVHN への適用（実験 EI・EJ，§\ref{sec:exp_natural}）において，
        本フレームワークの TwoNN ID 推定が Pope ら\cite{pope2021intrinsic}の報告値
        （CIFAR-10: $d_\mathrm{ID} \approx 26$）と整合することで，
        本フレームワークの自然画像への汎化性が実証的に支持される．
  \item \textbf{$\mknee$の探索的有用性と安定化戦略}：
        Fashion-MNIST では$\mknee = 12$が$\hat{d}_\mathrm{ID} \approx 9$〜10 と対応し（部分的支持）．
        MNIST では$\mknee \in \{32, 64\}$と変動（数値的安定性：不支持）．
        命題\ref{thm:mknee_instab}による理論的根拠と，
        アンサンブル化・密グリッド・KL アニーリングによる実践的安定化戦略を提示した
        （§\ref{sec:disc_mknee_improve}）．
\end{enumerate}

副次的知見（探索的・条件付き）：
位相的に非自明な合成多様体での$\AUsat \approx \demb$，
MNIST での大域・局所構造形成の時間的分離，
IsometricAE による$\kap \approx 1$の等長潜在空間の実現，
Conv-AE での TwoNN ID の全結合型との整合性，
および CIFAR-10 実験（EI）での潜在 TwoNN ID が$m=64$で生データ TwoNN ID（25.20）と一致し
Pope ら\cite{pope2021intrinsic}の報告値を独立に再現した点．

\textbf{今後の課題}：
$\mknee$安定化の定量的検証（密グリッド・アンサンブル），
ImageNet 等の大規模データへの拡張，
線形 VAE の定理の非線形設定への厳密な拡張，
相互情報量・レート歪み曲線との定量比較，
および$\tau_{\mathrm{knee}}$の曲率に基づく自動選択が今後の課題である．
