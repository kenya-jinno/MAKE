...
%  MDPI MAKE submission — main_paper_MAKE37.tex
%  English version of main_paper_NN37.tex (Japanese draft, version 37)
%  Version 37: revisions in response to Review_codex36.txt (Minor revision;
%  Major 1--7, Minor 1--10)
%    - Torus m=3--5 over-pruning (AU=2 < d_emb) confirmed by the new experiment
%      Exp-Syn-Verify (5 seeds); the incorrect "AUsat=demb for all m > demb"
%      statement fixed; MSE cross-check shown to detect the boundary; V1/V2
%      limitation (not a test of AU=demb) discussed (Major 1)
%    - verification-point (m_ver) pre-specification rule defined; per-seed
%      verdicts (unanimous) and threshold-invariance ranges (V1: 0.67--0.94,
%      V2: 2.3--6.6) added; V1/V2 stated as a conservative reporting convention,
%      not a validated statistical test (Major 2, Minor 3)
%    - [dhat, 2dhat] renamed "default search window"; generality softened (Major 3)
%    - bridge between surrogate rate activation and the AU variance threshold
%      added to the theory section (Major 4)
%    - Step-1 validity wording softened (reference-AE route = pragmatic
%      stabilization; PCA = illustrative contrast); DANCo-784 degeneration
%      explained (Major 5, Minor 6, 7)
%    - search-cost savings scoped to "the MNIST grid used in this paper";
%      symmetric Step-1 accounting added (Major 6, Minor 5)
%    - compactness / topological-embedding assumptions made explicit in
%      Proposition 1 (Major 7)
%    - abstract qualifiers reorganized; kappa caution; other minor fixes
%      (Minor 1, 4, 8)
%  Compiled with official mdpi.cls
%  Compile: pdflatex main_paper_MAKE37.tex  (x3 for cross-references)
...
setting, and verification---in which \textbf{a search window for $m$ is
...
on MNIST (3+ seeds each), the three steps operate under a reproducible
decision rule (Swiss Roll: $\AUsat = \dtrue = 2$; Torus:
$\AUsat = \demb = 3$ for $m \geq 2\demb$, with boundary over-pruning below
$2\demb$ detected by the MSE cross-check; MNIST:
$\hatdID = 10.03 \pm 0.12$ consistent with the AU slowdown region), and on
the MNIST grid used in this paper the ID-guided search window reduces the
training runs of a full grid search by roughly 60--80\% while attaining the
grid-best downstream linear-probe accuracy inside the window.
...
the AU truncation fails at $\beta = 4$ ($\beta$-strengthening restores it
only in single-seed exploratory runs).
The protocol is thus not a general solution: \textbf{$\hatdID$ provides a
search window, and AU diagnoses the success or failure of truncation under
...
\begin{quote}
\textbf{A search window for the bottleneck dimension $m$ can be derived
...
\begin{enumerate}
  ...
  \item \textbf{Multi-seed validation and quantified applicability
        ...
        $\demb$ under sufficient margin, with the Torus boundary
        over-pruning detected by the MSE cross-check; 3--5 seeds) and on
        fully connected VAEs on MNIST
...
activation stabilizes only at $m \geq 2\demb = 3\hatdID$;
...
smallest tested grid point $\geq 2\hatdID$, before the AU curve is
observed} (to avoid post-hoc selection; checking several
$m_{\mathrm{ver}}$ is recommended).
...
to the same order as the manifold dimension of the data);
...
(Table~\ref{tab:failure}).
V1/V2 is \textbf{a conservative reporting convention, not a validated
statistical test}: the threshold $0.9$ and factor $3$ are working defaults,
and in all settings of this paper the verdicts are invariant for a V1
threshold in $(0.67, 0.94)$ and a V2 factor in $(2.3, 6.6)$, with per-seed
verdicts unanimous in every multi-seed setting (Section~\ref{sec:exp}; the
supplementary analysis script recomputes these).
...
($\demb > \dID$) and enlarge the margin (fourth row of
...
The proposition is a necessary condition under the idealization of zero
error and continuity; it does not directly apply to real neural AEs with
finite error, noise, or non-injective behavior on small sets, and we use it
only as an approximate guide for the lower end.
...
phenomenon that reproduces perfectly across seeds}: in a fine-grid
confirmation experiment ($m \in \{3, 4, 5, 6, 8\}$, 5 seeds;
Exp-Syn-Verify), all seeds give AU $= 2$ for $m \in \{3, 4\}$ and
AU $= 3$ for $m \geq 5$---the transition at $m = 5$ is deterministic.
...
pushed into a self-intersecting two-dimensional representation (an
immersion)---\textbf{the activation of the topologically required dimension
...
fourth row of Table~\ref{tab:failure} (topological obstruction) fires and
...
improvement beyond $m_{\mathrm{ver}}$)---following the fourth row of
Table~\ref{tab:failure}, the margin is enlarged and all checks pass at
$m = 6$ (\textbf{pass}; $\AUsat = 3 = \demb$, stable for all larger $m$;
3--5 seeds, $\sigma = 0$).
...
From $\hatdID \approx 10$--$12$, the search window is $m \in [10, 24]$
...
\textbf{Verdict} (Step-3 rule; $m_{\mathrm{ver}} = 20$ pre-specified as the
smallest tested grid point $\geq 2\hatdID$): AU $= 13.3 \pm 0.5$ with
...
attained inside the ID-guided search window $[10, 24]$ (at $m = 12$)},
...
search window, and is nearly flat for $m \geq 12$
...
stronger than consistency can be claimed; third failure mode of
Table~\ref{tab:failure}).
...
\textbf{$\hatdID$ provides a search window, and AU diagnoses whether the
...
each protocol step;
experimentally, the full protocol operated as designed on synthetic
manifolds with known ground truth ($\AUsat = \dtrue$ or $\demb$ for
$m \geq 2\demb$, with the Torus boundary over-pruning detected by the MSE
cross-check; 3--5 seeds, $\sigma = 0$) and on fully connected VAEs on MNIST
($\hatdID = 10.03 \pm 0.12$ consistent with the AU slowdown region;
3 seeds), with the ID-guided window reducing the training runs of the full
grid search used in this paper by roughly 60--80\% while retaining
