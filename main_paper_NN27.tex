...
%% AE/VAEにおける有効潜在次元の診断的解釈（版27）
%% コンパイル: platex main_paper_NN27.tex && dvipdfmx main_paper_NN27.dvi
...
\begin{enumerate}
  \item \textbf{有効次元の多指標診断フレームワーク}（主要貢献）：
        ...
        TwoNN 推定が自然画像でも数値的に安定することを確認する（§\ref{sec:exp_natural}）．
...
\begin{description}
  ...
  \item[RQ3] 幾何的正則化（IsometricAE）やボトルネック余裕（$m / \hat{d}_\mathrm{ID}$ の大小）は，
             ...
             合成多様体・実データで実証した；§\ref{sec:exp_ep}，§\ref{sec:exp_ep}，§\ref{sec:exp_real_geom}．）
\end{description}
...
\subsubsection{TwoNN ID の数値安定性と$\kap$の影響（実験 Exp-Stab・Exp-Geom・Exp-Iso-ID，詳細は付録 D）}\label{sec:exp_ep}
...
（詳細表は付録 D，表\ref{tab:eq_mnist}）．
Exp-Iso-ID（Swiss Roll，$m \in \{2,3,4,6,8\}$）では，$m \geq 3$で$\kap \in [1.1, 2.1]$の差があっても
TwoNN ID の差は$\pm2.3\%$以内にとどまった（詳細表は付録 D，表\ref{tab:iso_id}）．
...
なお，Exp-Mnist-Base 拡張実験（単一シード 42，$m \in \{1, 4, 8, \ldots, 256\}$）での$\mknee = 12$について補足する．
本多シード実験とは $m$ グリッドが異なるため，各$m$をループ処理する順序が変わり，
乱数状態（PyTorch global seed）の遷移経路が異なる可能性が高い
（各条件で $\texttt{torch.manual\_seed}(s)$ を冒頭でのみ設定した場合，ループ順の違いが以降の状態を変える）．
実験間の再現性を厳密に担保するためには，各$(m, \mathrm{seed})$の組ごとに独立に乱数を初期化する実装が必要であり，
Exp-Mnist-Base の $\mknee=12$ は予備的観察として扱うのが適切である．
Exp-Mnist-Dense（§\ref{sec:exp_el}）は各条件を独立初期化して設計しており，
得られた $\mknee = 10.3 \pm 0.9$（3シード）を本論文の主要な密グリッド結果とする．
\textbf{$\mknee$は AU 増加曲線の形状から導かれる探索的ヒューリスティックであり，
その精度は実験設定（$m$グリッドの選択・乱数シード・初期化の実装）に依存することを本実験は示す．}

\subsubsection{線形理論―実験橋渡し（実験 Exp-Mnist-PCA，詳細は付録 D）}\label{sec:exp_em}
...
$\mknee$の疎グリッドでの不安定性が理論的に裏付けられる（詳細な PCA スペクトル表・理論-実験比較表は付録 D）．
...
本実験の主眼は TwoNN ID が自然画像でも数値的に安定するかの確認に限定される（AU/$\mknee$ 側の安定同定は将来課題）．

\textbf{CIFAR-10（Exp-Nat-Cifar）— TwoNN の数値安定性確認}：
$m=64$での潜在 TwoNN ID$=25.21\pm0.41$（3シード，$\sigma=0.41$）は
生データ TwoNN ID（25.20）と整合し，Pope ら\cite{pope2021intrinsic}の報告値（$\approx 26$）と
同程度のスケールの推定値が得られることを確認した
（Exp-Geom 実験の MNIST 同様，$m \gg d_\mathrm{ID}$条件での数値安定性を支持）．
...
\textbf{（1）高$\beta$・薄型デコーダ条件での AU plateau-like behavior の観察（本研究条件下）}：
...
という本研究条件下での予備的証拠が得られた（mechanistic validation の一歩）．
...
という解釈と整合する．Conv-VAE への適用では，$\beta$をより大きく設定するか，
...
AU plateau-like behavior を本研究条件下で観察した（stable identification 5条件は全て未達；付録 C）．
