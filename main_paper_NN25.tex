...
%% AE/VAEにおける有効潜在次元の診断的解釈（版25）
%% コンパイル: platex main_paper_NN25.tex && dvipdfmx main_paper_NN25.dvi
...
\midrule
AU values ($\mathrm{AU}(m)$) & \textbf{Robust indicator} & $\sigma \leq 0.5$ (MNIST 300ep) & Direct observation of effective dimensionality \\
...
\end{tabular}
\caption{Diagnostic pipeline for effective latent dimensionality. The diagnostic candidate dimension $\dzopt$ is not a unique optimal solution but a candidate based on multiple indicators (AU values, TwoNN ID, $\mknee$). (Note: axis labels in the figure to be updated to English in final submission.)}
\label{fig:framework}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Exp-Syn-Top: Topological manifolds and VAE $\AUsat$ (standardized conditions, observed in this study).}
...
\midrule
Swiss Roll   & 2 & $\{1\}$        & 2 & 3 (exception, preprocessing-dependent)\\
...
\begin{table}[H]
\centering
\caption{Exp-Stab Section A: TwoNN ID comparison on Swiss Roll ($d_\mathrm{true}=2$). At $m=d_\mathrm{true}=2$, both AE ($\kap=1.791$) and IsometricAE ($\kap=1.055$) underestimate TwoNN$\approx1.55$ (bottleneck constraint is the main cause, not $\kap$ difference). At $m=4$ ($>d_\mathrm{true}$), TwoNN recovers to $2.03$, consistent with input-space TwoNN (numerical stability improvement; unbiasedness not guaranteed).}
\label{tab:ep_swiss}
\small
\begin{tabular}{lcccc}
\toprule
Condition & $m$ & $\kap$ & TwoNN ID & MLE ID \\
\midrule
Raw data (3D, reference) & 3 & N/A & $2.057$ & $1.841$ \\
AE latent space & 2 & $1.791$ & $1.553$ & $1.242$ \\
IsometricAE latent space & 2 & $1.055$ & $1.539$ & $1.228$ \\
AE latent space (extra dims) & 4 & $2.155$ & $2.028$ & $2.078$ \\
\midrule
\multicolumn{5}{l}{seed=42, $n_\mathrm{train}=3000$, $n_\mathrm{test}=500$, hidden=$[64,32]$, 150 epochs} \\
\bottomrule
\end{tabular}
\end{table}
}{%
...
Condition & $m$ & $\kap$ & TwoNN ID & MLE ID \\
\midrule
Raw data (3D, reference) & 3 & N/A & $\approx 2.06$ & $\approx 1.84$ \\
AE latent space & 2 & $\approx 1.79$ & $\approx 1.55$ & $\approx 1.24$ \\
IsometricAE latent space & 2 & $\approx 1.06$ & $\approx 1.54$ & $\approx 1.23$ \\
AE latent space (extra dims) & 4 & $\approx 2.15$ & $\approx 2.03$ & $\approx 2.08$ \\
\midrule
\multicolumn{5}{l}{(approximate values; to be updated after Exp-Stab completion)} \\
\bottomrule
\end{tabular}
\end{table}
}
...
\begin{table}[H]
\centering
\caption{Exp-Geom: MNIST AE $m$-sweep --- co-variation of $\kap$ and TwoNN ID ($\hat{d}_\mathrm{ID}\approx10$). TwoNN stabilizes at $m \geq 48 \approx 5\hat{d}_\mathrm{ID}$; TwoNN remains stable even when $\kap$ jumps at $m=96$--$128$ ($9.94\to37.0$).}
\label{tab:eq_mnist}
\small
\begin{tabular}{rrrrl}
\toprule
...
12  & 2.71 & 7.25 & 0.992 & not yet converged \\
16  & 3.28 & 8.17 & 0.993 & converging \\
20  & 3.48 & 8.37 & 0.995 & converging \\
32  & 4.32 & 9.36 & 0.997 & converging \\
48  & 4.44 & \textbf{10.16} & 0.998 & \textbf{converged} ($\approx\hat{d}_\mathrm{ID}$) \\
64  & 4.25 & 10.17 & 0.998 & stable \\
96  & 9.94 & 10.86 & 0.999 & stable (despite $\kap$ increase) \\
128 & 37.0 & 10.98 & 0.999 & stable (despite $\kap$ jump) \\
\midrule
\multicolumn{5}{l}{$n_\mathrm{train}=10{,}000$, 200ep, hidden=[512,256], seed=42; $\kap$: median of $n_\mathrm{eval}=8$ points} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Exp-Stab: (Left) TwoNN ID bar chart on Swiss Roll --- at $m=d_\mathrm{true}=2$, both AE and IsometricAE underestimate TwoNN$\approx1.55$ (bottleneck margin shortage is the main cause, not $\kap$ difference); at $m=4>d_\mathrm{true}$, TwoNN recovers to $2.03$. (Right) MNIST VAE TwoNN vs $m$ convergence curve (reference AE, $\kap\approx10^9$). See Table~\ref{tab:eq_mnist} for Exp-Geom results. (Note: axis labels to be updated to English in final submission.)}
  \label{fig:ep}
\end{figure}}{}
...
\begin{table}[H]
\centering
\caption{Exp-Iso-ID: Comparison of latent-space TwoNN ID between standard AE and IsometricAE on Swiss Roll ($m \in \{2,3,4,6,8\}$, input-space TwoNN$=2.427$, $d_\mathrm{true}=2$). At $m \geq 3$ ($>d_\mathrm{true}$), despite $\kap$ differing by factor 2, TwoNN ID difference stays below $0.5\%$.}
\label{tab:iso_id}
\small
\begin{tabular}{r cc | cc | c}
\toprule
 & \multicolumn{2}{c|}{AE (standard)} & \multicolumn{2}{c|}{IsometricAE} & \\
...
\begin{figure}[H]
  ...
  \caption{Exp-Iso-ID: (Left) Latent-space TwoNN ID by $m$ --- curves for standard AE and IsometricAE nearly overlap, showing that isometry improvement has minimal effect on TwoNN ID; (Right) Comparison of decoder condition number $\kap$ --- IsometricAE maintains $\kap \approx 1.1$ across all $m$, while AE rises to $\kap=2.10$ at $m=3$. (Note: axis labels to be updated to English in final submission.)}
  \label{fig:iso_id}
\end{figure}}{}
...
\midrule
PCA (80\% cumulative variance) & 43 dims & Exp-Mnist-Base-AE & Linear, tends to overestimate \\
PCA (90\% cumulative variance) & 86 dims & Exp-Mnist-Base-AE & Linear, even larger overestimate \\
MSE elbow (AE) & $m \approx 16$--20 & Exp-Mnist-Base-AE & Intuitive, requires smoothing \\
TwoNN ID (reference AE) & $\approx 10$--12 & Exp-Mnist-Base-AE & Nonlinear but subject to isometry bias \\
\textbf{$\mknee$ (VAE, $\tau=0.25$)} & \textbf{12}* & Exp-Mnist-Base-VAE & Direct observation of KL dynamics; depends on $\tau$, $m$ grid, seed \\
\multicolumn{4}{l}{*Single-seed extended experiment (seed 42, $m \in \{1,\ldots,256\}$); varies 32--64 in multi-seed experiments} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Exp-Mnist-Multi: Mean$\pm$std of AU and TwoNN ID for 3 seeds (42, 123, 777) at 150 epochs ($\beta=4$). AU values are highly consistent across seeds, but $\mknee$ varies with convergence.}
...
\begin{figure}[H]
  ...
  \caption{Exp-Mnist-Multi: AU patterns of MNIST VAE (3 seeds). Left: AU vs $m$ per seed (dotted lines: $\mknee$ per seed); Right: mean$\pm$std band. AU values are highly consistent across seeds, but $\mknee$ varies between 32 and 64. (Note: axis labels to be updated to English in final submission.)}
...
\begin{table}[H]
\centering
\caption{Exp-Mnist-Multi: Mean$\pm$std of AU and TwoNN ID for 3 seeds at 300 epochs ($\beta=4$). AU and TwoNN ID are highly consistent, but $\mknee$ still varies.}
...
\begin{figure}[H]
  ...
  \caption{Exp-Mnist-Multi: AU patterns of MNIST VAE (3 seeds, 300 epochs). Left: AU vs $m$ per seed (dotted lines: $\mknee$); Right: mean$\pm$std band. AU and TwoNN ID bands are very narrow, but $\mknee$ varies between 32 and 64 across seeds. (Note: axis labels to be updated to English in final submission.)}
...
\begin{table}[H]
\centering
\caption{Exp-Mnist-PCA: Linear theory prediction ($\beta=4$) from MNIST PCA spectrum vs.\ nonlinear VAE observations.}
\label{tab:theory_pred}
\small
\begin{tabular}{ccc|l}
\toprule
$\tau^2$ & Water level $\beta\tau^2$ & Linear theory $d^*$ & Note \\
\midrule
1.0  & 4.0  & 1   & Conventional assumption ($\lambda_1=5.22$ only exceeds)\\
0.30 & 1.20 & 10  & Range where $d^*=10$: $\tau^2 \in (0.28, 0.31)$ \\
0.25 & 1.00 & 12  & Setting for $d^*=12$ \\
0.14 & 0.57 & 20  & Setting for $d^*=20$ ($\tau^2 \in (0.138, 0.150)$) \\
\midrule
\multicolumn{3}{c|}{Nonlinear FC-VAE observations (300ep, 3 seeds)} & AU(64)$=22.3\pm0.5$; TwoNN ID$(64)=10.03\pm0.12$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Exp-Mnist-PCA: Comparison between linear theory (Theorem \ref{thm:linear_vae}) and nonlinear MNIST VAE (Exp-Mnist-Base, 300ep, 3 seeds).}
\label{tab:theory_inherit}
\small
\begin{tabular}{lll}
\toprule
Property & Linear theory prediction & Nonlinear VAE observation \\
\midrule
Monotonicity of AU$(m)$ & Monotone non-decreasing (guaranteed in linear setting) & Approximately increasing on average (3-seed mean); non-monotone fluctuations observed in single-seed extended grid (Table~\ref{tab:e4_ae}) \\
$d^*$ at $\tau^2=1$ & $d^*=1$ (only $\lambda_1=5.22>4$) & AU$(64) \approx 22$ (large discrepancy) \\
$\tau^2$ consistent with $d^* \approx 10$ & $\tau^2 \approx 0.30$ (back-calculated) & Effective noise of nonlinear decoder $\ll 1$ \\
Sharpness of AU saturation & Sharp step ($r\to 0$ at $m=d^*$) & Gradual increase (instability $\sigma=15$ in $\mknee$) \\
Numerical reproducibility of $\mknee = d^*$ & Exact match (assuming sharp transition) & Not reproduced ($\mknee \in \{32,64\}$; §\ref{sec:exp_multiseed2}) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Exp-Mnist-PCA: (Left) MNIST PCA eigenvalue spectrum and linear-theory water-filling level ($\beta\tau^2 \approx 1.18$, $d^*=10$). (Right) Comparison of linear theory prediction (red solid line; sharp saturation assumed at $d^*=10$) and observed AU$(m)$ curve of nonlinear FC-VAE (per seed and mean). The gradual AU increase in the nonlinear VAE provides the theoretical basis for $\mknee$ instability. (Note: axis labels to be updated to English in final submission.)}
  \label{fig:em}
\end{figure}}{}
...
\begin{table}[H]
\centering
\caption{Exp-Mnist-Dense: Comparison of $\mknee$ between dense and sparse grids ($\tau=0.25$, MNIST FC-VAE, $\beta=4$). Validates the grid-sensitivity bound of Proposition \ref{thm:mknee_instab}.}
\label{tab:el_mknee}
\small
...
\midrule
Sparse grid, 150ep (Exp-Mnist-Base) & 32 & 64 & 32 & 64 & $53 \pm 15$ \\
Sparse grid, 300ep (Exp-Mnist-Base) & 32 & 64 & 32 & 32 & $43 \pm 15$ \\
Dense grid, 300ep (Exp-Mnist-Dense) & 8 (step 1 for 8--16) & 11 & 9 & 11 & $10.3 \pm 0.9$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Exp-Mnist-Dense: (Left) Dense-grid AU vs $m$ (3 seeds). (Right) Comparison by grid density $\times$ epochs. $\mknee$ variation in the dense grid can be assessed by the upper bound of Proposition~\ref{thm:mknee_instab} (maximum grid interval). (Note: axis labels to be updated to English in final submission.)}
  \label{fig:el}
\end{figure}}{}
...
\begin{figure}[t]
  ...
  \caption{Exp-Fashion: $m$-dependence of MSE, AU, and TwoNN ID for Fashion-MNIST AE (left) and VAE (right) (100 epochs). In the VAE, the AU growth rate drops sharply around $m=12$ ($\mknee$ at $\tau_{\mathrm{knee}}=0.25$), and TwoNN ID asymptotically converges to $\approx 9.4$ (AE) and $\approx 8.7$ (VAE) at $m \geq 32$. (Note: axis labels to be updated to English in final submission.)}
  \label{fig:ef}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Exp-Conv-AE: $m$-dependence of MSE, AU, and TwoNN ID for MNIST Conv-AE (left) and Conv-VAE (right) (100 epochs). Conv-AE confirms AU$=m$ (all dims active) with TwoNN ID converging to $\approx 11$ at $m \geq 32$. Conv-VAE shows AU$\approx m$ (no saturation due to insufficient convergence at 100 epochs). (Note: axis labels to be updated to English in final submission.)}
  \label{fig:eg}
\end{figure}}{}
...
\begin{table}[H]
\centering
\caption{Exp-Conv-M512: MNIST Conv-VAE with $m \in \{384, 512\}$ (3-seed mean; KL cyclic annealing, $\beta_\mathrm{max}=4$, 150ep). No clear AU saturation plateau observed up to $m=512$.}
\label{tab:en_ext}
\small
...
\caption{Exp-ConvV-HiBeta: Comparison of MNIST Conv-VAE AU vs $m$ (left) and activation rate AU/$m$ (right) for $\beta_{\max} \in \{4, 10, 20\}$. Increasing $\beta_{\max}$ shifts AU saturation to lower dimensions, demonstrating the $\beta$-dependence of $d^*(\beta)$. (Note: axis labels to be updated to English in final submission.)}
\label{fig:hibeta}
\end{figure}
}{}
...
\begin{figure}[H]
  ...
  \caption{Exp-Nat-Cifar: $m$-dependence of AU and TwoNN ID for CIFAR-10 Conv-VAE (KL cyclic annealing, 3 seeds). TwoNN ID shows convergence consistent with Pope et al.\cite{pope2021intrinsic} ($d_\mathrm{ID} \approx 26$). (Note: axis labels to be updated to English in final submission.)}
  \label{fig:ei}
\end{figure}}{}
...
\begin{figure}[H]
  ...
  \caption{Exp-Nat-SVHN: $m$-dependence of AU and TwoNN ID for SVHN Conv-VAE (KL cyclic annealing, 3 seeds). (Note: axis labels to be updated to English in final submission.)}
  \label{fig:ej}
\end{figure}}{}
...
\begin{figure}[H]
  ...
  \caption{Exp-NatImg-AU: AU dynamics of CIFAR-10 Conv-VAE. (Left) Arch-A (Standard), (Right) Arch-B (Deep). Dotted line: AU$=m$ reference (no saturation). Increasing $\beta_{\max}$ strengthens AU suppression, but no saturation plateau is reached under any condition. (Note: axis labels to be updated to English in final submission.)}
  \label{fig:natimg_au}
\end{figure}}{}
...
\begin{table}[H]
\centering
\caption{Exp-NatImg-Sweep: Main results of CIFAR-10 extended sweep (mean$\pm$std, 3 seeds). Under $\beta_\mathrm{max}=80$, Thin decoder, AU plateau-like behavior observed at $m=512$ (AU$=24.1\pm2.8$). All five stable-identification criteria remain unmet (§\ref{sec:natimg_stable_id}).}
\label{tab:natimg_sweep}
\small
\begin{tabular}{llcccl}
\toprule
$\beta_{\max}$ & Decoder & $m$ & AU & TwoNN ID (latent) & Note \\
\midrule
4  & Standard & 384 & $208\pm12$ & $25.4\pm0.5$ & 参照：Exp-Nat-Cifar（\S\ref{sec:exp_natural}）の再掲；Exp-NatImg-AU（$\beta_{\max}\geq10$）とは別設定 \\
10 & Standard & 384 & $185\pm15$ & $25.2\pm0.4$ & --- \\
20 & Standard & 512 & $241\pm18$ & $25.3\pm0.4$ & no saturation observed \\
40 & Standard & 512 & $210\pm21$ & $25.1\pm0.5$ & no saturation observed \\
40 & Deep      & 512 & $225\pm19$ & $25.4\pm0.4$ & no saturation observed \\
...
\begin{table}[H]
\centering
\caption{Exp-NatImg-Knee: Comparison of $\mknee$ detection methods for CIFAR-10 ($\beta_\mathrm{max}=80$, thin decoder) over 3 seeds. Smaller inter-seed $\sigma$ indicates higher reproducibility.}
...
\textbf{本論文のコアメッセージ}：
\begin{quote}
AU 値（頑健指標，$\sigma \leq 0.5$）と TwoNN ID（数値安定な補助的参照推定量）を
主要な読み取り指標とし，$\mknee$（探索的指標）はその補完として用いる——
この役割分担の明確化が本多指標診断フレームワークの中心的貢献である．
\end{quote}
...
\begin{table}[H]
\centering
\caption{Three-tier summary of framework applicability (validity range). A: stable operation confirmed; B: partial confirmation with limiting conditions; C: stable identification not achieved.}
\label{tab:applicability_tier}
\small
\begin{tabular}{p{3.5cm}p{2.6cm}p{2.4cm}p{2.4cm}p{3.5cm}}
\toprule
Setting & TwoNN ID & AU monitoring & $\mknee$ identification & Evidence \\
\midrule
\textbf{FC-VAE +} \textbf{MNIST/F-MNIST}
  & A ($\sigma\leq0.12$)
  & A ($\sigma\leq0.5$)
  & B (dense grid: $10.3\pm0.9$)
  & Exp-Mnist-Base, Exp-Mnist-Dense, Exp-Fashion \\
\addlinespace
\textbf{Conv-VAE + MNIST}
  & A (stable)
  & B (AU$<m$ onset at $m\geq96$)
  & B ($\mknee$ identifiable at $\beta\geq10$; not reached at $\beta=4$)
  & Exp-Conv-Fix, Exp-Conv-Ann, Exp-Conv-M256/M512, Exp-ConvV-HiBeta \\
\addlinespace
\textbf{Natural images}
\textbf{(CIFAR-10/SVHN)}
  & A (consistent with Pope et al.)
  & B (plateau-like behavior at $\beta_{\max}=80$, thin decoder)
  & C (all 5 criteria unmet; stable ID not achieved)
  & Exp-Nat-Cifar, Exp-NatImg-Sweep, Exp-NatImg-Knee \\
\midrule
\multicolumn{5}{p{14.5cm}}{\small
Stage 1 $\hat{d}_\mathrm{ID}$ is a supplementary reference estimator subject to isometry bias ($\kap\approx10^9$; numerical stability demonstrated in §\ref{sec:exp_ep}). $\mknee$ is an exploratory indicator in all settings, not a unique optimal bottleneck dimension.}\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{$\beta$-sensitivity analysis: AU vs $m$ for MNIST VAE ($\beta \in \{1,2,4,8\}$, 50 epochs, subset). At $\beta=1$, AU$\approx m$ (insufficient regularization); at $\beta=4$, $m_{\mathrm{knee}}=12$ (vertical dashed line); at $\beta=8$, signs of excessive AU suppression at small $m$. (Note: axis labels to be updated to English in final submission.)}
