...
%  MDPI MAKE submission — main_paper_MAKE31.tex
%  English version of main_paper_NN31.tex (Japanese draft, version 31)
...
This occurs because the normalized increase rate $\rho(m)$ fluctuates subtly around the threshold
...
(see \S\ref{sec:exp_el} for the experimental setup).
...
\begin{itemize}
  ...
  \item \textbf{Grid-density factor (one of the main causes)}: with the dense grid (Exp-Mnist-Dense), $\mknee = 10.3 \pm 0.9$ (per-seed values 11, 9, 11) --- a dramatic improvement over the sparse-grid $43\pm15$, consistent with the estimate $\hat{d}_\mathrm{ID} \approx 10$
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Multi-seed validation of the main experiments}:
        ...
        single-seed only.
        In this paper we added Exp-Mnist-Multi-150 (150 epochs, 3 seeds; \S\ref{sec:exp_multiseed})
        and Exp-Mnist-Multi-300 (300 epochs, 3 seeds; \S\ref{sec:exp_multiseed2}).
...
(the 300-epoch multi-seed experiment Exp-Mnist-Multi-300 and the dense-grid
factor-separation experiment Exp-Mnist-Dense-300;
...
($\approx 26$) (the CIFAR-10 Conv-VAE experiment Exp-Nat-Cifar);
